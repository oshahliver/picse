...
real(8), dimension(5) :: xi_all_core, X_all_core
...
Fe_number_layers, eps_Al, eps_H2O, omega, xi_H_core_predicted, &
...
eps_T_zero, eps_H2O, eps_Al, P_CS
...
real(8), intent(inout), optional :: M_core_should, R_surface_should, &
...
logical :: core_segregation_model, inner_core_segregation_model
...
!~ 		print *, 'Next layer transition reached aswell:', self%lay + i, '->', self%lay + i + 1
		...
		!but only of layer 3 is not already skipped.
		else
			if (self%lay<=2)then
