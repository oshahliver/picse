...
from pics.physicalparams import T0_list, K0_list, K0prime_list, rho0_list, \
                                ...
                                temperature_jumps, mS, material_YMg, \
                                material_YSi, material_YO, material_YH, \
                                material_YS
from pics.runparams import eps_Psurf, eps_Mtot, eps_layer, param_colors, \
...
def convergence_test(eps_list = np.array([1., .5, .1]), contents = [],
                     T_center=None, P_center=None):
    ...
    """
    yaxis_labels = [[r'$R_{tot} \ [R_{\oplus}]$', r'$P_{surf} \ [GPa]$'], 
    ...
    for d in range(len(data)):
        ...
        for e in range(len(eps_list)):
            pl = Planet(contents=contents, T_center = T_center, 
                        P_center = P_center, majorConstraint='M_tot', 
                        tempType='adiabatic', differentiated=True, 
                        layermasses=[.3, 1.], eps_r=eps_list[e],
                        rhoType = rhoType)
    ...
    for dat in data:
        for i in range(len(dat)):
            for j in range(len(dat[i])):
                ax[i][j].semilogx(eps_list, dat[i][j], marker='s', 
                ...
                if j == 1:
                    ax[i][j].tick_params(labelright='on', labelleft='off',
                      right='on', top='on')
                    ax[i][j].yaxis.set_label_position('right')
                    
                if j == 0:
                    ax[i][j].tick_params(labelright='off', labelleft='on',
                      right='on', top='on')
...
def ComputeCoreMass(contents = None, Mg_number=None, M_surface = 1.,
    ...
    Mg_number_mantle = min(Mg_number_mantle, .9999999)
    Si_number_mantle = SiMg/(SiMg + 1./Mg_number_mantle - 1)
    ...
    fractions = fortfunctions.functions.compute_abundance_vector(simg=SiMg, 
                                                     femg=1./Mg_number_mantle-1.,
                                                     n_mats=len(contents[2]),
                                                     ymgi=[material_YMg[i-1] for i in contents[2]],
                                                     ysii=[material_YSi[i-1] for i in contents[2]],
                                                     xih2oi=[0. for i in contents[2]],
                                                     xifei=[1.-Mg_number_mantle for i in contents[2]],
                                                     xialsii = [0. for i in contents[2]],
                                                     xialmgi = [0. for i in contents[2]],
                                                     contents = contents[2],
                                                     additional = [])
    
    
    Q1 = sum([fractions[i]*Mg_number_mantle*material_YMg[contents[2][i]-1] for i in range(len(contents[2]))])
    Q2 = sum([fractions[i]*Mg_number_mantle*material_YMg[contents[2][i]-1] for i in range(len(contents[2]))])*mMg+\
            sum([fractions[i]*(1.-Mg_number_mantle)*material_YMg[contents[2][i]-1] for i in range(len(contents[2]))])*mFe +\
            sum([fractions[i]*material_YSi[contents[2][i]-1] for i in range(len(contents[2]))])*mSi+\
            sum([fractions[i]*material_YO[contents[2][i]-1] for i in range(len(contents[2]))])*mO
    Q3 = sum([fractions[i]*(1.-Mg_number_mantle)*material_YMg[contents[2][i]-1] for i in range(len(contents[2]))])
    Q4 = 1.+xi_H_core/2.
    Q5 = (xi_H_core/2.*(mFe+mO) + mFe + xi_H_core*mH)
    ...
    Q = Mg_number_mantle/(1.-Mg_number_mantle)*mMg + \
        Si_number_mantle/(1.-Si_number_mantle)*mSi + \
        O_number_mantle/(1.-O_number_mantle)*mO + mFe
    
    Q1 = (M_surface-M_ocean)*(Mg_number_mantle/Mg_number-1.)
    Q2 = (1.-Mg_number_mantle)/mFe*Q + Mg_number_mantle/Mg_number - 1
    return Q1/Q2
    

def PlotCoreMasses(M_ocean = np.linspace(0, .1, 3), 
                   SiMg = np.linspace(0, 1., 32),
                   Mg_numbers = np.linspace(.5, 1., 32),
                   dim=1, figsize=(10, 5)):
    ...
    if dim == 1:
        ...
        Mg_numbers_mantle = np.linspace(.5, 1.)
        Mg_numbers = np.linspace(0.2, .7, 6)
        
        core_masses = np.zeros([len(M_ocean), len(Mg_numbers), 
                                len(Mg_numbers_mantle)])
        mantle_masses = np.zeros([len(M_ocean), len(Mg_numbers), 
                                  len(Mg_numbers_mantle)])
        
        for o in range(len(M_ocean)):
            for i in range(len(Mg_numbers)):
                for j in range(len(Mg_numbers_mantle)):
                    core_masses[o][i][j] = ComputeCoreMass(Mg_number=Mg_numbers[i],
                    ...
                    mantle_masses[o][i][j] = 1. - core_masses[o][i][j] - M_ocean[o]
                    
        plots = plotTools.Plot(col=2, row=1, 
                    axis_limits=[[[[0.5, 1.0], [0.0, 1.]], [[.5, 1.], [.0, 1.]]]], 
                    majorlocatorx=[[.1, .1]],
                    majorlocatory=[[.1, .1]], 
                    minorlocatorx=[[.05, .05]], 
                    minorlocatory=[[.05, .05]], 
                    axis_labels = [[[r'$1-\xi_{\rm Fe}$', 
                                     r'$M_{\rm Core}/M$'], 
                                    [r'$1-\xi_{\rm Fe}$', 
                                     r'$M_{\rm Mantle}/M$']]],
                    sharex=False, 
        ...
        for o in range(len(M_ocean)):
            ...
            for i in range(len(Mg_numbers)):
                ...
                pl, = ax[0].plot(Mg_numbers_mantle, core_masses[o][i],
                  linestyle = linestyles[o],
                  color=color_list[i], zorder=10)
        
                ax[1].plot(Mg_numbers_mantle, mantle_masses[o][i],
                  linestyle = linestyles[o],
                  color=color_list[i], zorder=10)
    
                if o == 0:
                    legend_list1.append(r'$\rm Mg \# = \ $'+str(round(Mg_numbers[i], 3)))
                    plot_list1.append(pl)
                    
                if i == 0:
                    legend_list2.append(r'$M_{\rm Ocean}/M = \ $'+str(round(M_ocean[o], 3)))
                    plot_list2.append(pl)
                    
                    
        legend1 = ax[0].legend(plot_list1, legend_list1, loc = 0, framealpha=.75,
                    labelspacing=0, fancybox=True)

        legend3 = ax[0].legend(plot_list2, legend_list2, loc = 1, framealpha=.75,
                    labelspacing=0, fancybox=True)
        ...
        plots.fig.savefig('/home/os18o068/Documents/PHD/Abbildungen/Figures_paper_1/core_masses.pdf',
        ...
        for i in range(len(Mg_numbers)):
            for j in range(len(SiMg)):
                data[i][j] = ComputeCoreMass(Mg_number_mantle=Mg_numbers[i], 
                    SiMg = SiMg[j], M_ocean=0.1, M_surface=1., Mg_number=0.5)
                
        fig, ax = plt.subplots()
        
        ax.imshow(data.T)
        
        fig.savefig('/mnt/c/Users/os18o068/Documents/PHD/Abbildungen/core_masses.pdf',
                    format='pdf')
        
               
class Shell():
    def __init__(self, radius=.1, T=None, m=None, P=None, d=None, layer=None, 
                 tempType = 1, status='bare', contents=[], FeMg=None, SiMg=None,
                 fractions=[], Fe_number=[], X_H2O=1., gamma=1.36, eps_r=.5, 
                 rhoType='integrate', d0 = None, adiabatType=1, 
                 q = 1.0, eps_T_zero = 0.0, saturation = False, xiBr=1., 
                 **kwargs):
        ...
        self.xiPerov = None
        self.xiBr = xiBr
        self.xiFe_mantle = FeMg/(1.+FeMg)
        
        if not sum(self.fractions) == 1.:
            pass
            '''
        ...
        self.mix = Material.Mixture(T=self.temp, P=self.pres, contents=self.contents,
                           fractions=self.fractions, Fe_number=self.Fe_number,
                           X_H2O=self.X_H2O, saturation=self.saturation,
                           **kwargs)
        ...
        self.gradients = eosfort.gradients(r=self.radius, 
                                       y=params, 
        ...
        self.initials = {'radius':self.radius, 
                         'T':self.temp, 
                         'm':self.mass, 
                         'P':self.pres,
                         'd':self.dens, 
    ...
    def prt(self, digits = 3, **kwargs):
        ...
        print ('=======================================')
        print ('Shell properties:\nradius:',self.radius*1.0e-3,
               'km \nenclosing mass:', self.mass,'kg')
        print ('\nstatus:', self.status)
        print ('\ntempType:', self.tempType)
        print ('\nThe gradients are:')
        print ('\ndP/dr [Pa/m]:', formatstr.format(Decimal(str(self.gradients[0]))), 
               '\ndm/dr [kg/m]:', formatstr.format(Decimal(str(self.gradients[1]))),
               '\ndT/dr [K/m]:', formatstr.format(Decimal(str(self.gradients[2]))),
               '\ndrho/dr [kg/m4]:', round(self.gradients[3], digits))
    ...
    def getAbundances(self, **kwargs):
        ...
        #are no other hydrated phases
            ...
            if phPer == 2:
                ...
                self.xiH2O_Per = .5*self.xiBr
                
            if phPv == 1:
                self.XH2O_Perov = .03*self.X_H2O[2]
                
            else:
                self.XH2O_Perov = 0.0*self.X_H2O[2]
            
            self.XH2O_Per = Material.eta_H2O_Ws(xi_H2O_Per=self.xiH2O_Per, 
                                                FeMg=self.FeMg)#self.mix.mix[0].X_H2O*self.X_H2O[0]
            
            #Compute water content in Perovskite as mole fraction
            self.xiH2O_Perov = Material.xi_H2O_Perov(X_H2O_Perov=
                                                self.XH2O_Perov, FeMg=self.FeMg,
                                                lay=self.layer)
            
            #Compute water content in Wuestite as mole fraction
            self.xiH2O_Ws = Material.xi_H2O_Ws(X_H2O_Per=self.XH2O_Per,
                                               FeMg=self.FeMg, lay=self.layer)
            
            #Compute mole fraction of Wuestite
            self.xiWs = Material.xi_Ws(SiMg=self.SiMg, FeMg=self.FeMg,
                                       xi_H2O_Ws=self.xiH2O_Ws, lay=self.layer)
            ...
            self.fractions = [xiWs_Mg*self.xiBr, xiWs_Mg*(1.-self.xiBr), 
                              xiWs_Fe, self.xiPerov]
        ...
        #adjusted to the present water content
            ...
            self.xiH2O_Ol = Material.xi_H2O_Ol(X_H2O_Ol=self.XH2O_Ol, 
                                               FeMg=self.FeMg, lay=self.layer)
            
            #Compute mole fraction of Olivine
            self.xiOl = Material.xi_Ol(SiMg=self.SiMg, FeMg=self.FeMg,
                                  xi_H2O_Ol=self.xiH2O_Ol, lay=self.layer)
            
            #Compute mole fraction of Enstatite
            self.xiEn = 1.- self.xiOl
    ...
    def getContents(self, **kwargs):
        ...
        if self.layer < 2:
            ...
            if self.contents == [9] or self.contents == [9, 9]:
                ...
                self.N_tot = self.indigenous_mass/(mFe+.13*mS)
        ...
        #composition is (Mg,Fe)O + (Mg,Fe)SiO3    
            ...
            self.N_tot = self.indigenous_mass/(
                    self.xiPerov * (1.-self.xiH2O_Perov)*m_perovskite + \
                    self.xiWs * (1.-self.xiH2O_Ws)*m_wuestite + \
                    (self.xiWs*self.xiH2O_Ws+self.xiPerov*self.xiH2O_Perov)*mH2O)
            
            #Comptue molar abundances of constituents
            self.N_Mg = self.N_tot*(1.-self.xiFe_mantle)
            self.N_Fe = self.N_tot*self.xiFe_mantle
            self.N_Si = self.N_tot*self.xiPerov
            self.N_H2O = self.N_tot*(self.xiWs*self.xiH2O_Ws + \
                                     self.xiPerov*self.xiH2O_Perov)
        ...
        #composition is (Mg,Fe)2SiO4 + (Mg,Fe)2Si2O6
            ...
            #Compute total amount of moles in the shell
            self.N_tot = self.indigenous_mass/(self.xiEn * m_enstatite + self.xiOl * \
                        (1-self.xiH2O_Ol)*m_olivine + \
                        self.xiOl*self.xiH2O_Ol*mH2O)
            
            #Compute molar abundances of constituents
            self.N_Mg = self.N_tot*2*(1-self.xiFe_mantle)*(1-self.xiOl*self.xiH2O_Ol)
            self.N_Fe = self.N_tot*2*self.xiFe_mantle*(1-self.xiOl*self.xiH2O_Ol)
    ...
    def Construct(self, dr=1., overconstruct = False, fortran=False, **kwargs):
        ...
        if self.status == 'constructed' and not overconstruct:
            ...
            print ('''pass overconstruct=True to the Construct method to ignore
            this message''')
            ...
            self.radius, newy, newgrads = eosfort.integrate(r_start=self.radius, 
                                h=dr,
                                y_in=params, 
                                gammag0=self.gamma, 
...
class Layer():
    def __init__(self, r_in = None, r_out=None, T=None, P=None, contents=[], \
                 fractions = [], m=None, eps_r=.5, tempType = 1,\
                 Fe_number=[], X_H2O=1., rhoType='integrate', 
                 mass_should=None, FeMg=None, SiMg=None, layer=None, 
                 gamma = 1.36, d0=None, adiabatType=1, q=1.0, 
                 eps_T_zero = 0.0, saturation = False, xiBr=1., **kwargs):
        ...
        self.d0 = d0
        self.q = q
        self.eps_T_zero = eps_T_zero #parameter that defines how low temperatures are handeled
        
        #set initial conditions
        self.pres, self.mass, self.temp = P, m, T
        self.mass_should = mass_should
        self.indigenous_mass = 0. #mass contained within the layer
        self.contents=contents
        self.fractions=fractions
        self.saturation = saturation
        ...
        if len(fractions)==0:
            ...
            frac  = 1./len(self.contents)
            self.fractions.append(frac)
            
        if len(Fe_number) == 0:
            print ('setting Fe# = 0.0')
            self.Fe_number = [0. for i in range(len(self.contents))]
        ...
        try:
            shell_boundary = Shell(T=self.temp, P=self.pres, contents=self.contents,
                      fractions =self.fractions, radius=self.radius,
                      m=self.mass, tempType = self.tempType, layer=self.layer,
                      Fe_number=self.Fe_number, X_H2O=self.X_H2O, 
                      rhoType=self.rhoType, FeMg=self.FeMg, SiMg=self.SiMg,
        ...
        shell = Shell(T=self.temp, P=self.pres, contents=self.contents,
                           fractions =self.fractions, radius=self.radius,
                           m=self.mass, tempType = self.tempType,
                           Fe_number=self.Fe_number, X_H2O=self.X_H2O, 
                           rhoType=self.rhoType, FeMg=self.FeMg, SiMg=self.SiMg, 
                           layer=self.layer, gamma=self.gamma, 
                           d0=self.d0, adiabatType=self.adiabatType,
                           q=self.q, 
                           eps_T_zero = self.eps_T_zero,
                           saturation=self.saturation,
                           xiBr=self.xiBr,
    ...
    def Update(self, **kwargs):
        ...
        self.radius, self.pres, self.mass, self.temp, self.dens = \
            shell.radius, shell.pres, shell.mass, shell.temp, shell.dens
    ...
    def major_bisection(self, param_is, param_should, eps, direction):
        ...
        if direction[0]*(param_should - param_is)/param_is < direction[1]*eps\
        or param_is < 0.:
            self.shells.pop(-1)
            self.shells[-1].reset()
            self.Update()
    ...
    def construct_all_shells(self):
        ...
        while self.shellIteration:
            ...
            if self.pres < P_zero:
                print ('Zero pressure reached at', 
                       self.pres,'Pa after', 
                       self.shellIterationCount, 'iterations')
            ...
            if self.contents == [0]:
                if self.shells[-1].mix.mix[0].phase == 'vapor':
                    ...
                    print ('Water vapor phase reached at', 
                           round(self.temp, 3) ,'K, ', round(self.pres*1.0e-5, 3),' bar', 'after', 
                           self.shellIterationCount, 'iterations')
            ...
            self.major_bisection(param_is=self.mass/m_earth, 
                             param_should=self.mass_should, eps=eps_Mtot, 
                             direction=[1, -1])
        
        
    def Plot(self, pres_unit='GPa', temp_unit='K', dens_unit = 'kg/m3', 
             mass_unit = 'm_earth', log=True, **kwargs):
       
        title_list = ['Pressure ['+pres_unit+']', 
                      'Mass ['+mass_unit+']', 
                      'Density ['+dens_unit+']', 
                      'Temperature ['+temp_unit+']']
        
        radius_list = np.array([shell.radius for shell in self.shells])
        plot_list = np.array([[shell.pres for shell in self.shells],
        ...
        axes[2].set_xlabel('Radius [km]'); axes[3].set_xlabel('Radius [km]')
...
class Planet():
    def __init__(self, P_center=1.0e12, 
        ...
        for l in range(len(self.contents)):
            ...
            mix = Material.Mixture(contents=self.contents[l], 
                                   Fe_number=[FeMg_mantle 
                                    for ll in range(len(self.contents[l]))],
        ...
        self.seed_material = Material.Mixture(contents=self.contents[0], 
                                              fractions=self.fractions[0],
                                            P=self.P_center, T=self.T_center,
                                            saturation = self.saturation[0])
        self.seed_material.Compute()
        self.rho_center = self.seed_material.dens
        self.M_seed = 4./3.*np.pi*self.rho_center*R_seed**3        
        self.seed = Layer(m=self.M_seed, 
                          T=self.T_center, 
                          P=self.P_center, 
                          tempType = self.tempType,
                          r_in = self.R_seed, 
    ...
    def prt(self, digits=4, **kwargs):
        ...
        try:
            ...
            print ('R_surface_is [R_earth]:', round(self.R_surface_is/r_earth, 
               ...
               '\nM_H2O [wt%]:', round(self.H2O_count*mH2O/self.M_surface_is*100, digits),
        ...
        for i in range(len(self.layers)):
            ...
            dat.append([i, material_str, ftool.scinot(lay.radius/r_earth, digits=digits),
    ...
    def GetLayerConstraints(self, lay=None, skip=0, **kwarg):
        ...
        if self.layerConstraint[lay+skip] == 'mass':
            ...
            if skip > 0:
                self.constraintValue_is = 0.
            
            self.constraintValue_should = \
                self.layermasses[lay+skip]*m_earth
            ...
            self.constraintValue_should = \
            (self.layermasses[0]+self.layermasses[1]+self.layermasses[3])*m_earth
        ...
        #extract layer radius in m
            ...
            self.constraintValue_should = \
                self.layerradii[lay+skip]*r_earth
    ...
    def minor_bisection(self, **kwargs):
        ...
        if not self.layerConstraint[self.lay] == 'none':
            ...
            reldev = (self.constraintValue_should - self.constraintValue_is)/\
            self.constraintValue_should
            ...
            #the desired value to specified accuracy                                    
                ...
                #initiate next layer
                    ...
                    if not self.lay == len(self.contents) - 1:
                        ...
                        if abs(reldev) < eps_layer or \
                        reldev*self.direction < -eps_layer:
                            skip = 1
                            #if this was the last layer, then
                            #terminate internal structure integration
                    ...
                    for i in range(1+skip):
                        ...
                        nextlayer = Layer(m=mass, 
                        T=new_T, 
                          P=pres, \
                          tempType = self.tempType, \
                          r_in = radius, \
                          contents= \
                          self.contents[self.lay+1], \
                          fractions=\
                          self.fractions[self.lay+1],\
                          eps_r=self.eps_r,
                          X_H2O=self.X_H2O[self.lay+1],
                          Fe_number=\
                          self.Fe_number[self.lay+1],
                          rhoType=self.rhoType,
                          layer=self.lay+1,
                          FeMg=self.FeMg_mantle,
                          SiMg=self.SiMg,
    ...
    def major_bisection(self, **kwargs):
        ...
        #Overshoot
        if direction[0]*reldev < direction[1]*eps\
        or param_is < 0.:
    ...
    def Construct(self, print_time=False, fortran=True, echo=False,  **kwargs):
        ...
        if self.status=='constructed':
            ...
            while self.layerIteration:
                ...
                if self.echo:
                    ...
                    for m in range(len(self.layers[-1].materials)):
                        print (self.layers[-1].fractions[m]*100,'%',
                               self.layers[-1].materials[m])

                    tabl=tabulate([], headers=["R [R_e]", "P [MPa]",\
                                            'm [M_e]', 'T [K]','rho [kg m-3]',
                                            'dr [km]', 'layer', 'contents'])
                    print()
                    print (f"{tabl}")
                    print ()
                    print ('check')
                ...
                if self.layermasses[self.lay] == 0.:
                    self.shellIteration = False
                    self.layerIteration = False
                ...
                while self.shellIteration:
                    ...
                    if self.layers[-1].temp < self.T_zero[self.lay]:
                        print ('Zero temperature reached at', 
                               round(self.layers[-1].temp,2),'K after', 
                               self.shellIterationCount, 'iterations')
                    ...
                    if self.contents[-1] == [0] and self.vapor_stop:
                        if self.layers[-1].shells[-1].mix.mix[0].phase == \
                            'vapor' or self.layers[-1].dens < 10:
                            print ('Water vapor phase reached at', 
                                   round(self.layers[-1].temp, 3) ,'K, ', 
                                   round(self.layers[-1].pres*1.0e-5, 3),' bar', 
                                   'after', 
                                   self.shellIterationCount, 'iterations')
                    ...
                    if self.differentiated and not self.vapor_reached and not\
                    self.majorComplete:
                    ...
                    if self.echo:
                        ...
                        try:
                            dat = [[ftool.scinot(radius/r_earth, digits=digits),
                                ...
                                ftool.scinot(self.layers[-1].dr/1000., 
                                             digits=digits),
                                self.lay, 
                        ...
                        tabl = tabulate(dat, headers=["R [R_e]", "P [GPa]",\
                                            'm [M_e]', 'T [K]','rho [kg m-3]',
                                            'dr [km]', 'layer', 'contents'])
                        ...
                        if self.layerComplete:
                            print ('\nDesired pressicion for layer', self.lay-1,
                           'reached after',self.shellIterationCount,'iterations')
                        
                if self.majorComplete and self.echo:
                    print ('\nDesired pressicion for', self.majorConstraint,\
                   'reached after',self.shellIterationCount,'iterations')
            ...
            try:
                ...
                try:
                    self.ocean_frac_is = np.log10(self.layers[4].indigenous_mass/\
                                    self.layers[4].mass)
            ...
            if self.add_atmosphere:
                self.atmosphere = Atmosphere.Atmosphere(r_in=self.R_tot, \
                                                        T=self.T_surface_is, \
                                                        P=self.P_surface_is, \
                                                        contents=[0],\
                                                        m=self.M_surface_is, \
        ...
        self.layer_properties = [{'P_out':None, 'T_out':None, 'rho_out':None,
                                   'R_out':None, 'indigenous_mass':0.} 
                                    for i in range(len(self.layers))]
        ...
        for i in range(len(self.layers)):
            ...
            self.layer_properties[i]['indigenous_mass'] = \
                                        self.layers[i].indigenous_mass
    ...
    def construct_atmosphere(self, contents=[0], tempType='isothermal', **kwargs):
        ...
        if self.have_atmosphere:
            print ('''NOTE: atmosphere has already been constructed or will
    ...
    def Update(self, force=False):
        ...
        if not self.status == 'constructed':
            print ('''WARNING: You are about to update a non-constructed planet.
        ...
        if self.status == 'constructed' or force == True:
            ...
            self.layermasses = [lay.indigenous_mass/m_earth for lay in 
                                self.layers]
            self.X_H2O = [lay.X_H2O for lay in self.layers]
            self.Fe_number = [lay.Fe_number for lay in self.layers]

            self.Mg_number_is = Material.Mg_number(contents=self.contents,
                                            layers=self.layers,
                                            X_H2O=self.X_H2O)
    ...
    def computeEquilibriumTemperature(self, N=1, T_star = 5800., a_mean=1.,
                                      albedo=.3, eps_emm=.78, eta_cloud=0.):
...
class Population():
    def __init__(self, contents=[], P_center=1.0e12, itType='cont', \
                 fractions=[], layermasses=[], T_center=300.01, \
                 differentiated=False, tempType='isothermal', \
                 P_dissoc=5.0e11, f_dissoc=1., **kwargs):
    ...
    def generate(self, **kwargs):
        ...
        if self.itType == 'cont':
            ...
            for p in range(len(self.contents)):
                ...
                pla = Planet(P_center=self.P_center[p],\
                             T_center=self.T_center, \
                             tempType=self.tempType, \
                             R_seed=2.,\
        ...
        #iterate over central pressures
            ...
            for p in range(len(self.P_center)):
                ...
                pla = Planet(P_center=self.P_center[p],\
                             ...
                             fractions=self.fractions[p], \
                             layermasses=lm,\
                             differentiated=self.differentiated, **kwargs)
...
class MR_relation():
    def __init__(self, P_min=1.0e10, P_max=5.0e12, T=300,\
                 contents=[], fractions =[], layermasses=[], \
                 tempType='isothermal', T_center=300, differentiated=False,\
                 f_dissoc=[], P_dissoc=[], **kwargs):
        ...
        if len(self.fractions)==0:
            ...
            for c in range(len(self.contents)):
                ...
                for cc in range(len(self.contents[c])):
                    frac  = 1./len(self.contents[c][cc])
                    self.fractions[c].append([frac 
                                  for i in self.contents[c][cc]])
    ...
    def generate(self, N=5, scaling = 'log', **kwargs):
        ...
        if scaling == 'lin':
            self.P_center=np.linspace(self.P_min, self.P_max, self.N, 
                                      endpoint=True)
            
        elif scaling == 'log':
            self.P_center=np.logspace(np.log10(self.P_min), 
                                      np.log10(self.P_max), self.N, \
                                      endpoint=True) 
        ...
        for i in range(len(self.contents)):
            ...
            pop = Population(contents=contents, P_center=self.P_center, \
                             T_center=self.T_center, fractions=fractions, \
    ...
    def plot(self, axis=None, replot=False, solar=False, log=True, \
             colors=[], linestyles=[], legends=[], tick_params={}, **kwargs):
                ...
                if not len(legends) == len(self.populations):
                    ...
                    for i in range(len(pla.contents)):
                        ...
                        for j in range(len(pla.contents[i])):
                            lab = ' '.join([str(100*pla.fractions[i][j])+r'$\% \ $', \
        ...
        for m in range(len(self.M_tot)):
            ...
            if log:
                plot, = self.ax.loglog(np.asarray(self.M_tot[m])/m_earth, \
                      np.asarray(self.R_tot[m])/r_earth, color=col, \
                      linestyle = lnst, marker='o')
            
            else:
                plot, = self.ax.plot(np.asarray(self.M_tot[m])/m_earth, \
                      np.asarray(self.R_tot[m])/r_earth, color=col, \
                      linestyle = lnst, marker='o')
        ...
        if solar:
            self.ax.scatter(solar_system[0], solar_system[1], color='k',
                            s=5, zorder=10)
