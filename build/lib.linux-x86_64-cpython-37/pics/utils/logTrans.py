    ...
    if dim == 1:
        ...
        for i in range(nx):
            if not math.isnan(data_dummy[i]):
                if data_dummy[0] / abs(data_dummy[0]) == data_dummy[i] / abs(
                    data_dummy[i]
                ):
                    pass
        ...
        if sign_inversion:
            for i in range(nx):
                ...
                if not dat == 0.0:
                    ...
                    data_dummy[i] = (np.log10(abs(dat)) - int(exp_min)) * dat / abs(dat)
        ...
        for i in range(nx):
            for j in range(ny):
                ...
                if not math.isnan(data_dummy[i][j]):
                    if data_dummy[0][0] / abs(data_dummy[0][0]) == data_dummy[i][
                        j
                    ] / abs(data_dummy[i][j]):
                        pass
            ...
            for i in range(nx):
                for j in range(ny):
                    ...
                    if not dat == 0.0:
                        ...
                        data_dummy[i][j] = (
                            (np.log10(abs(dat)) - int(exp_min)) * dat / abs(dat)
                        )
...
def plot(
    ...
    if sign_inversion:
        exp_max_plot = round(np.nanmax(data) + 0.5, 0)
        exp_min_plot = round(np.nanmin(data) - 0.5, 0)
        ...
        exp_max_plot = round(exp_max + 0.5, 0)
        exp_min_plot = round(exp_min - 0.5, 0)
        n_ticks = int(exp_max_plot - exp_min_plot) + 1
    ...
    fig, ax = plt.subplots()
    im = ax.imshow(
        data.T, cmap=cmap, extent=[0.0, 1.0, 0.0, 1.0], norm=norm, origin="lower"
    )
