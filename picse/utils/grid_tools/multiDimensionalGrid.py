...
import pandas as pd
from picse.physicalparams import material_list
import numpy as np
import os
import eosTables
import Material
...
from picse.runparams import param_labels, param_lims, water_phases
...
class Table:
    def __init__(
        ...
        for i in range(len(self.alphas)):
            ...
            if self.scalings[i] == "log":
                length = (
                    int(self.ends[i]) - int(self.starts[i])
    ...
    def get_index(self, val=None, which=0):
        if self.scalings[which] == "log":
            ...
            ind = (
                9 * 10 ** self.alphas[which] * (exponent - self.starts[which]) + offset
            )
            ...
            ind = int(
                val
                * (len(self.single_axes[which]) - 1)
                / (self.ends[which] - self.starts[which])
                + 0.5
            )
    ...
    def get_indices(self, val=None, which=0, order=1):
        ...
        if self.scalings[which] == "log":
            ...
            ind = [
                9 * 10 ** self.alphas[which] * (exponent - self.starts[which])
                + offset
                + r
                for r in relative_coordinates[order - 1]
            ]
            ...
            ind = [
                int((val - self.starts[which]) / delta) + r
                for r in relative_coordinates[order - 1]
            ]
    ...
    def for_recursive(
        self, c=0, iter_list=[], f=None, ranges=None, count=0, n=None, **kwargs
    ):
    ...
    def construct_grid(self, **kwargs):
        ...
        self.for_recursive(f=self.compute_input_grid, ranges=self.ranges)
    ...
    def compute_output_grid(
        ...
        **kwargs,
        ...
        if table_data:
            ...
            pnt = [rho, None, None, dTdP_S, dPdrho, None, 0.0, alpha, 0.0]

        else:
            if self.eps_H2O > 0.9999:
                sat = True
            else:
                sat = False
        ...
        if mix:
            ...
            pnt = [rho, None, None, 0.0, dPdrho, None, 0.0, alpha, 0.0]
        ...
        self.axes[ph][tuple(iter_list)][-3] = pnt[
            7
        ]  # alpha_th  #np.sqrt(x**2+y**2+z**2)
    ...
    def partial_row(self, iter_list, row_index=0, vals=None):
    ...
    def index_row(self, iter_list=None, order=1):
        ...
        """
        # print (iter_list)
        self.index_matrix[self.count] = [
            self.indices[i][iter_list[i]] for i in range(len(self.alphas))
        ]
        self.count += 1
    ...
    def construct_index_matrix(self, vals=None, order=1):
        ...
        """
        self.index_matrix = np.zeros(
            [(order + 1) ** len(self.alphas), len(self.alphas)], dtype="int"
        )

        self.indices = [
            self.get_indices(val=vals[i], which=i) for i in range(len(self.alphas))
        ]

        self.count = 0
        iter_ranges = [range(0, order + 1) for i in range(len(self.alphas))]
        self.for_recursive(f=self.index_row, ranges=iter_ranges)
        self.count = 0
    ...
    def row(self, iter_list=None, which=0, order=1):
        ...
        vals = self.axes[self.phase][tuple(self.index_matrix[self.count])][
            0 : len(iter_list)
        ]
        ...
        self.for_recursive(
            f=self.partial_row, ranges=iter_ranges, row_index=count, vals=vals
        )
    ...
    def interpolate(
        ...
        if not terminate:
            if proc == 0:
                ...
                iter_ranges = [range(0, order + 1) for i in range(len(self.alphas))]
                self.for_recursive(f=self.compute_result, ranges=iter_ranges, vals=vals)
                ...
                for i in range(4):
                    slopes.append([])
                    s = (
                        self.axes[phase][tuple(self.index_matrix[2 * i + 1])][3]
                    ...
                    pnt = self.axes[phase][tuple(self.index_matrix[2 * i])][3] + s * (
                        vals[2] - self.axes[phase][tuple(self.index_matrix[2 * i])][2]
                    )
    ...
    def create_data_file(self, file_name=None, meta=True):
        ...
        for ph in range(self.n_phase):
            ...
            iter_ranges = [
                range(0, al)
                for al in [
                    self.axes_lengths[i + 1] for i in range(len(self.axes_lengths) - 2)
                ]
            ]

            self.for_recursive(
                f=self.write_individual_params, ranges=iter_ranges, ph=ph
            )
    ...
    def read_data_file(self, file_name="table.tab", loc="./eos_tables/"):
        ...
        for p in range(self.n_phase):
            ...
            while True:
                ...
                self.for_recursive(
                    f=self.read_individual_params, ranges=iter_ranges, ll=ll, cc=c, ph=p
                )
                self.count = 0
    ...
    def plot_slice(
        ...
        **kwargs,
        ...
        if not len(self.alphas) - len(axes) == len(vals):
            ...
            plot_data = np.zeros(
                [self.axes_lengths[1 + axes[0]], self.axes_lengths[1 + axes[1]]]
            )
        ...
        ax.set_xticklabels([f"${t:.0f}$" for t in xTicks], fontsize=fnts)
        ax.set_yticklabels([f"${t:.0f}$" for t in yTicks * 1e-9], fontsize=fnts)
        ...
        im = ax.imshow(
            plot_data.T * scale, origin="lower", extent=extent, aspect=aspect, cmap=cmap
        )
        ...
        if addCbar:
            axin = ax.inset_axes([1.1, 0.0, 0.05, 1.0])
