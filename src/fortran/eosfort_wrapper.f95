...
MODULE wrapper
   ...
   private
   public :: load_eos_tables, pure_curve, create_planet

contains

!#######################################################################
!Load all EoS table files for subsequent table interpolations.
   SUBROUTINE load_eos_tables(verbose)
...
!  M_surface_should, M_ocean_should, Mg_number_should, E_grav, E_int)
...
! type(planet) :: pl
! real(8), intent(in) :: M_surface_should, M_ocean_should, Mg_number_should
...
! !integer, intent(in) :: core_segregation_model, inner_core_segregation_model
! logical :: csm, icsm

! ! if(core_segregation_model==0)then
! !         csm = .false.
! ! elseif(core_segregation_model==1)then
! !         csm = .true.
! ! endif

! ! if(inner_core_segregation_model==0)then
! !         icsm = .false.
! ! elseif(inner_core_segregation_model==1)then
! !         icsm = .true.
! ! endif
...
! X_all_core=X_all_core, P_CS=P_CS, core_segregation_model = csm, &
! M_surface_should = M_surface_should, M_ocean_should = M_ocean_should, &
! Mg_number_should = Mg_number_should, &
! inner_core_segregation_model = icsm)
