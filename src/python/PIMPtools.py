...
from PIMPphysicalparams import T0_list, K0_list, K0prime_list, rho0_list, aP_list, \
                                molar_mass_list, Rgas, mH, kB, mMg, mFe, mSi, mO, mS, \
                                q_list, G, m_earth, r_earth, n_list, aT_list, bT_list, \
                                cT_list, gamma0_list, thetaD0_list
from PIMPrunparams import dens_min, dens_max, eps_P, bisectionstep_limit, pres_round, \
                            temp_round, dens_round, mass_round, radius_round
#import PIMPeos
import numpy as np
import time
...
def C2(x2, y2):
    return mFe + (1.-y2)*mMg + x2*mSi + (2.*x2 + 1.)*mO

def C3(x3, y3):
    return (x3*y3 + 2.*y3)*mFe + (2. - 2.*y3)*mMg + (2.-x3-4.*x3*y3)*mSi + (6.-2.*x3)*mO

def MFe_mantle(M2, M3, x2, x3, y2, y3):
    return (M2/C2(x2,y2)+(x3*y3+2.*y3)*M3/C3(x3,y3))*mFe

def MFe_core(M1):
    return mFe*M1/(.87*mFe + .13*(mFe+mS))

def MSi_mantle(M2, M3, x2, x3, y2, y3):
    return (M2/C2(x2,y2) + (2.-x3)*M3/C3(x3,y3))*mSi

def FeSi_tot(M1, M2, M3, C2, C3, x3, y3):
    return (MFe_core(M1) + MFe_mantle(M2, M3, x2, x3, y2, y3))/MSi_mantle(M2, M3, x2, x3, y2, y3)*mSi/mFe

def dydx(x, y, derivative_type, whichEOS = None, lay = None, materials = None, 
         density_type = None, Ttype = 'isothermal ambient', matdens = None,
    ...
    if derivative_type == 'structure':
        matdensRK = density(whichEOS, lay, y, matdens, dens_min, dens_max, 
                            eps_P, materials, laymatfrac_list, Ttype = Ttype, identifier = 'Runge Kutta')[1]
        
        if density_type == 'constant':
            dgrad = 0.
        elif density_type == 'integrate':
            dgrad = meandens_grad(x, y, matdensRK, whichEOS, lay, materials, laymatfrac_list)[0]
        
        if temperature_type == 'isothermal':
            Tgrad = 0.
        elif temperature_type == 'isothermal ambient' or temperature_type == 'ambient isothermal':
            Tgrad = 0.
            y[2] = 300.
            ...
            if lay == 3:
                ...
                Tgrad = -gamma_bulk * y[2] / (y[3] * mean_dEOSdrho(y, matdensRK, whichEOS,
                                       lay, materials, laymatfrac_list,
                                       Ttype = temperature_type)) * G * y[1] * y[3] / x**2
            elif lay == 2:
                gamma_bulk = 1.96 * (mean_dens(md, lay, materials, laymatfrac_list)/y[3])**2.5
                Tgrad = -gamma_bulk * y[2] / (y[3] * mean_dEOSdrho(y, matdensRK, whichEOS,
                                       lay, materials, laymatfrac_list, 
                                       Ttype = temperature_type)) * G * y[1] * y[3] / x**2
            elif lay == 1:
                gamma_bulk = 1.26 * (mean_dens(md, lay, materials, laymatfrac_list)/y[3])**2.9
                Tgrad = -gamma_bulk * y[2] / (y[3] * mean_dEOSdrho(y, matdensRK, whichEOS, 
                                       lay, materials, laymatfrac_list,
                                       Ttype = temperature_type)) * G * y[1] * y[3] / x**2
            elif lay == 0:
        ...
        try:
            return [-G * y[1] * y[3] / x**2, 4. * np.pi * x**2 * y[3], Tgrad, dgrad], Tgrad, dgrad
        ...
        return [x**3/(np.exp(x)-1.)], None, None
...
def RungeKutta(x, y, h, derivType, whichEOS = None, lay = None, materials = None, 
               order = 4, density_type = None, Ttype = 'isothermal',
               materialdensities = None, laymatfrac_list = None):
    ...
    if order == 4:
        ...
        alist = [[0., 0., 0., 0.,], [.5, 0., 0., 0.], [0., .5, 0., .0], [.0, .0, 1., 0.]]
        blist = [1./6., 1./3., 1./3., 1./6.]
        clist = [0., 1./2., 1./2., 1.]
    ...
    for i in range(order):
        ...
        newdydx, Tgrad, dgrad = dydx(x + h*clist[i], ki, derivType, whichEOS, 
                                     lay, materials, density_type, Ttype = temperature_type, 
                                     matdens = materialdensities, laymatfrac_list = lmfl)
    ...
    if derivType == 'structure':
        ...
        Pgrad = pres_grad(x, y)
        mgrad = mass_grad(x, y)
        nextmd = density(whichEOS, lay, y, materialdensities, dens_min, dens_max, \
                             eps_P, materials, laymatfrac_list, 'nextmatdens',\
                             Ttype = temperature_type)[1]
...
def rho0T(T, T0, d0, ll, aT, bT, cT):
    ...
    try:
        result = d0/np.exp(aT*T+.5*bT*T**2+2.*cT/T**3-aT*T0-.5*bT*T0**2-2.*cT/T0**3)
...
def thetaD(thetaD0, rho0, gamma, d):
    try:
        return thetaD0 * (d / rho0)**gamma#np.exp((gamma0_list[ll] - gamma(d, ll))/q_list[ll])
    except ZeroDivisionError:
...
def Eth(d, T, thetaD0, rho0, gamma0, q, molmass, nparticles, ll, **kwargs):
    ...
    try:
        return (T/Tdebye)**3 * 9. * n_list[ll] * Rgas * T * yendEth * d /\
            molmass
...
def EOS(d, T, which, ll, temperature_type = 'isothermal ambient', 
        MGD_acc = 1.0e-4, MGD_eps_x = 0.5):
    
    if temperature_type == 'isothermal ambient' or temperature_type == 'ambient isothermal':
        T = T0_list[ll]
    ...
    #Mie-Grüneisen-Debye (MGD)
        ...
        if temperature_type == 'isothermal ambient' or temperature_type == 'ambient isothermal':
            ...
            E2 = Eth(d, 300., acc, eps_x, ll)
            deltaP = (E1 - E2) * gamma(d, ll)
        return 3./2. * K0 * (eta**(7. / 3.) - eta**(5. / 3.)) * (1. + 3. / 4. * (K0prime - 4.) * (eta**(2. / 3.) - 1.)) + deltaP
    
    #Vinet (Vinet)
    elif which == 2:
        eta = d/d0
        return 3.*K0*eta**(2./3.)*(1.-eta**(-1./3.))*np.exp(3./2.*(K0prime-1.)*(1.-eta**(-1./3.)))
    ...
    #Belonoshko (Bel)
        ...
        if temperature_type == 'isothermal ambient' or temperature_type == 'ambient isothermal':
            ...
            deltaP = 3. * Rgas * gamma(d, ll) * (T - T0_list[ll]) * (d/molar_mass_list[ll])
        return 3./2. * K0 * (eta**(7./3.) - eta**(5./3.)) * (1. + 3./4. * (K0prime - 4.) * (eta**(2./3.)-1.)) + deltaP
...
def dEOSdrho(d, T, which, ll, temperature_type = 'isothermal ambient'):
    ...
    if which == 0:
        K = K0 + aP * (T-T0)
        eta = d/d0T
        return 3./2.*K/d0T*((7./3.*eta**(4./3.)-5./3.*eta**(2./3.))*(1.-3./4.*(4.-K0prime)*(eta**(2./3.)-1.)) \
                            + 1./2.*(eta**(7./3.)-eta**(5./3.))*(4.-K0prime)*eta**(-1./3.))
    ...
    #Mie-Grüneisen-Debye (MGD)
        ...
        return 3./2.*K0/d0*((7./3.*eta**(4./3.)-5./3.*eta**(2./3.))*(1.-3./4.*
                            ...
                            eta**(5./3.))*.5*(4.-K0prime)*eta**(-1./3.))

    #Vinet (Vinet)
    elif which == 2:
        eta = d/d0        
        return K0/d0*np.exp(3./2.*(K0prime-1.)*(1.-eta**(-1./3.)))*(2.*eta**(-1./3.)*
                            (1.-eta**(-1./3.)) + eta**(-2./3.)* 3./2. * eta**(-2./3.)*
                            (1.-eta**(-1./3.))*(K0prime-1.))
    ...
    # Belonoshko (Bel)
        ...
        if temperature_type == 'isothermal ambient' or temperature_type == 'ambient isothermal':
            ...
            delta = 3.*Rgas*gamma(d, ll)*(T-T0_list[ll])/molar_mass_list[ll]
        return 3./2.*K0/d0*((7./3.*eta**(4./3.)-5./3.*eta**(2./3.))*(1.-3./4.*
                            (4.-K0prime)*(eta**(2./3.)-1.)) - 1./2.*(eta**(7./3.)-
...
def simplemass(r, r0, d):
    return 4./3.*np.pi*d*(r-r0)**2

def mean_dens(md, lay, ll, laymatfrac_list):
    return 1./sum([laymatfrac_list[lay][i]/md[i] for i in range(len(ll))])
        
def dens_grad(r, y, md, EOS, ll, Ttype = 'isothermal ambient'):
    return  -G * y[1] * y[3] / (r**2 * PIMPeos.dEOSdrho(md, y[2], EOS, ll, Ttype))

def meandens_grad(r, y, materialdensities, EOS, lay, ll, laymatfrac_list, Ttype = 'isothermal ambient'):
    ...
    for l in range(len(ll)):
        try:
            dgrad_list.append(dens_grad(r, y, materialdensities[l], EOS[ll[l]], ll[l], Ttype))
        except IndexError:
            print ('WARNING: IndexError in function meandens_grad()')
            if len(dgrad_list) != len(laymatfrac_list[lay]):
                print ('REASON: density gradient and layer fractions have not the same dimensions')
            print ('parameters that lead to falure are:\nlayer =', lay,', l =', l,', ll =', ll)
            print ('layer fractions =', laymatfrac_list[lay], ', dgrad_list =', dgrad_list)
            sys.exit()

    return 1./sum([laymatfrac_list[lay][i] / dgrad_list[i] for i in range(len(ll))]), dgrad_list

def mean_dEOSdrho(y, matdens, whichEOS, lay, ll, laymatfrac_list, Ttype = 'isothermal ambient'):        
    return (sum(laymatfrac_list[lay][i] * dEOSdrho(matdens[i], y[2], whichEOS[ll[i]], ll[i], temperature_type = Ttype) for i in range(len(ll))))

def pres_grad(r, y):
    return - G*y[1]*y[3]/r**2

def mass_grad(r, y):
    return 4.*np.pi*r**2*y[3]

def temp_grad(r, y):
    return None


def bisec(which, y, a, b, eps, ll, identifier, Ttype = 'isothermal ambient', materialdensity = None,
          firstlimit = 10, secondlimit = 20, thirdlimit = 45, forthlimit = 200,
          firstepsmin = .995, firstepsmax = 1.005, secondepsmin = .9, secondepsmax = 1.1,
          thirdepsmin = .5, thirdepsmax = 1.5, densmin = 5.0e2, densmax = 2.0e4):
        ...
        if EOSc == P:
            ...
            while abs(EOSc - P)/P > eps:
                ...
                if counter > limit and not exceeded:
                    ...
                    a = densmin
                    b = densmax
                    print ('NOTE: setting maximal density range for bisection to [', a, b,']')
                elif counter > limit and tripleexceeded:
                    print ('WARNING: bisection step limit exceeded in <', identifier,'> !')
                    print ('-> parameters that lead to failure are:')
                    print ('counter =', counter, '\nlimit =', limit)
                    print ('a =',a, '\nb =',b, '\nc =',c,'\na0 =',a0, '\nb0 =',b0, '\nc0 =',c0, '\ndens =', d, 
                           '\npres = ', P * 1.0e-9, 'GPa', '\ntemp =', T, '[K] \nTtype =', Ttype, '\nmatdens =', materialdensity, '\nM/Mearth =',y[1]/m_earth)
                    print ('EOS(c) =', EOSc, '\nEOS type =', which, '\nmaterial =', ll)
                    eps = 10.
        return c


def density(whichEOS, lay, param_list, matdens_list, a, b, eps, ll, laymatfrac_list, identifier, 
            Ttype = 'isothermal ambient', layvic = False):
    ...
    for l in range(len(ll)):
        ...
        if layvic:
            ...
            try:
                ...
                a = dens_min
                b = dens_max
        matdens = bisec(whichEOS[ll[l]], param_list, a, b, eps, ll[l], identifier, Ttype = Ttype, materialdensity = matdens)
        matdens_dummy_list.append(matdens)
    try:
        return 1./sum([laymatfrac_list[lay][i]/matdens_dummy_list[i] for i in range(len(ll))]), matdens_dummy_list
    except TypeError:
        print ('laymatfrac =', laymatfrac_list[lay], 'matdens dummy =', matdens_dummy_list)
        print ('M/Mearth =', param_list[1]/m_earth, 'pres =', param_list[0], 'temp =', param_list[2], 'dens =', param_list[3])
        sys.exit()
        
def layer_transition_notification(x, y, Mtot, newlayer):
    print ('-----------------------------------------')
    print ('-> transition to layer', newlayer, 'at: ', round(x * 1.0e-3, radius_round),
           'km', ',', round(y[0] * 1.0e-9, pres_round), 'GPa',', M/Mtot = ',
           round(y[1]/Mtot,mass_round), 'T =', round(y[2], temp_round),'K')