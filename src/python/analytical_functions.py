...
def dP_MGD(eta = 0, ll = 5, T = 300, **kwargs):
    ...
    if tempType == 'isothermal':
        ...
        E2 = PIMPtools.Eth(d, 300., ll, **kwargs)
        deltaP = (E1 - E2) * PIMPtools.gamma(d, ll)
        
    return deltaP
    
def P_MGD(eta = 0., K0 = 0., K0prime = 0., ll = 5, T = 300, **kwargs):
    d = eta*rho0_list[ll]
    tempType = ftool.checkKey(name='tempType', **kwargs)
...
def P_Bel(eta = 0, K0 = 0, K0prime = 0, ll = 0, T = 300, **kwargs):
    ...
    if tempType == 'isothermal':
        ...
        deltaP = 3. * Rgas * PIMPtools.gamma(d, ll) * (T - T0_list[ll]) * \
...
def dPdrho_Bel(eta = 0, K0 = 0, K0prime = 0, ll = 0, T = 300, **kwargs):
    ...
    if tempType == 'isothermal':
        ...
        delta = 3.*Rgas*PIMPtools.gamma(d, ll)*(T-T0_list[ll])/molar_mass_list[ll]
    return 3./2.*K0/d0*((7./3.*eta**(4./3.)-5./3.*eta**(2./3.))*(1.-3./4.*
                        (4.-K0prime)*(eta**(2./3.)-1.)) - 1./2.*(eta**(7./3.)-
