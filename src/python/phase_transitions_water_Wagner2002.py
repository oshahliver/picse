...
r2_list = [np.complex(-72.597457432922, -78.100842711287), 
           np.complex(-5.57107698030123E-05, 4.64578634580806E-05), 
           np.complex(2.34801409215913E-11, -2.85651142904972E-11)]
t_list = [t1, t2]

#real constants
g0_list = [-632020.233449497, 0.655022213658955, -1.89369929326131E-08,
...
aevap_list = [-7.85951783, 1.84408259, -11.7866497, 22.6807411, 
              -15.9618719, 1.80122502]

#convention: Ih/l/v, Ih/III/l, III/V/l, V/VI/l, VI/VII/l, Ih/III/II, III/V/II,
#VI/V/II, VII/VI/VIII, Ih/II/IX
#in K
t_triple_list = [T_triple, 251.165, 256.164, 273.31, 355., 238.46, 248.86,
                 218.16, 268.16, 73.4, 130.]
#in Pa
p_triple_list = [P_triple, 208.566e6, 350.1e6, 632.4e6, 2216.0e6, 212.9e6,
                 344.3e6, 620.0e6, 2100.0e6, 89.6e6, 828.5e6]

#high pressure transition Schwager 2008
#[P(GPa), T(K)]
data_schwager=[
[1.59004e+1, 7.50567e+2],
[1.79379e+1, 9.25928e+2],
[1.91294e+1, 1.01505e+3],
[2.11794e+1, 1.13581e+3],
[2.37529e+1, 1.23935e+3],
[2.68479e+1, 1.33429e+3],
[2.97783e+1, 1.39187e+3],
[3.30557e+1, 1.44659e+3],
[3.65110e+1, 1.48119e+3],
[3.92795e+1, 1.48991e+3],
[4.11624e+1, 1.58481e+3],
[4.38178e+1, 1.70847e+3],
[4.73337e+1, 1.85514e+3],
[5.05093e+1, 1.97595e+3],
[5.47225e+1, 2.10254e+3],
[5.96303e+1, 2.22053e+3],
[6.32514e+1, 2.28675e+3],
[6.79986e+1, 2.35014e+3],
[7.25787e+1, 2.38766e+3],
[7.76829e+1, 2.40509e+3],
[8.28744e+1, 2.41964e+3],
[8.74617e+1, 2.42555e+3],
[8.96262e+1, 2.42563e+3]]
...
#Table 2
LtoX_coeffs_Dunaeva2010 = [.2524, .0019, .2795, .5, 1.1675]

#From fit to extracted data from figure 2
LtoVII_coeffs_Dunaeva2010 = [-2.29294290e-01,  4.11453047e+01,  5.46863218e+02]

#Fit solidus T(P), T output in K P input in GPa
LtoVII_coeffs_Sun = [ 1.14666190e-04, 
                     -7.94260914e-02,  
                     1.83673796e+01,  
                     5.52107003e+02]
...
coeffs_French2016 = [-2.26536372e-07,  2.64778698e-04, -1.19895845e-01,  2.37743503e+01,
        3.96728823e+02]
...
def bisec(a, b, y, f, *args, eps = 1.0e-8, plot = False, axis = None):
    ...
    if fc == y:
        ...
        while abs(fc - y)/y > eps:
            if plot:
                axis.plot(fc, c, linestyle = 'None', marker = 'x', color = 'k', 
                          zorder = 100, markersize = 6)
            counter += 1
            c = (a + b)/2.
            fc = f(c, *args)
            fb = f(b, *args)
        ...
        if plot:       
            axis.scatter(fc, c, marker ='o', color = 'r', s = 100, 
                         zorder = 1000, facecolor = 'none')
    return c

def iceI(t):
    #pressure in MPa
    theta = t/273.16
    return (1. + sum([amelt_list[i] * (1. - theta**bmelt_list[i]) for i in 
                      range(3)])) * 611.657e-6

def iceIII(t):
    #pressure in MPa
    theta = t/251.165
    return (1. - 0.299948 * (1. - theta**60)) * 208.566

def iceV(t):
    #pressure in MPa
    theta = t/256.164
    return (1. - 1.18721 * (1. - theta**8)) * 350.1

def iceVI(t):
    #pressure in MPa
    theta = t/273.31
    return (1. - 1.07476 * (1. - theta**4.6)) * 632.4
...
def sublPres(t):
    ...
    return np.exp(1./theta * sum([asubl_list[i] * theta**bsubl_list[i] 
                                  for i in range(3)])) * 611.657e-6
...
def evapPres(t):
    ...
    """
    theta = (1.-t/T_critical)
    a1, a2, a3, a4, a5, a6 = aevap_list
    res = P_critical * np.exp(T_critical/t*(a1*theta+a2*theta**1.5+\
                                             a3*theta**3+a4*theta**3.5+\
                                             a5*theta**4+a6*theta**7.5))
    return res
...
def solidusPres(T=None, order=2, acc = 1e-4):
    P = ftool.bisec(f = solidusTemp, whicharg = 'P', y = T, identity='solidusPres',
                a = 1e3, b = 1e11, noisy = False, order = 2, acc = acc)
    return P
    '''
    if T >= 750.:
...
def interpolateWagnerSchwager(N_Schwager=3, N_Wagner=3, order=3,
                              whicharg = 'T'):
...
def plot_phases(N = 16):
    ...
    phase_strings = ['Ih', 'Mazevet', 'French', 'V', 'VI', 'II', 'III', 'liquid', 'vapor', 'gas']
    fig, ax = plt.subplots()
...
def plot_the_shit(xlims = [100., 1000.], ylims = [1e5, 1e12], 
                  minor = [True, True], axis_scales = [1., 1.],
                  showOnlyStates = True):
    ...
    ax.tick_params(which='major', labelsize=fnts2, direction = 'out',
                   size=8, top=True, right=True, pad = 10)
    ax.tick_params(which='minor', labelsize=fnts2, direction = 'out',
                   size=4, top=True, right=True)
    ...
    if not showOnlyStates:
        ax.text(.05, .1, s = 'solid', fontdict = font1, transform = trans)
    ax.text(.6, .9, s = 'solid', fontdict = font1, transform = trans)
    ax.text(.3, .5, s = 'liquid', fontdict = font1, transform = trans)
    ax.text(.6, .1, s = 'vapor', fontdict = font1, transform = trans)
    #ax.text(.7, .1, s = 'gas', fontdict = font1, transform = trans)
    ax.text(.7, .5, s = 'supercritical', fontdict = font1, 
            transform = trans)
    ax.text(T_critical * 1.01, P_critical * 1.1, r'Critical  point',
            fontdict = font1)
    '''
    ...
    '''
    inter = interpolateWagnerSchwager(order=1, whicharg='T',
                                      N_Schwager=1,
                                      N_Wagner=1)
    ...
    ax.scatter(t_triple_list[0], p_triple_list[0], color = phase_color, 
               zorder = 2, s = 20)
    ...
    phase_colors = [(.7, .7, .9), 
                    ...
                    (.75, .75, .75)]
    #Fill gas region
    ax.fill_between([T_critical, xlims[1]], [P_critical, P_critical],#, [1.0, P_critical],
                    color = phase_colors[4], 
                    zorder = 1)
    #Fill scf region
    ax.fill_between(temp_fill_list_scf, 
    ...
    #Fill liquid region
    ax.fill_between(temp_fill_list_liq, 
                    pres_fill_list_liq,#, [1.0, P_critical],
                    color = phase_colors[1], 
                    zorder = 0)
    #Fill vapor region
    ax.fill_between(x, vapor,#, [1.0, P_critical],
    ...
    ax.set_ylim(ylims)
    ax.set_yscale('log')
    ax.set_xlabel(r'$\rm Temperature \ [K]$', fontsize=fnts2, labelpad=20.)
    ax.set_ylabel(r'$\rm Pressure \ [bar]$', fontsize=fnts2, labelpad=20.)
    xticks = matplotlib.ticker.FuncFormatter(lambda x, pos: r'${0:g}$'.format(x/axis_scales[0]))
    ax.xaxis.set_major_formatter(xticks)
    yticks = matplotlib.ticker.FuncFormatter(lambda y, pos: r'$10^{{{0:d}}}$'.format(int(np.log10(y/axis_scales[1]))))
    ax.yaxis.set_major_formatter(yticks)
    
    plt.savefig('/home/os18o068/Documents/PHD/Abbildungen/Figures_paper_2/water_phase_diagram.pdf', 
    format = 'pdf', bbox_inches='tight', dpi = 320)
    plt.close(fig)
    #return fig, ax
    #plt.close('all')
