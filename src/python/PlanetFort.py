...
from PIMPphysicalparams import T0_list, K0_list, K0prime_list, rho0_list, \
                                ...
                                material_YS, material_YFe, mS, sigmaSB
...
def ComputeCoreMass(contents = None, Mg_number=None, M_surface = 1.,
    ...
    fractions = fortfunctions.functionspy.compute_abundance_vector(simg=SiMg, 
                                                     ...
                                                     xih2oi=[0. for i in contents[n]],
                                                     xifei=[1.-Mg_number_mantle for i in contents[n]],
                                                     xialsii = [0. for i in contents[n]],
                                                     xialmgi = [0. for i in contents[n]],
                                                     contents = contents[n],
                                                     additional = impurity)
    #Count 
    #Note that the indices are shifted by one because the original definition
    #of the arrays comes from the fortran code.
    Q1 = sum([fractions[i]*Mg_number_mantle*material_YMg[contents[n][i]-1] 
              for i in range(len(contents[n]))])
    
    #Compute total normalized mass in the mantle
    Q2 = sum([fractions[i]*Mg_number_mantle*material_YMg[contents[n][i]-1] 
              for i in range(len(contents[n]))])*mMg+\
            sum([fractions[i]*(1.-Mg_number_mantle)*material_YMg[contents[n][i]-1] 
                 for i in range(len(contents[n]))])*mFe +\
            sum([fractions[i]*material_YSi[contents[n][i]-1] 
                 for i in range(len(contents[n]))])*mSi+\
            sum([fractions[i]*material_YO[contents[n][i]-1] 
                 for i in range(len(contents[n]))])*mO+\
            sum([fractions[i]*material_YH[contents[n][i]-1] 
                 for i in range(len(contents[n]))])*mH
    ...
    core_frac += M_IC / M_surface * (1. / mFe - Q4 / Q5)
...
class Planet():
    def __init__(self, P_center=1.0e12, 
        ...
        self.E_int = 0e0
        self.E_tot_is = 0e0
        self.E_tot_should = E_tot_should * G * m_earth**2 / r_earth * 3./5.
        self.L_int_should = L_int_should * 4. * np.pi * r_earth**2 * sigmaSB * 300**4
        self.L_int_is = 0e0
        self.L_eff_is = 0e0
        self.L_eff_should = L_eff_should * 4. * np.pi * r_earth**2 * sigmaSB * 300**4
        ...
        try:
            ...
            print ('R_surface_is [R_earth]:', round(self.R_surface_is/r_earth, 
               ...
               '\nM_surface_is [M_earth]:', round(self.M_surface_is/m_earth, 
                               digits),
               '\nrho mean [gcc]:', round(self.rho_mean/1000,digits),
               '\nT_surface_is [K]:', round(self.T_surface_is, digits),
               '\nT_surface_should [K]:', round(self.T_surface_should, digits),
               ...
               '\n\nMg_number_should:', round(self.Mg_number_should, digits),
               ...
               '\nL_int_should [W]:', round(self.L_int_should/(4*np.pi*r_earth**2* sigmaSB*300**4), digits),
               '\nL_int_is [W]:', round(self.L_int_is/(4*np.pi*r_earth**2* sigmaSB*300**4), digits),
               '\nE_tot_should [G M_E²/R_E]:', round(self.E_tot_should/(G * m_earth**2 / r_earth * 3/5), digits),
               '\nE_tot_is [G M_E²/R_E]:', round(self.E_tot_is/(G * m_earth**2 / r_earth * 3/5), digits)
               )
            ...
            try:
                ...
                print ('Core mass frac:', round(self.M_core_is/self.M_surface_is*m_earth,digits))
                print ('Core radius [km]:', round(self.layer_properties[1]['R_outer']/1000, digits))
    ...
    def complete_layer_properties(self):
        for i in range(len(self.layer_properties)):
            ...
            self.layer_properties[i].update({"mass_fraction":self.layer_properties[i]['indigenous_mass'] / self.M_surface_is})


            if i > 0:
                self.layer_properties[i]['P_inner'] = self.layer_properties[i - 1]['P_outer']
                self.layer_properties[i]['T_inner'] = None#self.layer_properties[i - 1]['T_outer']
                self.layer_properties[i]['R_inner'] = self.layer_properties[i - 1]['R_outer']
        ...
        for i in range(len(self.cleaned_layer_properties)):
            ...
            self.cleaned_layer_properties[i].update({"mass_fraction":self.cleaned_layer_properties[i]['indigenous_mass'] / self.M_surface_is})            
            
            if i > 0:
                self.cleaned_layer_properties[i]['P_inner'] = self.cleaned_layer_properties[i - 1]['P_outer']
                self.cleaned_layer_properties[i]['T_inner'] = None#self.layer_properties[i - 1]['T_outer']
                self.cleaned_layer_properties[i]['R_inner'] = self.cleaned_layer_properties[i - 1]['R_outer']
    ...
    def extract_hydro_structure(self, trim = False):
        ...
        self.dms = [self.M_surface_is*(1.-10**self.ocean_frac_is)]
        ...
        hdiffs = [(self.locs[i+1] - self.locs[i]) 
                 for i in range(len(self.locs)-1)]
        mdiffs = [(self.dms[i+1] - self.dms[i]) 
                 for i in range(len(self.dms)-1)]
        voldiffs = [(self.dvols[i+1] - self.dvols[i])
                    for i in range(len(self.dvols)-1)]
    ...
    def update_oxide_fractions(self):
        ...
        if self.xi_Fe_mantle > 0.:
            self.xi_FeO_mantle = (1. - self.Si_number_mantle)
            self.xi_FeO_mantle /= ((1. - self.xi_Fe_mantle) / self.xi_Fe_mantle + \
                                   1. - self.Si_number_mantle)
                
        else:
            self.xi_FeO_mantle = 0.
            
        self.xi_SiO2_mantle = (1. - self.xi_FeO_mantle) * self.Si_number_mantle
        self.xi_MgO_mantle = 1. - self.xi_FeO_mantle - self.xi_SiO2_mantle
        
        #Update oxygen fugacity
        self.logfO2 = Material.logfO2(self.P_CS,
                                      self.xi_all_core[0],
                                      self.xi_FeO_mantle)
    ...
    def compute_luminosity(self, S0, eps = 0., alpha = 0.):
        self.L_eff_is = sigmaSB * self.T_surface_is**4 * (1. - eps / 2.)
        self.L_eff_is -= .25 * S0 * (1. - alpha)
        self.L_eff_is *=  4 * np.pi * self.R_surface_is**2
        self.L_int_is = sigmaSB * self.T_surface_is**4 * 4. * np.pi * self.R_surface_is**2
    ...
    def Update(self):
        ...
        if self.layer_properties[1]['R_outer']-\
            self.layer_properties[0]['R_outer'] < 1e3:
            self.outer_core_exists = False
    ...
    def Construct(self, print_time=False, fortran=True, echo=False,  **kwargs):
        ...
        for i in range(len(layer_props_dummy)):
            ...
            self.layer_properties[i]['mass_fraction'] = layer_props_dummy[i][4] / self.M_surface_is
        ...
        self.M_core_is = (self.layer_properties[0]['indigenous_mass'] + \
                            self.layer_properties[1]['indigenous_mass'])/m_earth
        self.M_mantle_is = (self.layer_properties[2]['indigenous_mass'] + \
                            self.layer_properties[3]['indigenous_mass'])/m_earth
        
        
            
        self.M_H2O_core = self.H_count/2.*mH2O/self.M_surface_is
        self.M_H2O_is = (self.H2O_count + self.H_count / 2.) * mH2O / m_earth
        ...
        self.L_int_is = 4 * np.pi * sigmaSB * self.T_surface_is**4 * self.R_surface_is**2
        ...
        '''
        try:
            self.ocean_depth = (self.layer_properties[4]['R_outer'] - \
                            self.layer_properties[3]['R_outer'])/r_earth
        except IndexError:
            self.ocean_depth = 0.
        #Update mean density of the planet
        self.rho_mean = self.M_surface_is/(4./3.*np.pi*self.R_surface_is**3)
    ...
    def generate_simple_model(self, res=0, equi='mass'):
        ...
        for i in range(N):
            ...
            rho = 3./4.*M/np.pi/(R_out**3 - R_in**3)
        ...
        for i in range(N):
            ...
            dMoI = 8./15.*np.pi*self.simple_densities[i]*(R_out**5 - R_in**5)
            print ('dMoI =', dMoI)
    ...
    def plot(self, scatter=False, layerColoring = False, axis=[], x_axis='radius',
             save = True, file_name='planet_structure', spec='', file_dir='./', 
             prem=False, **kwargs):
        ...
        axis[0][0].plot(self.profiles[0]*1e-3, self.profiles[1],
                        linewidth=lwdth, color=color)
        axis[0][1].plot(self.profiles[0]*1e-3, self.profiles[2]*1.0e-9,
                        linewidth=lwdth, color=color)
        axis[0][2].plot(self.profiles[0]*1e-3, self.profiles[3]*1.0e-3,
                        linewidth=lwdth, color=color)
        axis[1][0].plot(self.profiles[0]*1e-3, self.profiles[4]/m_earth, 
                        linewidth=lwdth, color=color)
        axis[1][1].plot(self.profiles[0]*1e-3, self.profiles[5], 
                        linewidth=lwdth, color=color)
        axis[1][2].plot(self.profiles[0]*1e-3, self.profiles[7]/\
                        (G * m_earth**2) * r_earth, 
                        linewidth=lwdth, color=color)
        ...
        if prem:
            ...
            axis[0][2].text(.6, 9., 'PREM', color='b')
        '''  
        ...
        axis[ax][0].text(.05, pos0-4*d, 
                      ...
                      transform = axis[0][0].transAxes) 
        axis[ax][1].text(1.55, pos0, 
                        r'$M_{\rm H_2 O}/M = \ $'+str(round(self.M_H2O_is*100,3))+r'$\ \rm wt\%$',
                      transform = axis[0][0].transAxes) 
        axis[ax][1].text(1.55, pos0-d, 
                        r'$M_{\rm H_2 O, Core}/M = \ $'+str(M_H2O_core)+r'$\ \rm wt\%$',
                      transform = axis[0][0].transAxes) 
        axis[ax][1].text(1.55, pos0-2*d, 
                        r'$M_{\rm H_2 O, Mantle}/M = \ $'+str(M_H2O_mantle)+r'$\ \rm wt\%$',
                      transform = axis[0][0].transAxes) 
        axis[ax][1].text(1.55, pos0-3*d, 
                        r'$X_{\rm Ice}= \ $'+str(0.0)+r'$\ \rm wt\%$',
                      transform = axis[0][0].transAxes)         
        axis[ax][1].text(1.55, pos0-4*d, 
                        r'$X^{\prime}_{\rm Ice}= \ $'+str(0.0)+r'$\ {\rm wt\%} M_\oplus^{-1}$',
                      transform = axis[0][0].transAxes) 

        axis[ax][2].text(3.05, pos0-0*d, 
                        r'$M/M_\oplus= \ $'+str(round(self.M_surface_is/m_earth,4)),
                      transform = axis[0][0].transAxes)

        axis[ax][2].text(3.05, pos0-1*d, 
                        r'$R/R_\oplus= \ $'+str(round(self.R_surface_is/r_earth,4)),
                      transform = axis[0][0].transAxes)
        axis[ax][2].text(3.05, pos0-2*d, 
        ...
        fig.savefig(file_dir+file_name+'_'+spec,
                bbox_inches='tight',
                format = 'pdf')
    ...
    def check_convergence(self, accs = {"M_surface":1.0e-4,
                                        "Mg_number":1.0e-3, 
                                        "T_surface":1.0e-2, 
                                        "ocean_frac":1.0e-2, 
                                        "x_H_core":1.0e-2},
                          check_params = ["M_surface",
                                          "Mg_number",
                                          "T_surface",
                                          "ocean_frac"]):
