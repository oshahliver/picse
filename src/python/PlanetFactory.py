...
class Toolkit():
    ...
    def GenerateTrainingSet(self, loc = 'test_set_00', Ns = 2,
                            lims = np.array([[1.0e-3, .1], #core mass
                                             [500, 5000], #T center 
                                             [1.0e9, 2.0e11],  #P center
                                             [1.0e-2, .2] #outer mantle mass
                                             ]),
                    ...
                    fractions = [[1.], [.87, .13], [1., 1., 1.], [1., 1., 1.], [1.]],
                    layerradii = [.1915, .0, .0, .0, 0.],
                    X_H2O = [[1.], [1., 1.], [1., 1., 1.], [1., 1., 1.], [0.]],
        ...
        for i in range(len(core_masses)):
            for j in range(len(mantle_masses)):
                ...
                pl=PlanetTest.Planet(contents = contents, 
                                 fractions = fractions, 
                ...
                input_grid[i][j] = pl.layers[1].mass/pl.M_surface_is, mm_is/pl.M_surface_is
    ...
    def ExtractTrainingData(self, planets=None):
        ...
        for i in range(N):
            pl = planets[i]
            params_out = pl.finals['T_surface_is'],\
            ...
            for j in range(len(data[0][0])):
                for k in range(2):
                    for l in range(2):
                        ...
                        ax[k][l].scatter(data[0][i][j][k], data[1][i][j][l],
    ...
    def PlotTrainingData(self, data=None):
        ...
        for permutation in range(4):
            ...
            x_labels = [r'$T_{center} \ \rm [K]$',
                        r'$P_{center} \ \rm [GPa]$',
                        r'$M_{core}/M_{tot}$',
                        r'$M_{OM}/M_{tot}$']
            
            y_labels = [r'$T_{surf} \ \rm [K]$',
                        r'$M_{tot}/M_{\oplus}$',
                        r'$\rm Mg \#$',
                        r'$M_{ocean}/M_{tot}$']
            ...
            for i in range(len(data)):
                for j in range(2):
                    for k in range(2):
                        ...
                        ax[j][k].scatter(data[i][0][x_indices[j]],
    ...
    def bisect(self, val_is, val_should, acc, direction, predictor=True, 
               log=False, newval=None):
    ...
    def iterate(self, planet=None, what='M_surface', how='P_center',
        ...
        if not predictor == 'linear' and not predictor == 'none':
            print ('''WARNING: invalid value given for keyword argument
                   <predictor>''')
        ...
        elif what == 'T_surface' or what == 'P_surface' or \
        what == 'M_surface' or what == 'M_ocean' or what == 'ocean_frac':
        ...
        while self.iteration: 
            ...
            if update_val_should:
                if updateType == 0:
                    ...
                    try:
                        val_should = (planet.layermasses[0] + \
                        planet.layermasses[1] + \
                        planet.M_H2O_should)*m_earth
            ...
            self.bisect(val_is, val_should, acc, direction, 
                        predictor = predictor, log=log, newval=newval)
            ...
            if self.iteration:
                ...
                if predictor == 'linear' and not self.bisection:
                    ...
                    #compute predictor slope for passive parameter
                    self.passive_slope = [(p2[i]-p1[i])/(x2-x1) for i in range(len(passives))]
                ...
                if log:
                    newval = max(10**(np.log10(self.oldhowvals[-1]) + self.delta), 
                             sanity_borders[how][0])
                                        
                else:
                    newval = max(self.oldhowvals[-1] + self.delta, 
                             sanity_borders[how][0])
                
                self.passive_delta = [(newval - self.oldhowvals[-1])*self.passive_slope[i]
                                      for i in range(len(passives))]
                
                #If slope of passive parameter is not zero update it
                if not self.passive_slope == 0.:
                ...
                if how == 'M_core':
                    planet.initials['layermasses'][0] = newval*planet.initials['inner_core_frac']
                    planet.initials['layermasses'][1] = newval - planet.initials['layermasses'][0]
            ...
            if terminationCount > 2:
                if termination:
                    if predictor == 'linear':
                        print ('Limit reached three times in a row. Switching to manual bisection...')
                        predictor = 'none'
                        terminationCount = 0
                    else:
                        print ('Limit reached three times in a row. Terminating iteration...')
                        self.iteration = False
            ...
            if self.oldhowvals[-1] == newval:
                newval = (self.oldhowvals[-2] + newval)/2.
    ...
    def model_hydro(self, 
                    ...
                    outer_core_mat_fracs= [1., .11, .17, .21],
                    inner_core_mat_fracs = [1., 0., 0.],
                    P_CS = 50e9,
                    initial_predictor = 0,
                    deltas = None,
        ...
        outer_core_mat_fracs[0] = 1. - (sum(outer_core_mat_fracs) - outer_core_mat_fracs[0])
        inner_core_mat_fracs[0] = 1. - (sum(inner_core_mat_fracs) - inner_core_mat_fracs[0])
        ...
        if ocean:
            ...
            fractions = [inner_core_mat_fracs, 
                         outer_core_mat_fracs,#outer_core_mat_fracs, 
                         [1., 1.], 
                         [1., 1.], 
                         [1.]]

            Si_number_layers = [.0, .0, Si_number_mantle, Si_number_mantle, 0.]
            ...
            Si_number_layers = [.0, .0, Si_number_mantle, Si_number_mantle]
            #Fe number layers will be updated during run time
            #Fe number is defined as Fe / (Fe + Mg) so it is always 1 in the core
            Fe_number_layers = [1., 1., Fe_number_mantle, Fe_number_mantle]
            layermasses = [0., 0., 100., 100.]
            M_ocean_should = 0.
            ocean_frac_should = -10
        ...
        tc_pred, pc_pred, mc_pred = self.predict(M_surface_should,
                                                 Mg_number_should,
        ...
        SiMg = Si_number_mantle/(1. - Si_number_mantle)
        ...
        if initial_predictor == 0:
            ...
            xi_all_core = Material.mat2at_core(xi = outer_core_mat_fracs, xiH = 0.)
            ...
            #in the entire core.
            layermasses[0] = PlanetFort.ComputeCoreMass(Mg_number = Mg_number_should,
            ...
            layermasses[1] = PlanetFort.ComputeCoreMass(Mg_number = Mg_number_should,
                                                M_surface = M_surface_should,
                                                Mg_number_mantle = 1.-Fe_number_mantle,
                                                M_ocean = M_ocean_should,
                                                SiMg = SiMg,
        ...
        X_impurity_slope = 2. * (X_impurity - X_impurity_0_layers[3])
        ...
        if iterationType == 0:
            ...
            vals_should = [Mg_number_should, M_surface_should*m_earth, T_surface_should]
            ...
            hows = ['P_center', 'T_center']
            vals_should = [M_surface_should*m_earth, E_tot_should * (G * m_earth**2 / r_earth * 3/5)]
            ...
            hows = ['P_center', 'T_center']
            vals_should = [M_surface_should*m_earth, L_int_should * 4. * np.pi * r_earth**2 * sigmaSB * 300**4]
...
#            if T_surface_match and Mg_number_match and M_ocean_match:
        ...
        M_core = PlanetFort.ComputeCoreMass(Mg_number = Mg_number_should,
                                            ...
                                            xi_S_core = xi_FeS)
        '''                   
        test = PlanetFort.ComputeCoreMass(Mg_number = Mg_number_should,
                                            M_surface = M_surface_should,
                                            Mg_number_mantle = 1.-Fe_number_mantle,
                                            M_ocean = M_ocean_should,
                                            SiMg = SiMg,
    ...
    def probe_parameter_space(self, res = 3, obj='earth', hyd=False):
        ...
        SiMg = np.linspace(.5, 1., N)
        Si_numbers = SiMg/(1.+SiMg)
        Mg_numbers = np.linspace(.4, .7, N)
        
        xi_Fe = np.linspace(0., .25, 3)
        xi_S = np.linspace(0., .9999, 3)
            ...
            for m in range(len(xi_S)):
                ...
                for i in range(N):
                    ...
                    for j in range(N):
                        ...
                        pl= self.model_hydro(P_center=9.5e10, 
                            match_mass=True, 
                            eps_r=0.25, 
                            predictor_T='none', 
                            ocean=False, 
                            ocean_frac_should=-10., 
                            temp_jumps=[0., 1400.*M_solar[obj]**.75, 
                                        0., 0., 0.], 
                            Si_number_should=si, 
                            P_surface_should=1.0e5, 
                            T_surface_should=1500., 
                            T_center=4000.,  
                            Fe_number_mantle=fe, 
                            Mg_number_should=mg, 
                            eps_H2O=eps_H2O, 
    ...
    def plot_parameter_space(self, obj='earth', hyd=False):
            ...
            for m in range(len(planets[l])):
                for i in range(len(planets[m][l])):
                    for j in range(len(planets[m][l][i])):
                        ...
                        if pl.converged:
                            data[l][m][i][j][0] = (pl.R_surface_is/r_earth-R_solar[obj])/R_solar[obj]
        ...
        vmins, vmaxs = ftool.extract_value_range(data = data, 
                                                 scalings = scalings)
        ...
        for i in range(len(data.T)):
            ...
            for l in range(len(xi_Fe)):
                ...
                axes[l][0].text(-.75, .5, r'$\xi_{\rm Fe} = \ $'+Fe_str, 
                           ha='center', 
                           va ='center',
                           fontsize=fnts3,
                           color = 'grey',
                ...
                for m in range(len(xi_S)):
                    ...
                    if l == 0:
                        ...
                        axes[l][m].text(.5, 1.25, r'$\xi_{\rm S} = \ $'+S_str, 
                               ha='center', 
                               va ='center',
                               fontsize=fnts3,
                               color = 'grey',
                    ...
                    xtick_labels = np.linspace(.4, .7, 4)
                    ytick_labels = np.linspace(.5, 1., 6)
                    ...
                    im = ax.imshow(data.T[i].T[l][m]*scalings[i],
                              origin = 'lower',
                              cmap = newcmap,
                              vmin = vmins[i],
                              vmax = vmaxs[i],
            ...
            axes[0][-1].text(1., 1.5, r'$\rm {}$'.format(obj.capitalize()), 
                           ha='center', 
                           bbox=bbox_props, 
                           fontsize=fnts4,
            ...
            fig.savefig(data_dir+obj.capitalize()+'/'+obj+suff+\
                        figure_labels[i]+'.pdf',
                        format='pdf',
                        bbox_inches='tight') 
...
#                            planet.finals['M_surface_is']
    ...
    def create_planet_grid(self, res=3, write=False):
        ...
        Mg = np.linspace(.5, .5, 2**res)
        temps = np.linspace(300., 1500., 2**res)
        
        for i in range(2**res):
            for j in range(2**res):
                for k in range(2**res):
                    of=-8;im=0;m=masses[i];o=False;it=1;si=.5;eps_r=.25;FeS=.3;Pc=9.5e10
                    T0 = 1400.
                    sim = .5
                    pcs = 4e10
                    icmf = [1.]
                    ocmf = [1., 0., 0., 0.]
                    pl=self.model_hydro(P_center=Pc, 
                                            match_mass=True, 
                                            eps_r=eps_r, 
                                            predictor_T='none', 
                                            ocean=o, 
                                            ocean_frac_should=of, 
                                            temp_jumps=[0., T0*m**.75, 0., 0., 0.], 
    ...
    def plot_grid(self):
        ...
        data = {'M':data[0], 'expMg':np.exp(data[1]), 'Mg':data[1], 'Pc':data[3], 
                'Mc':data[4], 'MM':data[0]*data[0], 'MMg':data[0]*data[1], 
                'MgMg':data[1]**2, 'Tc':data[5], 'Ts':data[2],
                'TsTs':data[2]**2, 'TsM':data[2]*data[0], 'TsMg':data[1]*data[2],
                'expM':np.exp(data[0])}
        ...
        masses = np.linspace(-1, .5, 4)
        Mg = np.linspace(.2, .8, 4)
        temps = np.linspace(300., 1500., 4)
        ...
        def Tc(M, Mg, Ts):
            ...
            return model3.intercept_ +b3*Ts+c3*Ts*M+d3*M+e3*Ts*Mg+Mg*f3
    ...
    def predict(self, M, Mg, of, Si, Ts, Fe_number_mantle, xi_Fe_core,
                model_path = '/home/os18o068/Documents/PHD/Projects/Planets/Data/planet_grid_test/',
                model_name = 'models_with_ocean.pkl'):
        
        print ('predicting for values ', M, Mg, of, Si, Ts, Fe_number_mantle,
               xi_Fe_core)
       ...
       -1029.63074067]
        ...
        '''
        return [10**models[0].result[0], 10**models[1].result[0], models[2].result[0]]