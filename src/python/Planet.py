...
from PIMPrunparams import eps_Psurf, eps_Mtot, eps_layer, param_colors, \
                    plot_units, suffix, eps_Mg_number
import numpy as np
import time
#import eos
...
class Shell():
    def __init__(self, radius=.1, T=None, m=None, P=None, d=None, layer=0, \
                 tempType = 0, status='bare', contents=[],\
                 fractions=[], Fe_number=[], X_H2O=[], gamma=1.36, eps_r=.5, 
                 rhoType='integrate', **kwargs):
        ...
        self.dmdr = 4. * np.pi * self.radius**2 * self.dens
    ...
    def prt(self, digits = 3, **kwargs):
        ...
        print ('\ntempType:', self.tempType)
        print ('\nThe gradients are:')
        print ('\ndP/dr [Pa/m]:', formatstr.format(Decimal(str(self.dPdr))), 
               '\ndm/dr [kg/m]:', formatstr.format(Decimal(str(self.dmdr))),
               '\ndT/dr [K/m]:', formatstr.format(Decimal(str(self.dTdr))),
               '\ndrho/dr [kg/m4]:', round(self.drhodr, digits),
               '\ndT/dP [K/Pa]:', formatstr.format(Decimal(str(self.dTdP))),
               '\ndP/drho [Pa m3/kg]:', formatstr.format(Decimal(str(self.dPdrho))))
    ...
    def construct(self, dr=1., overconstruct = False, fortran=False, **kwargs):
        ...
        if self.status == 'constructed' and not overconstruct:
            ...
            self.radius, newy = eosfort.integrate(r_start=self.radius, 
                                h=dr,
                                y_in=params, gammag=self.gamma, 
...
class Layer():
    def __init__(self, r_in = None, r_out=None, T=None, P=None, contents=[], \
                 fractions = [], m=None, eps_r=.5, tempType = 0,\
                 Fe_number=[], X_H2O=[], rhoType='constant', 
                 mass_should=None, **kwargs):
        ...
        if len(X_H2O) == 0:
            ...
            self.X_H2O = [0. for i in range(len(self.contents))]
    ...
    def construct(self, **kwargs):
        ...
        if not self.force_bisection:
            #initiate new shell
            shell = Shell(T=self.temp, P=self.pres, contents=self.contents,\
                  fractions =self.fractions, radius=self.radius, d=self.dens,
                  m=self.mass, tempType = self.tempType, eps_r=self.eps_r, \
                  Fe_number=self.Fe_number, X_H2O=self.X_H2O, 
                  rhoType = self.rhoType, **kwargs)
...
class Planet():
    def __init__(self, P_center=1.0e12, T_center=300, R_seed=.1, contents=[],\
                 tempType = 0, layerType='homogeneous', \
                 fractions=[], P_surface_should=P_earth, 
                 majorConstraint='P_surface',
                 layerConstraint='mass', add_atmosphere=False, eps_r=.5, \
                 layermasses=[], M_surface_should=1., T_surface_should=300, 
                 R_surface_should=1., P_dissoc=5.0e11, f_dissoc=1., 
                 Fe_number=[], X_H2O=[], rhoType='integrated', echo=False, 
                 brucite_dissoc=False, vapor_stop=False, 
                 Mg_number_should=Mg_number_solar, Si_number_should=0.,
                 Mg_number_layer = False, n_refine = 32, modelType=0, 
                 SiMg=0., **kwargs):
        ...
        if self.modelType == 1:
            self.fractions[1] = [Material.xi_per(self.SiMg),
                                    Material.xi_ol(self.SiMg)]
        ...
        if len(Fe_number) == 0:
            ...
            for c in self.contents:
                self.Fe_number.append([0. for i in range(len(c))])
            

        if len(X_H2O) == 0:
            print ('setting X_H2O = 0.0')
            for c in self.contents:
                self.X_H2O.append([0. for i in range(len(c))])
        ...
        self.seed = Layer(m=self.M_seed, T=self.T_center, \
                          P=self.P_center, tempType = self.tempType,\
                          r_in = self.R_seed, contents=self.contents[0], \
                          fractions=self.fractions[0], eps_r=self.eps_r,\
                          Fe_number=self.Fe_number[0], X_H2O=self.X_H2O[0], 
                          rhoType=self.rhoType, **kwargs)
    ...
    def prt(self, digits=5, **kwargs):
        ...
        for i in range(len(self.layers)):
            ...
            for j in range(len(self.contents[i])):
                print (round(self.fractions[i][j]*100, 3), '%', 
                       material_list[self.contents[i][j]])

            print ('mass [M_earth]: ', round(self.layers[i].indigenous_mass/\
                   m_earth, digits))
        ...
        try:
            ...
            print ('--------------------------------')
            print ('R_surface_is [R_earth]:', round(self.R_surface_is/r_earth, digits),
               '\nM_surface_is [M_earth]:', round(self.M_surface_is/m_earth, digits),
               ...
               '\nM_H2O_should [M_earth]:', round(self.M_H2O_should, digits))
            print ('--------------------------------')
            ...
            while self.layerIteration:
                ...
                while self.shellIteration:
                    ...
                    try:
                        ...
                        if self.layers[-1].contents==[11] and \
                        shell.mix.mix[0].phase != last_shell.mix.mix[0].phase:
                            if self.additionally_refined:
                                ...
                                for i in range(self.n_refine - 1):
                                    ...
                                    if sh.mix.mix[0].phase == 2:
                                        delta_Mg = sh.indigenous_mass/(mMg+mO)\
                                        *mMg
                                        
                                    else:
                                        delta_Mg = sh.indigenous_mass/(mMg+2*mO+2*mH)\
                                        *mMg
                                    
                                    self.Mg_count += delta_Mg
                                    self.Mg_number_is = self.Mg_count/mMg/\
                                    ...
                                    if (self.Mg_number_is - self.Mg_number_should)/\
...
# =============================================================================
                    ...
                    if self.majorConstraint == 'P_surface':
                        ...
                        self.major_bisection(pres, self.P_surface_should, 
                                             eps=eps_Psurf, direction=[-1, -1])  
                    ...
                    #use total mass to define surface of planet
                        ...
                        self.major_bisection(mass, self.M_surface_should, 
                                             eps=eps_Mtot, direction=[1, -1])
                    ...
                    self.majorComplete:
                        ...
                        if self.layerConstraint == 'mass' and self.Mg_number_layer and lay == 1:
                            if lay == 1:
                                sh = self.layers[lay].shells[-2]
                                if sh.mix.mix[0].phase == 2:
                                    delta_Mg = sh.indigenous_mass/(mMg+mO)\
                                ...
                                #the desired value                                   
                                elif abs(self.Mg_number_is - self.Mg_number_should)/\
                                self.Mg_number_should < eps_Mg_number:
                                    ...
                                    #initiate next layer
                                        ...
                                        nextlayer = Layer(m=mass, T=temp, P=pres, \
                                                          tempType = self.tempType, \
                                                          r_in = radius, \
                                                          contents= \
                                                          self.contents[lay+1], \
                                                          fractions=\
                                                          self.fractions[lay+1],\
                                                          eps_r=self.eps_r,
                                                          X_H2O=self.X_H2O[lay+1],
                                                          Fe_number=\
                                                          self.Fe_number[lay+1],
                                                          rhoType=self.rhoType)
                        ...
                        if self.layerConstraint == 'mass' and checkLayer \
                        and not self.layerComplete:
                            ...
                            if (self.layermasses[lay]*m_earth - laymass)/\
                            laymass < -eps_layer:
                                '''
                            ...
                            #specified accuracy                                    
                            elif abs(self.layermasses[lay]*m_earth - \
                                     laymass)/laymass < \
                                     eps_layer:
                                ...
                                if len(self.layers)==len(self.contents) \
                    ...
                    if self.echo:
                        ...
                        try:
                            dat = [[ftool.scinot(radius/r_earth, digits=digits),
                                ...
                                ftool.scinot(self.layers[-1].dr/1000., digits=digits),
                                lay, 
                        ...
                        if self.layerComplete:
                            print ('\nDesired pressicion for layer', lay-1,
    ...
    def Plot(self, scatter=False, layerColoring = False, axis=[], 
        ...
        title_list = [r'$Pressure \ [GPa]$', r'$Mass \ [M_\oplus]$', 
                      r'$Density \ [kg/m^3]$', r'$Temperature  \ [K]$',
                      r'$Gravity \ [m/s^2]$', r'$v_{esc} \ [km/s]$']
        ...
        plot_list_dummy = [[[shell.pres*1.0e-9 for shell in layer.shells] 
                    ...
                    [[shell.mass/m_earth for shell in layer.shells] 
                                            for layer in self.layers ],
                    [[shell.dens for shell in layer.shells] 
                                            for layer in self.layers ],
