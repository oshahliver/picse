...
def temp(r=None, rb = 10e5, 
    ...
    T0, rho, k, qb, Qi = T0_list[layer], rho_list[layer], k_list[layer], qb_list[layer], Qi_list[layer]
    H = Qi / M
...
test = ftool.bisec(f=temp, whicharg = 'r', a=1e5, b=1e7, layer=0, y=273*.95, noisy=False, 
            M = M_solar['europa'] * m_earth, 
...
print ('temp =', temp(r=test, M = M_solar['europa'] * m_earth, 
            aa = R_solar['europa'] * r_earth * f,
            rb = rb,
            layer = 0) / 0.95)
