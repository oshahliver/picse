...
def density(P = None, T = None, **kwargs):
    ...
    """
    return ftool.bisec(whicharg = 'd', f = pressure, T = T, y = P, a = 1.0e-3, 
                        b = 1.0e6, identity = 'density', acc=1.0e-10, **kwargs)
...
def F(T=None, P=None, d=None):
    ...
    return mazevet.compute(d/1000., T)[5]
...
def U(T=None, P=None, d=None):
    ...
    return mazevet.compute(d/1000., T)[6]*1000.
...
def u_spec(P = None, T = None, d=None, **kwargs):
    ...
    return mazevet.compute(d/1000., T)[3]*1000.*1.0e-7
...
def dPdrho_S(T = None, P = None,  d = None, **kwargs):
    ...
    derlog = mazevet.compute(d/1000., T)[2]
...
def together(T=None, P=None):
    ...
    d = density(T=T, P=P)
    pres, dlnpdlrho, dlnPdlnrho, u_spec, c_v, E, U = \
    mazevet.compute(d/1000., T)
    
    #convert Mbar to Pa in the denuminator
    dlnPdlnrho = dlnPdlnrho*1.0e6*1.0e5

    #convert gcc to kg m-3 in the denominator
    dlnPdlnrho = dlnPdlnrho / 1000.
