...
Mg_numbers = np.linspace(.2, .7, 2)
xi_Fe = np.linspace(0., Fe_number_mantle, 3)
Si_numbers = np.linspace(.4, .5, 1)
xi_Stv = np.linspace(0., .2, 3)
temps = np.linspace(500, 2000, 2)
masses = np.logspace(-1, np.log10(3.), 24)


#M, Mg#, T_surf
accs = [[1.0e-4, 1.0e-1, 1.0e-4],
        [1.0e-4, 1.0e-1, 1.0e-2],
        [1.0e-5, 1.0e-1, 1.0e-2]]

N_params = 24


data = np.empty([N_params, len(Si_numbers), len(temps), len(Mg_numbers), 
                 len(masses)])

ocean_masses = np.empty([len(Si_numbers), len(temps), len(Mg_numbers), 
                         len(masses)])
        ...
        for j in range(len(Mg_numbers)):
            ...
            for k in range(len(masses)):
                ...
                pl=toolkit.model_hydro(P_center=9.5e10, 
                                       ...
                                       temp_jumps=[0., CMB_fac*delta_T_CMB(masses[k]), 
                                                   0., 0., 0.], 
        ...
        for j in range(len(Mg_numbers)):
            ...
            for k in range(len(masses)):
                ...
                dat = pl.M_surface_is/m_earth, pl.R_surface_is/r_earth, pl.Mg_number_is, \
                    ...
                    pl.P_center, pl.ocean_depth, pl.xi_H_core, pl.xi_H_core_predicted,\
                    pl.Mg_number_should, pl.M_surface_should/m_earth, pl.T_surface_should, \
                    pl.ocean_frac_should, pl.ocean_frac_is, pl.P_H2_CMB, \
