...
from PIMPphysicalparams import T_triple, T_critical, P_triple, P_critical, mO,\
                                mH, kB, NA, Rgas
...
def test(T=None, P=None):
    return together(T=T, P=P)

def Plot(P_min=10., P_max=1.0e13, T_min=100., T_max=1.0e4, log=True,
         res=3, data_log = False, Nx_ticks=10, Ny_ticks=10,
         params = [0]):
    labels = [r'$\rho \ \rm [kg \ m^{-3}]$',
    ...
    if log:
        ...
        x_tick_labels = [r'$10^{}$'.format('{'+str(int(xx))+'}') for xx in x_tick_labels]
        y_tick_labels = np.linspace(np.log10(P_min), np.log10(P_max), Ny_ticks)
        y_tick_labels = [r'$10^{}$'.format('{'+str(int(yy))+'}') for yy in y_tick_labels]
...
def analyze(P_min=10., P_max=1.0e12, T_min=100., T_max=1.0e5, P_log=True,
         T_log=True, analytical=None, order=2, resolution = 16, param=2,
         **kwargs):
    ...
    if T_log:
        ...
        major_tick_labels_x=np.linspace(np.log10(T_min), np.log10(T_max), 
                                            nnT, dtype=int)
    ...
    if P_log:
        ...
        major_tick_labels_y=np.linspace(np.log10(P_min), np.log10(P_max), 
                                        nnP, dtype=int)
        
    else:
        P_list=np.linspace(P_min, P_max, N_P)
        major_ticks_y=np.linspace(0, 1, 6)
        major_tick_labels_y=np.linspace(int(P_min*1.0e-9), 
                                        int(P_max*1.0e-9), 6)
    ...
    for i in range(len(T_list)):
        for j in range(len(P_list)):
            dat_table[i][j] = watab.interpolate(order=order, x=T_list[i], \
                     y=P_list[j], param=param)
            ...
            dat_residual[i][j] = (dat_analytical[i][j] - dat_table[i][j])/\
                                    dat_analytical[i][j]
    ...
    dat_residual=(np.log10(abs(dat_residual))-def_zero)*\
                    dat_residual/abs(dat_residual)
    ...
    im00=axes[0][0].imshow(dat_analytical.T, cmap='rainbow', extent=[0,1,0,1],
        norm=None, aspect='equal', origin='lower', vmin=0., vmax=cbar_max_lin)
    
    im01=axes[0][1].imshow(np.log10(dat_analytical.T), cmap='rainbow', 
             norm=None, aspect='equal', origin='lower', extent=[0,1,0,1],
             vmin=cbar_min_log, vmax=cbar_max_log)

    im10=axes[1][0].imshow(dat_table.T, cmap='rainbow',extent=[0,1,0,1],
        norm=None, aspect='equal', origin='lower', vmin=0., vmax=cbar_max_lin)
    
    im11=axes[1][1].imshow(np.log10(dat_table.T), cmap='rainbow', 
        norm=None, origin='lower', vmin=cbar_min_log, extent=[0,1,0,1],
        vmax=cbar_max_log)
    ...
    cmap=mpl.colors.ListedColormap([(1., .5, .5), (1., .0, .0), (.6, .0, .0),
                                    (.2, .0, .0), (.0, .0, .0), (.0, .0, .0),
                                    (.0, .2, .0), (.0, .6, .0), (.0, 1., .0),
                                    (.5, 1., .5)])
    ...
    im = ax.imshow(dat_residual.T, 
                   cmap=cmap, extent=(0, 1, 0, 1),
                     norm=norm, aspect='equal', origin='lower',
                     vmin=def_zero, vmax=-def_zero)
...
def replot(dat_analytical, dat_table, dat_residual, P_min=10., P_max=1.0e12, 
           T_min=100., T_max=1.0e5, P_log=True, T_log=True, 
           color_scheme='rainbow', **kwargs):
    ...
    im01=axes[0][1].imshow(np.log10(dat_analytical.T), cmap='rainbow', 
             extent=(0,1,0,1), norm=None, aspect='equal', origin='lower', 
             vmin=cbar_min_log, vmax=cbar_max_log)
    ...
    fig, ax = plt.subplots()
    im = ax.imshow(dat_residual.T, 
                   cmap='gist_stern', extent=(0,1,0,1),
                     norm=None, aspect='equal', origin='lower', vmin=-1, vmax=1)
    ...
    cbar_ticklabels = [-1, -.1, -.01, -.001, -.0001, -.00001, 0., .00001, 
                       .0001, .001, .01, 1]
    print (cbar_ticks)
    ax.set_xlabel(r'$log_{10}(T) \ [K]$')
    ax.set_ylabel(r'$log_{10}(P) \ [Pa]$')
