...
TABLE2_gamma = [[-2.604037e-2, 2.104060e-2, -1.515928e-3], 
...
def ue(d=None):
    ...
    """
    return a0+a1*d+a2*d**2+a3*d**3+a4*np.log(d)+a5*np.log(d)**2
...
def ft(d=None, T=None):
    ...
    for i in range(1):
        ...
        for k in range(3):
            ...
            K1 += TABLE2_alpha[k] * T * (3.*np.log(1.-np.exp(-Ti/T))-\
                              D(z=Ti/T, n=3))*d**(kk/k0)
    ...
    for j in range(4):
        ...
        for k in range(3):
            ...
            K2 += TABLE2_gamma[j][k]*T*np.log(1.-np.exp(-Tj/T))*d**(kk/k0)
...
def dftdd(d=None, T=None):
    ...
    for j in range(4):
        ...
        for k in range(3):
            kk = k-4
            #print (j, kk, TABLE2_gamma[j][k])
            K2 += TABLE2_gamma[j][k]*T*np.log(1.-np.exp(-Tj/T))*d**(kk/k0-1.)*kk/k0
...
def dfeddd(d=None, T=None):
    ...
    """
    return 2*a2 + 6*a3*d - a4/d**2 - 2*a5/d**2*(np.log(d)-1.)
    

def dfnddd(d=None, T=None):
    """Second density derivative of nuclear contribution
    """
    return 2*b2 - 10.*b3*b4*d**8*np.exp(-b4*d**10)*(9.-10.*b4*d**10)
...
def dftddd(d=None, T=None):
    ...
    for i in range(1):
        ...
        for k in range(3):
            kk = k-4
            factor = kk/k0 *(kk/k0 - 1.)*d**(kk/k0-2.)
            #print (i, kk, TABLE2_alpha[k])
            K1 += TABLE2_alpha[k] * T * (3.*np.log(1.-np.exp(-Ti/T))-\
                              D(z=Ti/T, n=3))*factor
    ...
    for j in range(4):
        ...
        for k in range(3):
            ...
            factor = kk/k0 *(kk/k0 - 1.)*d**(kk/k0-2.)
            #print (j, kk, TABLE2_gamma[j][k])
            K2 += TABLE2_gamma[j][k]*T*np.log(1.-np.exp(-Tj/T))*factor
...
def dftdT(d=None, T=None):
    ...
    for i in range(1):
        ...
        for k in range(3):
            kk = k-4
            #print (i, kk, TABLE2_alpha[k])
            K1 += TABLE2_alpha[k] * (4.*D(Ti/T) - \
                              3.*np.log(1.-np.exp(-Ti/T)))*d**(kk/k0)
    ...
    for j in range(4):
        ...
        for k in range(3):
            kk = k-4
            #print (j, kk, TABLE2_gamma[j][k])
            K2 += TABLE2_gamma[j][k]*(Tj/T*1./(np.exp(Tj/T)-1.)-\
                              np.log(1.-np.exp(-Tj/T)))*d**(kk/k0)
...
def dftdT2(d=None, T=None):
    ...
    for i in range(1):
        ...
        for k in range(3):
            kk = k-4
            #print (i, kk, TABLE2_alpha[k])
            K1 += TABLE2_alpha[k] * (-12./T*(Ti/T/(np.exp(Ti/T)-1.)-D(Ti/T)) + \
                              3.*np.exp(-Ti/T)*Ti/T**2)*d**(kk/3.)
    ...
    for j in range(4):
        ...
        for k in range(3):
            kk = k-4
            #print (j, kk, TABLE2_gamma[j][k])
            K2 += TABLE2_gamma[j][k]*(-Tj/T**2*1./(np.exp(Tj/T)-1.)+\
                              Tj**2/T**3*(np.exp(Tj/T-1.))**(-2)*np.exp(Tj/T)+\
                              np.exp(-Tj/T)*Tj/T/(1.-np.exp(-Tj/T)))*d**(kk/3.)
...
def Density(P=None, T=None):
    ...
    d = ftool.bisec(f=Pressure, a=a, b=8000., y=P, whicharg='d', T=T,
                    identity='French dens', acc=1.0e-10, limit=100)
