        ...
        for i in range(self.b):
            ...
            if self.a > 1:
                ...
                for p in range(6):
                    ...
                    pl, = self.ax[i].plot(dat[0][p], dat[1][p], marker='s',
                                 linestyle='-')
    ...
    def adjust(self, labels=None):
        ...
        for i in range(len(self.ax)):
            if self.a > 1:
                ...
                #general adjustments
                axx.tick_params(which='both', top=True, right = True, left=True,
                                direction='in', size=ticks)
                
                axx.set_xscale('log')
                axx.grid(color=(.95, .95, .95), which ='both', linewidth=1)
                axx.grid(color=(.95, .95, .95), which ='major', linewidth=2)
            ...
            self.ax[0].text(.15, 2.05, r'$\rm Mg$'+'#'+r'$=0.76 \ ($'+'~'+
                   r'$\rmsolar)$',
                   fontsize=14)
            
            self.ax[0].text(.15, 1.92, r'$\rm T = 300 \ K$', fontsize=14)
            
            self.ax[0].text(.6, .4, r'$\rm (R_{hyd} - R_{dry})/R_{dry}$'
                   +' ~'+r'$5-10$'+'%', color=(.4, .4, .4), fontsize=14)
            ...
            try:
                ...
                for p in range(len(self.plots[0])):
                    ...
                    self.ax[0].text(0.15, 1.76-.1*p, labels[p], 
                           color=col)
            ...
            self.ax[0].errorbar([4.736, 4.357], [1.602, 1.511], zorder=10,
                   xerr=errorsx, yerr=errorsy, color='k', marker='o', 
                   linestyle='None')
