...
class ColorMap():
    def __init__(self, col=1, row=1, n_bin = 3,
                 hspace = 1.0, wspace = .25):
        ...
        for n in range(n_bin):
            self.colors.append((1-1/(n+1), 1-1/(2*(n+1)), 1-1/(n+1)))
    ...
    def Plot(self, Z=None):
        for i in range(self.row):
            for j in range(self.col):
                ax = self.ax[i][j]
                self.im = ax.imshow(Z, interpolation='nearest', origin='lower', 
                        cmap=self.cm, aspect = 'auto')
...
class Plot():
    ...
    def __init__(self, col=1, row=1, axis_labels=None, axis_limits=None,
                 minorlocatorx=None, majorlocatorx=None, minorlocatory=None,
                 majorlocatory=None, x_minor_labels=None, y_minor_labels=None,
                 x_major_labels=None, y_major_labels=None, hspace=1.0, wspace=0.25,
                 plot_titles = None, logx=False, logy=False, majorlabelsize=14, 
                 minorlabelsize=8, axislabelsize=16, axislabelpad=5,
                 majortickwidth=1, minortickwidth=1, minor_labels = False,
                 sharex=False, sharey=False, titlefontsize=16, title_pad=10,
                 figsize=(20, 10), grid=True, top=True, right = True, left=True,
                 bottom=True, labelright = True, labeltop=True, labelleft=True
                 ):
        ...
        if not axis_limits == None:
            ...
            self.axis_limits = [[[[0., 1.], [0., 1.]] for j in range(col)] 
                                for i in range(row)]
        ...
        for i in range(row):
            ...
            for  j in range(col):
                delta_lims_x = self.axis_limits[i][j][0][1]-\
                                self.axis_limits[i][j][0][0]
                                
                n_major_ticks_x = int(delta_lims_x)+1
                
                delta_lims_y = self.axis_limits[i][j][1][1]-\
                ...
                if majorlocatorx[i][j] == None or self.logx[i][j]:
                    if self.logx[i][j]:
                        ...
                        self.majorlocatorx[i].append(np.linspace(int(self.axis_limits[i][j][0][0]),
                ...
                if majorlocatory[i][j] == None or self.logy[i][j]:    
                    if self.logy[i][j]:
                        ...
                        self.majorlocatory[i].append(np.linspace(int(self.axis_limits[i][j][1][0]),
        ...
        if not minorlocatorx == None:
            ...
            self.minorlocatorx = [[0.5 for j in range(col)]
                                    for i in range(row)]
        ...
        if not minorlocatory == None:
            ...
            self.minorlocatory = [[0.5 for j in range(col)]
                                    for i in range(row)]
        ...
        if not axis_labels == None:
            ...
            self.axis_labels = [[['x [a.u.]', 'y [a. u.]'] for j in range(col)]
                                for i in range(row)]
        ...
        for i in range(row):
            for j in range(col):
                ...
                for pos in ['top', 'bottom', 'left', 'right']:
                    axx.spines[pos].set_linewidth(1.)
                ...
                if i == 0:
                    axx.set_title(self.plot_titles[i][j], 
                                  fontsize=titlefontsize, 
                                  pad=title_pad)
                
                #Set x-axis labels
                axx.set_xlabel(self.axis_labels[i][j][0], fontsize=axislabelsize, 
                               labelpad=axislabelpad)
                
                #Set y-axis labels
                axx.set_ylabel(self.axis_labels[i][j][1], fontsize=axislabelsize, 
                               labelpad=axislabelpad)             
                ...
                axx.tick_params(which='major', length=plot_params['majorticklen'],
                                labelsize=majorlabelsize, width=majortickwidth)
                axx.tick_params(which='minor', length=plot_params['minorticklen'],
                                labelsize=minorlabelsize, width=minortickwidth)
                ...
                if self.logx[i][j]:
                    ...
                    axx.xaxis.set_major_locator(FixedLocator(
                            self.majorlocatorx[i][j]))                    
                    axx.xaxis.set_minor_locator(FixedLocator(xticks_minor))
                
                #create linear ticks
                else:
                    xticks_minor = np.arange(xmin, xmax+self.minorlocatorx[i][j],
                                       self.minorlocatorx[i][j])

                    xticks_major = np.arange(xmin, xmax+self.majorlocatorx[i][j],
                                       self.majorlocatorx[i][j])
                      
                    axx.xaxis.set_major_locator(MultipleLocator(
                            self.majorlocatorx[i][j]))
                    
                    axx.xaxis.set_minor_locator(MultipleLocator(
                            self.minorlocatorx[i][j]))
                ...
                if self.logy[i][j]:
                    ...
                    axx.yaxis.set_major_locator(FixedLocator(
                ...
                #create linear ticks
                    ...
                    yticks_minor = np.arange(ymin, ymax+self.minorlocatory[i][j],
                                       self.minorlocatory[i][j])

                    yticks_major = np.arange(ymin, ymax+self.majorlocatory[i][j],
                                       self.majorlocatory[i][j])
                      
                    axx.yaxis.set_major_locator(MultipleLocator(
                            self.majorlocatory[i][j]))
                    
                    axx.yaxis.set_minor_locator(MultipleLocator(
                            self.minorlocatory[i][j]))
                ...
                if grid:
                    ...
                    for x in xticks_minor:
                        axx.plot([x, x], [ymin, ymax], color=plot_params['gridcolor'],
                                 linewidth=1, zorder=-100, alpha=plot_params['gridalpha'])
                    
                    #draw minor y-grid lines
                    for y in yticks_minor:
                        axx.plot([xmin, xmax], [y, y], color=plot_params['gridcolor'],
                                 linewidth=1, zorder=-100, alpha=plot_params['gridalpha'])
                    ...
                    for x in xticks_major:
                        ...
                        axx.plot([x, x], [ymin, ymax], color=plot_params['gridcolor'],
                                 linewidth = 2, zorder=-100, alpha=plot_params['gridalpha'])
        ...
        for i in range(self.row):
            ...
            for j in range(self.col):
                ...
                if self.logx[i][j]:
                    ...
                    #process x axis
                    for ll in range(int(self.axis_limits[i][j][0][1]-\
                                        self.axis_limits[i][j][0][0])+1):
                        xlabel = self.axis_limits[i][j][0][0] + ll
                        self.x_major_labels[i][j].append(r'$10^{{{1:d}}}$'.
                                           format(1, xlabel))
                        
                else:
                    delta_x = self.axis_limits[i][j][0][1] -\
                                    self.axis_limits[i][j][0][0]                    

                    n_major_ticks_x = int(round(delta_x/\
                                                self.majorlocatorx[i][j]+1,0))
                
                    n_minor_ticks_x = int(round(delta_x/\
                                                self.minorlocatorx[i][j]+1,0))

                    x_major_labels_dummy = np.arange(self.axis_limits[i][j][0][0], 
                                                     self.axis_limits[i][j][0][1]+\
                                                     self.majorlocatorx[i][j], 
                                                     self.majorlocatorx[i][j])
    
                    x_minor_labels_dummy = np.linspace(self.axis_limits[i][j][0][0], 
                                                       self.axis_limits[i][j][0][1], 
                                                n_minor_ticks_x )
                    ...
                    for n in range(len(x_minor_labels_dummy)):
                        ...
                        if round(number/self.majorlocatorx[i][j], 0) == \
                        round(number/self.majorlocatorx[i][j], roundx) or \
                        n == len(x_minor_labels_dummy)-1:
                ...
                if self.logy[i][j]:
                    for ll in range(int(self.axis_limits[i][j][1][1]-\
                                        self.axis_limits[i][j][1][0])+1):
                        ylabel = self.axis_limits[i][j][1][0] + ll
                        self.y_major_labels[i][j].append(r'$10^{{{1:d}}}$'.
                                           format(1, ylabel))                

                else:
    
                    delta_y = self.axis_limits[i][j][1][1] -\
                                    self.axis_limits[i][j][1][0]
                                    
                    n_major_ticks_y = int(round(delta_y/\
                                                self.majorlocatory[i][j]+1,0))

                    n_minor_ticks_y = int(round(delta_y/\
                                                self.minorlocatory[i][j]+1,0))
                    
                    y_major_labels_dummy = np.arange(self.axis_limits[i][j][1][0], 
                                                     self.axis_limits[i][j][1][1]+\
                                                     self.majorlocatory[i][j], 
                                                     self.majorlocatory[i][j])
                    
                    y_minor_labels_dummy = np.linspace(self.axis_limits[i][j][1][0], 
                                                       self.axis_limits[i][j][1][1], 
                                                n_minor_ticks_y)
                    ...
                    for n in range(len(y_minor_labels_dummy)):
                        ...
                        if round(number/self.majorlocatory[i][j], 0) == \
                        round(number/self.majorlocatory[i][j],roundy) or \
                        n == len(y_minor_labels_dummy)-1:
