...
from PIMPphysicalparams import T0_list, K0_list, K0prime_list, rho0_list, \
                                aP_list, molar_mass_list, Rgas, mH, kB, G,\
                                EOS_type, material_list, EOSstring_list, NA, \
                                P_earth, m_earth, r_earth, material_plot_list,\
                                gas_molar_mass_list, gas_EOS_type, T_zero,\
                                gas_material_list, gas_gamma_list, P_zero, \
                                rho_zero
                                
from PIMPrunparams import eps_Psurf, eps_Pspace, eps_Mtot, param_colors,\
temp_round, plot_units
import numpy as np
#import eos
...
def m_barom(mu=None, r=None, g=None, R_planet=None, T=None, rho0=None, M0=None):
    H=mu*g/(Rgas*T)
    return -4*np.pi*rho0/H**3 * ((H**2*r**2 + 2*H*r + 2)*\
                                np.exp(-H*(r-R_planet)) - \
                                H**2*R_planet**2 - 2*H*R_planet -2) + M0


class Unit():
    def __init__(self, ll=None, P=1.0e5, T=300, d=0., **kwargs):
        self.which=ll
        self.pres=P
        self.temp=T
        self.dens=d
    ...
    def prt(self, digits = 20, level = 0):
        ...
        print ('material:', self.material, '\nfraction:', self.fraction,
               '\nEOS:', EOSstring_list[which],\
                '\nphase:', self.phase,'\n-----',\
                '\ndens  [kg/m³]:', round(self.dens, digits),'\npres  [GPa]  :',\
...
class Mixture():
    ...
    """
    def __init__(self, contents = [], fractions = [], T=300, P=1.0e5, d=None, \
                 **kwargs):
    ...
    def Compute(self, **kwargs):
        ...
        self.mix =  [Unit(i, P = self.pres, T = self.temp, **kwargs)
...
class Shell():
    def __init__(self, radius=.1, T=None, m=None, P=None, d=None,\
                 tempType = 'adiabatic', status='bare', contents=[],\
                 fractions=[], **kwargs):
        ...
        self.dPdr = None
        self.mix = Mixture(T=self.temp, P=self.pres, contents=self.contents,\
        ...
        self.dTdr = None
        self.dmdr = None
        self.gamma = sum([gas_gamma_list[l] for l in self.contents])/\
                    len([gas_gamma_list[l] for l in self.contents])
        self.status = status
        self.layer=-1
    ...
    def prt(self, **kwargs):
        print ('Shell at r =',self.radius*1.0e-3,'km, enclosing m =',\
               self.mass,'kg')
        print ('\nstatus:', self.status)
        print ('\ntempType:', self.tempType)
        print ('\nThe gradients are:')
    ...
    def Construct(self, dr = 1., overconstruct = False, **kwargs):
        ...
        if self.status == 'constructed' and not overconstruct:
            ...
            self.radius, newy = ftool.integrate(dydx=gradients, \
                        ...
                        fractions = self.mix.fractions, **kwargs)
...
class Layer():
    def __init__(self, r_in = None, r_out=None, T=None, P=None, contents=[], \
                 fractions = [], m=None, eps_r=.5, tempType = 'adiabatic',\
                 **kwargs):
        self.tempType = tempType
        self.indigenous_mass=0.
        self.r_in = r_in
    ...
    def Update(self, **kwargs):
        ...
        self.pres, self.mass, self.temp, self.dens = \
            shell.pres, shell.mass, shell.temp, shell.dens
    ...
    def plot(self, pres_unit='bar', temp_unit='K', dens_unit = 'kg/m3', 
             mass_unit = 'm_earth', rad_unit='r_earth', **kwargs):
        ...
        radius_list = np.array([shell.radius/plot_units[rad_unit] 
                                for shell in self.shells])
        plot_list = np.array([[shell.pres/plot_units[pres_unit] 
                                for shell in self.shells],
                     [shell.mass/plot_units[mass_unit]
                                for shell in self.shells],
                     [shell.dens/plot_units[dens_unit]
                                for shell in self.shells],
                     [shell.temp/plot_units[temp_unit]
                                for shell in self.shells]])
        ...
        for i in range(len(self.axes)):
            self.axes[i].plot(radius_list, plot_list[i], \
...
class Atmosphere():
    def __init__(self, r_in=1, r_out=None, T=300, P=P_earth, contents=[], \
                 fractions = [], m=1, eps_r=.5, tempType = 'isothermal',\
                 P_space=P_zero, M_atmos=.000001, majorConstraint='P_space', \
                 T_space=T_zero, rho_space=rho_zero, **kwargs):
        ...
        atm = Layer(r_in=self.R_surface, m=self.M_surface, P=self.P_surface,\
                    T=self.T_surface, contents=self.contents, \
                    fractions=self.fractions, tempType=self.tempType,\
                    eps_r=self.eps_r, **kwargs)
        ...
        while shellIteration:
            ...
            if self.layers[-1].pres < P_zero:
                print ('Zero pressure reached at', 
                       self.layers[-1].pres,'Pa after', shellIterationCount,
                       'iterations')
                shellIteration=False

            if self.layers[-1].temp < T_zero:
                print ('Zero temperature reached at', 
                       self.layers[-1].temp,'K after', shellIterationCount,
                       'iterations')
                shellIteration=False

            if self.layers[-1].dens < rho_zero:
                print ('Zero density reached at', 
                       self.layers[-1].dens,'kg m-3 after', shellIterationCount,
                       'iterations')
                shellIteration=False
            ...
            if self.majorConstraint == 'P_space':
                ...
                if (pres - self.P_space_should)/pres < - eps_Pspace:
                    ...
                    print ('Desired precission for surface pressure reached',\
                           'after',shellIterationCount,'iterations')
                    shellIteration = False
                ...
                if dev > eps_Mtot:
                    ...
                    print ('Desired precission for atmosphere mass reached',\
                           'after',shellIterationCount,'iterations')
                    shellIteration = False
            ...
            dat = [[digits.format(Decimal(str(radius/r_earth))),
            ...
            tabl = tabulate(dat, headers=["R [R_e]", "dr [km]", \
                                          "P ["+pres_unit+"]",'m [M_e]',\
                                          'T [K]','rho [kg m-3]'])
            ...
            if abs((v_esc - v0))/v_esc < .8:
                print ('Escape velocity reached after', shellIterationCount,
                       'iterations')
                print (v_esc - v0)
                print (v_esc, v0)
                shellIteration = False
    ...
    def plot(self, check=False, tempType='isothermal', log=False, pres_unit='GPa', 
             temp_unit='K', dens_unit = 'kg/m3', mass_unit = 'm_earth', 
             rad_unit='r_earth', **kwargs):
       
        self.layers[-1].plot(pres_unit=pres_unit, temp_unit=temp_unit, 
                   dens_unit = dens_unit, mass_unit = mass_unit, 
                   rad_unit = rad_unit)
        
        from PIMPrunparams import param_colors
        ...
        if check:
            ...
            mu = sum([gas_molar_mass_list[self.contents[i]]*\
                      self.fractions[i] for i in range(len(self.contents))])
            ...
            p = np.array([P_barom(mu=mu, r=xx, P0=P_surf, \
                                  T=T_surf, \
                        g=gravity, R_planet=R_surf) for xx in x])
            
            rho = np.array([rho_barom(mu=mu, r=xx, rho0=rho0, T=T_surf, \
                        g=gravity, R_planet=R_surf) for xx in x])
                
            mass = np.array([m_barom(mu=mu, r=xx, rho0=rho0, T=T_surf, \
                        g=gravity, R_planet=R_surf, M0=M_surf) \
                        for xx in x])
            ...
            x=x/r_earth
            self.layers[0].axes[0].plot(x, p/plot_units[pres_unit], 
                       markersize=3, marker='o', \
                       markevery=markevery, linestyle='None', \
                       color=param_colors[0], label='analytical model')
            self.layers[0].axes[2].plot(x, rho/plot_units[dens_unit], 
                       markersize=3, marker='o', \
                       markevery=markevery, linestyle='None', \
                       color=param_colors[2], label='analytical model')
            self.layers[0].axes[1].plot(x, mass/plot_units[mass_unit], 
                       markersize=3, marker='o', \
                       markevery=markevery, linestyle='None', \
                       color=param_colors[1], label='analytical model')
            self.layers[0].axes[3].plot(x, temp/plot_units[temp_unit], 
                       markersize=3, marker='o', \
                       markevery=markevery, linestyle='None', \
                       color=param_colors[3], label='analytical model')
            
            self.layers[0].axes[0].scatter(np.amax(x)*.9725,
                       np.amax(p)*.75/plot_units[pres_unit],
                       color=param_colors[0])
          
            self.layers[0].axes[0].plot([np.amax(x)*.97, np.amax(x)*.975], 
                       [np.amax(p)*.9/plot_units[pres_unit], 
                        np.amax(p)*.9/plot_units[pres_unit]],
                       color=param_colors[0], linestyle='-')
            
            self.layers[0].axes[0].text(np.amax(x)*.9775, np.amax(p)*.874/\
                      plot_units[pres_unit], 'computed'
                      )

            self.layers[0].axes[0].text(np.amax(x)*.9775, np.amax(p)*.72/\
                      plot_units[pres_unit], 'analytical'
                      )            
            '''
