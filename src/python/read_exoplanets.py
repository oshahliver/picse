...
from PIMPphysicalparams import M_solar, R_solar, T_solar, names_solar,\
    r_earth, m_earth
...
class Archive():
    def __init__(self, M_max = 10., T_max = T_critical, dM_max = .15):
    ...
    def readPureCurves(self):
        ...
        for i in range(len(self.pureCurveNames)):
            ...
            planets = ftool.load_objects(self.pureDir + self.pureCurveNames[i]+'_cali.pkl')
    ...
    def clean(self, **kwargs):
        ...
        try:
            ...
            params = ['pl_bmasse', 'pl_bmasseerr1', 'pl_bmasseerr2',
        ...
        self.df = self.df.drop(self.df[self.df['pl_bmasse'] + \
                                       self.df['pl_bmasseerr1'] > self.M_max].index)
        
        #Filter out planets for which lower mass error is too large
        self.df = self.df.drop(self.df[abs(self.df['pl_bmasseerr1'] / \
                                       self.df['pl_bmasse']) >  self.dM_max].index)
            
        #Filter out planets for which upper mass error is too large
        self.df = self.df.drop(self.df[abs(self.df['pl_bmasseerr2'] / \
                                       self.df['pl_bmasse']) >  self.dM_max].index)
    ...
    def add_max_water_radius(self, fct = _filter, **kwargs):
        ...
        for i in range(2):
            for j in range(2):
                ...
                rad = [fct(self.df['pl_bmasse'][i] + self.df[str1][i], 
                   self.df['pl_eqt'][i] + self.df[str2][i], **kwargs)
    ...
    def filter(self, fct = filter, max_mass_err = .1, **kwargs):
        ...
        self.df = self.df.drop(self.df[(self.df['pl_rade'] + \
                                       self.df['pl_radeerr1'] > \
    ...
    def plot(self, ax = None, vmin = 0, vmax = 1000, cmap = 'jet', n_ticks = 5,
             fnts1 = 14, fnts2 = 12, cbar_dims = (.05, .7, .03, .25), annotate = False,
             xy_slices = ['pl_bmasse', 'pl_rade'], z = 'pl_eqt', save = True,
             ylims = [0, 4], xlims = [0, 10], plotSolar = False, stage = 0):
        ...
        if stage > 0:
            ...
            if x == 'pl_bmasse' and y == 'pl_rade':
                ax.errorbar(x = self.df[x], y = self.df[y], 
            ...
            if stage > 1:
                ...
                for i in range(stage - 1):
                    if i < 2:
                        ...
                        ax.plot([pl.M_surface_is / m_earth for pl in curve],
            ...
            if plotSolar:
                ...
                for name in names:
                    ...
                    annots.append(ax.text(M_solar[name.lower()] + .05,
                               R_solar[name.lower()] - .1,
                               f'${name.capitalize()}$',
                                  fontsize = fnts2))
            ...
            if annotate:
                ...
                pos = {'x':[1.66, 8.5, 2.5, 2.2, 6.6, 2.6, 6.4, 5.],
                       'y':[.9,  2.75, 1.385, 1.521, 1.635, 1.242, 2.355, 2.133]}
                print (self.df['pl_name'].values)
                annots = []
                for i, txt in enumerate(self.df['pl_name']):
                    if self.df[x].values[i] < xlims[1]:
                        annots.append(ax.text(pos['x'][i], 
                                              pos['y'][i],
                                              f'{txt}',
                                              fontsize = fnts2))
        ...
        if save:
            plt.savefig('/home/os18o068/Documents/PHD/Projects/Planets/Data/arch_stage_{}.pdf'.format(stage),
                        format = 'pdf', bbox_inches = 'tight')
            plt.savefig('/home/os18o068/Documents/PHD/Projects/Planets/Data/arch_stage_{}.png'.format(stage),
