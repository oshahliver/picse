...
from PIMPphysicalparams import m_earth, r_earth, MoI_solar, delta_MoI_solar,\
    ...
    material_YS, mS, mH2O, mFe, mSi, mO, mMg, mH, mEn, mH2O, material_YFe,\
    Si_earth, Mg_earth, SFe_earth, SFe_solar, sigmaSB, V_ocean_earth
import Material
import readPREM
import PlanetFort
from PIMPrunparams import color_list, xi_Fe_mantle_max
...
params = ['Mg#', 'R', 'MoI', 'Si#', 'Si# m', 'xi_Fe', 'P_CS', 'T_CS',
          'X_S', 'X_Si', 'X_O', 'xi_FeS', 'xi_FeSi', 'xi_FeO', 'T0_CMB',
          'R_core (km)', 'R_IC (km)', 'R_HMI (km)', 'T_TBL (K)', 'xi_SiO2 m',
          'xi_FeO m', 'T_C', 'P_C', 'fO2', 'DeltaT_MTZ', 'M_H2O', 'M_H2O_mantle', 
          'Fe#_core', 'core_mass_frac', 'T_CMB', 'P_CMB', 'rho_CMB',
          'T_ICB', 'P_ICB', 'rho_ICB', 'T_MTZ', 'P_MTZ', 'rho_MTZ', 'xi_MgO m',
          'R_MTZ', 'xi_S_core', 'xi_S_mantle', 'S/Fe', 'Si/Fe', 'Mg/Fe', 'S/Si', 
          'M', 'f_IC', 'ocean_frac', 'P_surf', 'w_liq', 'w_scf', 'w_sol', 
          'h_liq', 'h_scf', 'h_sol', 'w_liq/w_H2O', 'w_SCF/w_H2O', 'w_sol/w_H2O',
          'V_liq', 'V_scf', 'V_sol', 'grav', 'hyd_struc', 'T_HMI', 'P_HMI', 'rho_HMI',
          'bulk_struc']

all_param_labels = [r'[Mg]/[Mg + Fe]', 
                    r'$R/R_\oplus$', 
                    r'$C/MR^2$',
                    r'[Si]/[Si + Mg]', 
                    r'$\rm Si\#_{\rm Mantle}$',
                    ...
                    r'$V_{\rm sol}$',
                    r'$g_{\rm Surf}$',
                    r'$\rm Hydro \ structure$', 'T_HMI', 'P_HMI', 'rho_HMI',
                    'bulk structure'
                    ]
...
def plot_N_layer_planet(N=15):
    ...
    for i in range(len(r1)):
        ...
        for j in range(len(r2)):
            r = np.array([r1[i], (1.-r1[i])*r2[j]+r1[i], r3])
            r = r*r_earth
...
def convert_X_impurity_to_xi_impurity(Si_number=None, xi_Fe=None, X=None, 
                            contents = [6,7,1], m = (2*mH+mO)):
    ...
    m_tilde = sum([fractions[i]*(1.-xi_Fe)*material_YMg[contents[i]-1] 
            ...
            sum([fractions[i]*material_YS[contents[i]-1] 
                 for i in range(len(contents))])*mS+\
            sum([fractions[i]*material_YFe[contents[i]-1] 
                 for i in range(len(contents))])*mFe
...
def compute_max_ocean_frac(Si_number=None, xi_Fe=None, X_ice = None, 
    ...
    xi_ice = convert_X_impurity_to_xi_impurity(Si_number = Si_number, 
                                     xi_Fe = xi_Fe,
                                     X = X_ice, 
                                     m = mH2O,
                                     contents = [6,7,1])
...
def filter_data_match(data, obj='titan', filter_type=1, acc_R = 1.0e-2,
                      acc_MoI = 2.0e-2):
    ...
    for it in range(iterations):
        for i in range(N):
            ...
            if sampler == 0:
                ...
                Si_number_min = 1./(3.-2*random_vals[4])
                Si_number_max = 1./(2.-random_vals[4])
                p = random.random()
            ...
            xi_ice = convert_X_impurity_to_xi_impurity(Si_number = random_vals[1],
                                             xi_Fe = random_vals[4],
                                             X = random_vals[5],
                                             m = mH2O,
                                             contents = [6,7,1])
            
            xi_ice_0 = convert_X_impurity_to_xi_impurity(Si_number = random_vals[1],
                                             xi_Fe = random_vals[4],
                                             X = random_vals[6],
                                             m = mH2O,
                                             contents = [6,7,1])
            ...
            X_ice_slope = compute_X_impurity_slope(X = random_vals[5],
                                                X_0 = random_vals[6],
                                                M_mantle = M_mantle)
           
            X_impurity_0_layers = [0., 0., random_vals[6], random_vals[6], 0.]
...
def extract_parameter_ranges(data, obj='titan', acc_R=1.0e-2,
    ...
    data, best_fit_index = filter_data_match(data, obj=obj, 
                             acc_R=acc_R, 
                             acc_MoI = acc_MoI,
...
def check_convergence(obj='titan', N_points = 4, acc_R=1.0e-2, acc_MoI = 2.0e-2,
    ...
    for i in range(N_points):
        ...
        ranges, fc, bfi = extract_parameter_ranges(data, 
                                          obj=obj, 
                                          acc_R=acc_R,
                                          acc_MoI = acc_MoI, 
                                          filter_type = filter_type)
    ...
    y_labels = [r'$\rm Mg \#$', r'$\rm Si \#$', r'$M_{\rm H_2 O}/M$', 
                r'$\xi_{\rm FeS}$', r'$\xi_{\rm Fe}$', r'${\bar{X}_{\rm Ice}}$',
    ...
    fig.savefig(data_dir+obj.capitalize()+'/'+obj+'_hyd_convergence.pdf',
                    format='pdf',
                    bbox_inches='tight')    
    plt.close(fig)
...
def create_input():
    ...
    while True:
        ...
        ocmf = [0., inputs[2], inputs[3], inputs[4]]
        xi=Material.mat2at_core(ocmf, xiH=0.)
        ...
        simmin = Material.Si_number_min(1.-fem)
        simmax = Material.Si_number_max(1.-fem)
        
        femmin = margin
        femmax = xi_Fe_mantle_max*(1. + margin)
        ...
        if sim < simmin*(1.+margin) or sim > simmax*(1.-margin):#simmin*1.001 or sim > simmax*.999:
            check = False
...
def planet_sampler(N=1):
    ...
    for i in range(N):
        mg = all_inputs[i][0]
        ocmf = [1., all_inputs[i][2], all_inputs[i][3], all_inputs[i][4]]
        icmf = [1.]
        pcs = all_inputs[i][1]
        ts = all_inputs[i][5]
        T0 = all_inputs[i][6]
        ...
        of=-8;im=0;m=all_inputs[i][8];o=False;it=1;eps_r=.25;FeS=ocmf[2];Pc=9.5e10
        pl=toolkit.model_hydro(P_center=Pc, 
                                match_mass=True, 
                                eps_r=eps_r, 
                                predictor_T='none', 
                                ocean=o, 
                                ocean_frac_should=of, 
                                temp_jumps=[0., T0*m**.75, dtmtz, 0., 0.], 
                                ...
                                xi_FeS=FeS, 
                                X_impurity=im,
                                X_impurity_0_layers=[0., 0., im, im, 0], 
                                sweeps=10,
                                iteration_limit=25, 
                                impurity=9, 
                                inner_core_frac=.5, 
    ...
    print ('Converged:', int((1.-fail_count/N)*100), '%')
        
    return planets


def plot_literature_ranges():
    ranges = np.array([[.329, .341],
                       [.338, .338],
                       [.327, .342], 
                       [.317, .33],
                       [.317, .338],
    ...
    add_cols = [(.5, .5, .5), (.75, .4, .2), (0., 0., .75), (.75, .25, 0.), (.75, .75, .75)]
    ...
    rect = patches.Rectangle((0, .313), 1, .048, 
                             linewidth=2, 
                             edgecolor='none', 
                             facecolor=np.asarray(add_cols[1]) + np.array([.2, .3, .3]),
                             transform = trafo)
    ...
    for i in range(len(add_planets)):
        pl = add_planets[i]
        ax.plot([0, 1], [MoI_solar[pl], MoI_solar[pl]], 
                transform = trafo, color = add_cols[i])
        ax.text(.1, MoI_solar[pl], pl.capitalize(), va = 'bottom',
                color = add_cols[i])
    #ax.arrow(0, .35, 0, .01, head_length = 0.002, head_width = .1, fc = 'k', ec = 'k')
    #ax.text (.1, .35, 'more oxidized', rotation = 90)
    ax.text (.1, .315, 'Possible range (Margot et al. 2021)', color = add_cols[1])
    
    for i in range(len(ranges)):
        ax.plot([i, i], ranges[i], linewidth=4, color = 'k')
        
    ax.scatter([1], [.338], color = 'k', zorder = 10, marker = 'o')
    fig.savefig('/home/os18o068/Documents/PHD/Abbildungen/MoI_ranges.pdf', format='pdf', bbox_inches='tight')
    plt.close(fig)


def run_planet(res=0,
               solar_SFe = False,
               mass = 1.,
               iterationType = 0,
               deltaType = 0,
    ...
    pl = toolkit.model_hydro(match_mass=True, 
                            eps_r=eps_r, 
                            predictor_T='none', 
                            ocean = ocean, 
                            ...
                            sweeps = 10,
                            iteration_limit = 10, 
                            outer_core_mat_fracs = [1., 0., 0., 0.],
                            inner_core_mat_fracs = [1., 0.],
                            initial_predictor = 0,
                            deltas = deltas,
                            core_segregation_model = False,
                            inner_core_segregation_model = False)
...
def T_surf(S0, eps = 0., alpha = 0.):
    return (S0 * (1. - alpha) / (4. * sigmaSB * (1. - eps / 2.)))**(1./4.)


def S0_ev(t, S0):
    fac = 365 * 24 * 3600 * 1e6
    return S0 * (1. + 0.0 * np.sin(2 * np.pi * t / fac / 50))


def run_track(T_range = [600, 200.], N = 5, S0 = 1360., eps = 0., alpha = 0.,
              dT = 10.):
    planets1 = []
    times1 = [0.]
    planets2 = []
    ...
    while True:
        ...
        dE = dE_grav + dE_int
        dt =  2. * abs(dE / (planets2[i+1].luminosity + planets2[i].luminosity))
        dt = max(dt, 1.)
        times2.append(times2[i] + dt)
        
        #Compute new dT
        dT = min(abs(pl.luminosity / dLdT), abs((times2[i+1]-times2[i]) / (dt/dT))) * 2.
        #dT *= 0.5
        i += 1
    ...
    ax[0].plot(np.asarray(times2) / fac, [pl.luminosity * 1e-15 for pl in planets2],
                marker = 's', markerfacecolor = 'None', label = 'effective')
    ax[0].plot(np.asarray(times2) / fac, [pl.intrinsic_luminosity * 1e-15 for pl in planets2],
                marker = 's', markerfacecolor = 'None', label = 'intrinsic')
    ...
    ax[1].plot(np.asarray(times2) / fac, [pl.R_surface_is / r_earth for pl in planets2],
                marker = 's', markerfacecolor = 'None')
    ...
    ax[2].plot(np.asarray(times2) / fac, [(pl.luminosity / (4. * np.pi * sigmaSB * pl.R_surface_is**2))**(1./4.) for pl in planets2],
                marker = 's', markerfacecolor = 'None', label = 'effective')
    ax[2].plot(np.asarray(times2) / fac, [pl.T_surface_is for pl in planets2],
                marker = 's', markerfacecolor = 'None', label = 'intrinsic')
...
def run_sample(res=0, obj='earth', add = True, hyd = False, earth_like = False,
               solar_SFe = False, sulfur_poor = False, mass = None,
               ocean_scale = 'lin', 
               sample_mass = False, mass_scale = 'log', iterationType = 0,
               deltaType = 0, initial_predictor = 0, deltas = None,
               core_segregation_model = True, ocean = False, 
               mass_range = [.1, 5.], Fe_number_mantle_range = [0., 1.],
               ocean_mass_frac_range = [-3., np.log10(.5)],
               PS_range = [1e5, 1e5], T_TBL_range = None, Mg_range = None,
               Si_number_mantle = None, T_0_delta_T_core_range = [1200., 1600.],
               delta_T_MTZ_range = [0., 0.], save_grid = False,
               inner_core_segregation_model = False, P_CS_range = [30e9, 70e9],
               scale_P_CS = False, log_core_FeO = False, log_core_FeSi = False,
    ...
    if obj == 'earth':
        ...
        PS = 1e4
        Mg_range = [.47, .53]
        T_TBL_range = [1600., 1900.]
        
    else:
        if T_TBL_range == None:
            T_TBL_range = [1400., 2000.]
        if Mg_range == None:
            Mg_range = [.2, .8]
    
    if obj == 'earth':
        ranges = [Mg_range, #Mg# tot
                  P_CS_range, #P_CS
                  [0., .2], #xi_FeS
                  [.0, .05], #xi_FeSi
                  [.0, .05], #xi_FeO
        ...
        while True:
            for j in range(len(ranges)):
                ...
                if j == 3 or j == 4:
                    ...
                    inputs[i][j] = min(inputs[i][j], max(1. - sum(inputs[i][2:j]), 0.)) 
            ...
            if scale_P_CS:
                eta = 1.
                inputs[i][1] *= m * (1. - 10**inputs[i][8])**eta
            ...
            ocmf = [0., inputs[i][2], inputs[i][3], inputs[i][4]]
            #print ("\nocmf =", ocmf)
            ...
            #data anyways.
            if obj == 'earth':                
                simmin = max(.45, simmin)
                simmax = min(.55, simmax)

                femmin = max(.01, femmin)
                femmax = min(.2, femmax)
            ...
            if sim < simmin * (1. + margin) or sim > simmax * (1. - margin):#simmin*1.001 or sim > simmax*.99
                check = False
    ...
    for i in range(N):
        ...
        PS = 10**PS
        mg = inputs[i][0]
        ocmf = [1., inputs[i][2], inputs[i][3], inputs[i][4]]
        #print ("ocmf =", ocmf)
        icmf = [1.]
        ...
        pl = toolkit.model_hydro(P_center=Pc, 
                                ...
                                ocean = put_ocean[i], 
                                ocean_frac_should = of, 
                                temp_jumps=[0., T0 * (m*(1.-10**of))**.75, dtmtz, 0., 0.], 
...
def run_curve(obj = 'pure_iron',
    ...
    for i in range(N):
        m = masses[i]
        pl = toolkit.model_hydro(P_center=5e11, 
                                match_mass=True, 
                                eps_r=eps_r, 
                                predictor_T='none', 
                                ocean = False,
...
def extract_params(planets):
    ...
    while i < N-1:
        ...
        if not abs(xiMgO + xiFeO + xiSiO2 - 1.) < .0001:
        ...
        all_all_out[i][38] = 1. - xiSiO2 - xiFeO #xi_MgO mantle
        all_all_out[i][39] = pl.layer_properties[2]['R_outer']/1000
        all_all_out[i][40] = pl.fractions[1][1]/(1. - pl.fractions[1][0]+sum(pl.fractions[1]))
        ...
        all_all_out[i][47] = pl.layer_properties[0]['indigenous_mass']/m_earth/pl.M_core_is #inner core mass fraction
        ...
        try:
            ...
            all_all_out[i][56] = pl.masses[0] / sum(pl.masses) #liq. water mass frac. wrt. water
            ...
            all_all_out[i][50] = 0. #liq. water mass fraction wrt. tot. mass
            all_all_out[i][51] = 0.
            all_all_out[i][52] = 0.
            all_all_out[i][53] = 0.
            all_all_out[i][54] = 0.
            all_all_out[i][55] = 0.
            all_all_out[i][56] = 0. #liq. water mass frac. wrt. water
            all_all_out[i][57] = 0.
            all_all_out[i][58] = 0.
            all_all_out[i][59] = 0.
            all_all_out[i][60] = 0.
            ...
            all_all_out[i][60] = 0.
            all_all_out[i][61] = 0.
        all_all_out[i][62] = pl.M_surface_is/m_earth / (pl.R_surface_is/r_earth)**2 #Surface gravity
        try:
...
def calibrate(all_planets, obj='earth', plot=False, reldev_R = .01, reldev_C = .01,
              MoI_real = None, 
              add_matches = False, 
    ...
    while i < N-1:
        ...
        if obj == 'earth':
            ...
            if count_r == len(dr)-2 and abs(dev_R)<reldev_R and abs(dev_MoI)<reldev_C \
            and count_d == len(dd) and core_check:
                go = True

        else:
            if core_check and comp_check and abs(dev_R)<reldev_R and abs(dev_MoI)<reldev_C:
                go = True
    ...
    while i < N_match-1:
        ...
        #Compute mole fractions of oxides in the mantle
        xiFeO = (1.-pl.Si_number_mantle)
        xiFeO /= ((1.-pl.xi_Fe_mantle)/pl.xi_Fe_mantle + 1. - pl.Si_number_mantle)
        xiSiO2 = (1. - xiFeO)*pl.Si_number_mantle
        xiMgO = 1. - xiFeO - xiSiO2
    ...
    print (not_converged_count, 'not converged (', 
...
def plot_correlations(all_all_out, indices, obj = 'earth', spec = 'all'):
    ...
    scales = [1., 100.]
    axes_lims = [[[-6, 0], [2800., 4400.]], [[0., 15.], [2800., 4400.]]]
    ...
    for i in range(2):
        ...
        ax[i].tick_params(which='both', direction='out', right=True, top=True,
                          labelsize = fnts2, pad=10)
        ax[i].set_xlabel(x_labels[i], fontsize = fnts2)
        ax[i].set_ylabel(y_labels[i], fontsize = fnts2)
        sc = ax[i].scatter(all_out.T[param_indices[i][0]]*scales[i], 
        ...
        trans = transforms.blended_transform_factory(ax[i].transData, 
                                                     ax[i].transAxes)       
 
        rect = patches.Rectangle((x[i][0], 0), dx, 1., 
                                 linewidth=2, 
                                 edgecolor='none', 
                                 facecolor=(.75, .75, .75),
                                 transform = trans,
        ...
        ax[i].text(sum(x[i])/2., .8, r'Earth reference', transform = trans, 
                   ha = 'center', size = fnts2)
        
    inset_ax = inset_axes(ax[-1], height="40%", width="5%", loc=1,
                          bbox_to_anchor = (-0.1, 0, .9, .9), 
    ...
    fig.savefig(spec+'/'+obj+'_corrs_'+spec+'.png', format='png', 
...
def plot_models(all_all_out, planets, indices, y_real = None, obj = 'earth',
                reldev_x = .01, reldev_y = .01, plot_label = 'All models',
                spec = 'all', x_real = None, x_range = .1, y_range = .1,
                plot_rect = True, plot_cross = True, mass = None, xy = [1, 2],
                xy_scale = ['log', 'linear'], x_lims = [.1, 5.], 
                y_lims = [0.5, 1.75], plot_PT = False,
                profile_x_lims = [0., 1.], plot_PREM = True, plot_CMB = True,
                profile_y_lims = [[1000., 6000.], [0., 400.], [1., 14.]],
                profile_cbar_loc = 3, plot_real = True, colormap = 'viridis',
                colormap_arch = 'jet', real_filter = False, max_mass_err = .1):
    ...
    maxs = np.array([[.9, .6, 1.],
                     ...
                     [.5, .6, 1800],
                     [8000., 70, .4]])
    overview_params = [[0, 3, 27], 
                       [17, 48, 47], 
                       [8, 9, 10], 
                       [5, 18, 17],
                       [7, 6, 2]]
    param_scales = [['lin', 'lin', 'lin'],
                    ['lin', 'log', 'lin'],
                    ['lin', 'log', 'log',],
                    ['lin', 'lin', 'lin'],
                    ['lin', 'lin', 'lin']]
    
    cbar_labels = [[all_param_labels[overview_params[i][j]]
                   for j in range(len(overview_params[i]))] 
                   for i in range(len(overview_params))]
    
    fnts = 15
    
    fig, ax = plt.subplots(len(overview_params), len(overview_params[0]), 
                           figsize = (16, 20))
    ...
    for i in range(len(overview_params)):
        for j in range(len(overview_params[0])):
            ...
            if plot_cross:
                ...
                trafo = transforms.blended_transform_factory(ax[i][j].transData, 
                                                             ax[i][j].transAxes)
                ax[i][j].plot([x_real, x_real], [0., 1.], color='grey',
                              transform = trafo)
                
                trafo = transforms.blended_transform_factory(ax[i][j].transAxes, 
                                                             ax[i][j].transData)
                ax[i][j].plot([0., 1.], [y_real, y_real], color='grey',
                              transform = trafo)
            ...
            sc = ax[i][j].scatter(all_all_out.T[x],
                                  all_all_out.T[y], 
                                  c=c, 
                                  vmin=mins[i][j], 
                                  vmax=maxs[i][j],
            ...
            if plot_rect:
                ...
                x_rect = x_real - dx
                y_rect = y_real - dy
                rect = patches.Rectangle((x_rect, y_rect), 2*dx, 2*dy, 
                                         linewidth=2, 
                                         edgecolor='r', 
                                         facecolor='none')
            ...
            ftool.my_colorbar(sc, pad=.25, label = cbar_labels[i][j])
    ...
    c = all_all_out.T[48]
    ax3[0].scatter(all_all_out.T[x], all_all_out.T[y], c=c, vmin=0., 
                vmax=1., cmap=colormap, zorder = 0)
    sc = ax3[1].scatter(all_all_out.T[x], all_all_out.T[y], c=c, vmin=0., 
                vmax=1., cmap=colormap, zorder = 0)
    
    arch.plot(ax = ax3[0], cmap = 'jet', fnts1 = fnts_arch)
    #cbar_ax = fig3.add_axes([1, 0, 0.01, 1])
    inset_ax_cbar = ax3[1].inset_axes((1.1, 0., .05, 1))
    cbar = plt.colorbar(sc, cax = inset_ax_cbar)
    cbar.ax.set_ylabel(r'${\rm Water \ mass \ fraction} \ w_{\rm H_2O}$', 
                       fontsize = fnts_arch)
    ...
    cols = np.array([[0.5, 0., 0.], [0., 0.5, 0.], [.5, 0.5, 1.]])
    cols = np.array([[0., 0., 0.], [0.5, 0.25, 0.], [1., 0.5, 0.]])    
    cols = np.array([[0., 0., 0.], [0.5, 0.125, 0.], [1., 0.25, 0.]])
    cols = np.array([[0, 0, 0], [0, 0.25, .5], [0, 0.5, 1]])
    ...
    inset_ax = ax1[-1].inset_axes((.6, 0.5, .05, .4))
    plt.subplots_adjust(wspace=.3)
    cbar = plt.colorbar(sm, cax = inset_ax, ticks=[0., .5, 1.])
    
    ticklabels = np.array([all_out.T[n].min(), 
                           (all_out.T[n].min() + all_out.T[n].max())/2., 
                           all_out.T[n].max()]).round(3)
    ...
    try:
        for i in range(3):
            ax1[i].tick_params(which='both', direction='out', top='on', 
                               right='on', labelsize=fnts+3, pad=10)
    ...
    for i in range(N_match):
        ...
        for j in range(3):
            ...
            ax1[j].plot(pl.profiles[0] / r_earth, 
        ...
        #plot PT-profiles
        if plot_PT:
            col = ftool.extract_col_from_cmap(cmap, PT_value_range_cmap, 10**pl.ocean_frac_is)
            ax2.semilogy(pl.profiles[1], 
                         pl.profiles[2],
                         color = col)
            ax2.scatter(pl.layer_properties[3]['T_outer'],
                        pl.layer_properties[3]['P_outer'],
                        color = 'r')
            
            inset_ax = inset_axes(ax2, height="40%", width="5%", loc=4,
                                  borderpad=10)
            plt.subplots_adjust(wspace=.3)
            cbar = plt.colorbar(sm, cax = inset_ax, ticks=[0., .5, 1.])
    ...
    if plot_CMB:
        ...
        box_col = (.75, .75, .75)
        for j in range(3):
            minsx = all_out.T[15].min()/r_earth*1000.
            maxsx = all_out.T[15].max()/r_earth*1000.
            
            minsy = all_out.T[28+j].min()*profile_scales[j]
            maxsy = all_out.T[28+j].max()*profile_scales[j]
            
            trans1 = transforms.blended_transform_factory(ax1[j].transData, 
                                                         ax1[j].transAxes)
    
            trans2 = transforms.blended_transform_factory(ax1[j].transAxes, 
                                                         ax1[j].transData)
            
            rect1 = patches.Rectangle((minsx,0), width=maxsx-minsx, height=1, 
                                     transform=trans1, color=box_col,
                                     alpha=1., linestyle=None)
    
            rect2 = patches.Rectangle((0,minsy), width=1, height=maxsy-minsy, 
                                     transform=trans2, color=box_col,
                                     alpha=1., linestyle=None)
    ...
    if plot_PREM:
        ax1[2].plot(PREM_data[0]*1000/r_earth, PREM_data[1]/1000,
                    color=(.5, .5, .5), linestyle='--',
                    linewidth = 3)
            
        ax1[2].text(.6, 10, 'PREM', color=(.5, .5, .5), fontsize = fnts+4)
    ...
    fig1.savefig(spec+'/'+obj+'.png', format='png', 
                 bbox_inches='tight', dpi=240)
    ...
    if plot_PT:
        ...
        ax2.text(x + .02, y, r'$\rm Ocean-Manlte \ interphase$', va = 'center',
                 transform = trafo, fontsize = fnts)
        fig2.savefig(spec+'/'+obj+'_'+spec+'_PT.png', format='png', 
                 bbox_inches='tight', dpi=240)
    fig.savefig(spec+'/'+obj+'_calibration.png', format='png', bbox_inches='tight',
                dpi = 240)
    
    fig3.savefig(spec+'/'+obj+'_data_selection.png', format='png',
                 bbox_inches = 'tight', dpi = 240)
    

def check_statistics(data, N=10, red=2, obj='earth', write = False,
                     spec = 'all', N_all=None):
    ...
    if write:
        ...
        with open(path, 'w') as f:
            f.write(str(len(data))+' of '+ str(N_all)+\
                    ' planets ('+str(int(len(data)/N_all*100))+'%)\n')
            for i in range(len(params)):
...
def estimate_maximal_uncertainty(N=10, obj='earth', spec ='all'):
    ...
    for i in range(N):
        ...
        C = MoI_solar[obj]*(1. - delta0 + p1*2*delta0)
        deltaC = .02 + p2*.03
        
        pairs[i][0] = C
        pairs[i][1] = deltaC
        
        all_all_out, planets, indices = calibrate(obj = obj, 
                                                  plot = False, 
                                                  reldev_C = deltaC, 
                                                  MoI_real = C)
    ...
    for i in range(N):
        for row in range(N_row):
            for col in range(N_col):
                ...
                sc = ax[row][col].scatter(pairs[i][0], pairs[i][1],
                                 c = all_diffs[i][p],
                                 cmap = 'copper',
                                 vmin = np.nanmin(all_diffs.T[p]),
                                 vmax = np.nanmax(all_diffs.T[p]))
        
                ftool.my_colorbar(sc, 
                                  pad=.25, 
                                  label=colorbar_labels[row][col])

    print ('pairs = ', pairs)
    
    fig.savefig(spec+'/'+obj+'_stat.pdf', format='pdf', bbox_inches='tight')
    fig.savefig(spec+'/'+obj+'_stat.png', format='png', bbox_inches='tight', dpi = 240)
...
def plot_uncertainties(all_all_out, indices, obj='earth', res = 6, N = 10, spec = 'all'):    
    ...
    labels = [[r'$\sigma_j(\rm Mg\#) \ [\%]$',
    	r'$\sigma_j(\rm X_{\rm FeO}^{\rm Mantle}) \ [\%]$'], [
    	r'$\sigma_j(R_{\rm Core}) \ [\%]$',
    	r'$\sigma_j(R_{\rm ICB}) \ [\%]$'],
    ...
    for i in range(res):
        ...
        ranges, low, high, rmsd = check_statistics(data, 
                                                   N=N, 
                                                   red=red, 
                                                   write = True, 
                                                   obj = obj,
                                                   spec = spec,
                                                   N_all = len(all_all_out))
    ...
    for i in range(len(which_params)):
    	for j in range(len(which_params[i])):
            ...
            ax[i][j].plot(np.arange(res), 100*all_std.T[which_params[i][j]][0], \
                    marker='x', label = 'lower bound', color = 'k',
                    linestyle = bound_line_styles[0])
            ax[i][j].plot(np.arange(res), 100*all_std.T[which_params[i][j]][1], \
                    marker='x', label = 'upper bound', color = 'k',
    ...
    fig.savefig(spec+'/'+obj+'_conv_'+spec+'.pdf', format='pdf', bbox_inches='tight')
...
def fit_sample(all_all_out):
    ...
    fig, ax = plt.subplots()
    sc = ax.scatter(np.log10(masses), np.log10(radii), c = temps, cmap ='jet',
                    vmin = c1, vmax = c2)
    ...
    def fct(theta, M, T):
        a, b, c, d, e = theta
        return a + b * np.log10(M) + c * (T / 1000) + \
            d * np.log10(M) * (T / 1000)
...
def fit_grid(all_all_out, obj = 'earth', spec = 'all'): 
    ...
    im = ax.imshow(df.corr(), extent = [0., 1., 0., 1.], aspect = 'equal',
                   origin = 'lower')
    ftool.my_colorbar(im, pad=.25, label = 'correlation')
    ...
    for i in range(len(xyz)):
        ...
        sc = ax[i].scatter(x_data, 
                        y_data, 
                        c = z_data,
                        cmap = 'jet')
    
        ftool.my_colorbar(sc, pad=.25, label = all_param_labels[z])
