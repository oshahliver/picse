...
coeffs = [np.array([-7.19993352e-02,  8.20956584e+00, -3.46793056e+02,  6.48875298e+03,
       -4.50699821e+04]), 
        np.array([-1.01962812e+01,  1.26707689e+03, -5.90347936e+04,  1.22208342e+06,
       -9.48250541e+06]), 
        np.array([ -14.93208847, 1589.76095947]), 
            np.array([-8.71692725e-04,  1.19579527e-01, -5.57672252e+00,  8.55574995e+01,
        1.08400218e+03]), 
        np.array([-31.11306139, 252.46893265, 863.14809939])]

data = np.array([
                 [[19.38, 20.12, 22.27, 24.94, 27.1, 29.67, 31.01, 32.66, 33.65], 
                  [0., 214.8, 383.7, 535.9, 671.6, 794.5, 891.7, 1013., 1087.],
                  'Hermann 2016 (ab initio)'],
                 [[29.67, 31.28, 32.66, 33.45, 33.45, 30.41], 
                  [794.5, 618.8, 418.3, 196.7, 0., 725.2], 'Hermann 2016'],
                 [[27.23, 33.65, 39.97], [1198., 1087., 1008.], 'Hermann 2016'],
                 [[1.0, 1.5, 2., 3., 4., 4., 6., 6., 8., 8., 8., 10., 11.5, 
                   11.5, 13., 15., 15., 13., 11.5, 4.], 
                  [1056, 1204., 1258., 1349., 1374., 1393., 1414., 1441., 1457., 
                   1464., 1442., 1459., 1443., 1498., 1521., 1550., 1521.,
                   1497., 1392., 1293.], 'Johnson & Walker 1993 (measured)'],
                 [[1.67, 2.7, 3.29], [1198., 1318., 1357.], 
                  'Irving et al. 1977 (measured)'],
                 ])
...
def T_trans(P=None, i=0): 
    T =  [coeffs[i][c]*P**(len(coeffs[i])-c-1) for c in 
             range(len(coeffs[i]))]
...
def plot(pmax=60., log=False):
    
    pres = np.array([np.linspace(0., P_triple2*1.0e-9, 100),
                     np.linspace(P_triple1*1.0e-9, pmax, 100),
                     np.linspace(P_triple2*1.0e-9, pmax, 100),
                     np.linspace(0., P_triple2*1.0e-9, 100),
                     np.linspace(0., pmax, 100)])
    ...
    for d in range(len(data)):
        ...
        sc = ax.scatter(dat[0], dat[1], zorder=10, marker=markers[d], s=40,
                            facecolor = facecol, edgecolor=edgecol)
        
        if d < 4:
            ax.plot(pres[d], T_trans(P=pres[d], i=d), color='grey', linestyle='-',
                    linewidth=2)
