...
orderedLayers = ['inner core', 'outer core', 'mantle', 'solid', 'supercritical', 'liquid']
...
class PlanetOverviewChart():
    ...
    def plot_advanced(self, 
        ...
        if auto_shape:
            ...
            try: 
                ...
                print ("WARNING: For auto_shape = False the key argument 'shape' needs to be specified.")
    ...
    def plot_simple(self, showLegend = True, fnts = 16):
        ...
        for i in range(len(self.structures)):
            ...
            pc = PlanetChart(nLayers = len(self.structures[i]),
                             colors = cols)
            pc.draw(ax = ax[i])
            ax[i].text(0.2, -1,
                       alphas[i],
        ...
        if showLegend:
            ...
            for i in range(len(uniqueStructures)):
                ...
                axx.text(0.3,
                        ...
                        va = 'center',
                        fontsize = fnts)
                axx.scatter(0.2, yPos,
                            s = 50,
                            color = self.colors[orderedLayers[i]])
                axx.set_xlim(0., 1)
            axx.axis('off')
            
        loc = '/home/os18o068/Documents/PHD/Abbildungen/'
        fig.savefig(loc + 'hydro_structures.pdf',
                    format = 'pdf',
                    bbox_inches = 'tight')
        plt.close()
...
class PlanetChart():
    def __init__(self, data, radius = 1., label_offset = .0,
                 on_scale = True, d1 = .02, d2 = .05, **kwargs):
        self.radius = radius
        self.ghosts = []
        self.slices = []
        self.data = data
        ...
        #Use uniform layer sizes
            ...
            self.layerSizes = [self.radius * (a + 1) / self.nLayers for a in range(self.nLayers)]
    ...
    def get_ghosts(self, res = 5):
        self.ghosts = []
        for i in range(self.nLayers):
            dcolor = .5 * np.min(np.ma.masked_equal(np.array(self.colors[i]), 0))
            print ('dcolor =', dcolor)
            if not self.ghostSizes[i] == None:
                gh = PieGhost(r = self.ghostSizes[i],
                              res = res, 
                              phi1 = self.phi1,
                              phi2 = self.phi2,
                              zorder = 100,
                              color = [max(c - dcolor, 0.) for c in self.colors[i]],
                              offset = [0., 0])
    ...
    def draw(self, res = 5, title = 'Your \ Title', titleFontSize = 14, 
             **kwargs):
        try:
            self.ax = kwargs['ax']
        except KeyError:
    ...
    def get_label_positions(self, fac = 1.1):
        ...
        for i in range(self.nLayers):
            ...
            self.connection_lines[i] = [[self.all_raw_xPos[i], self.all_xPos[i] + self.d1 * np.cos(np.pi / 2 - self.phi1)], 
                                        [-1+self.all_raw_yPos[i], -1+self.all_yPos[i]  - self.d1 * np.sin(np.pi / 2 - self.phi1)]]
        
        #Add central connection line
        self.connection_lines[-1] = [[0, 0 + self.d1 * np.cos(np.pi / 2 - self.phi1)], 
        ...
        for i in range(self.nLayers - 1):
            ...
            if drel < .1:# or self.all_yPos[i] > self.all_yPos[i + 1]:
                print ("adjusting label {}".format(i))
                adjusted_indices.append(i + 1)
                self.all_yPos[i + 1] *= fac
                self.all_xPos[i + 1] *= fac

                self.connection_lines[i + 1] = [[self.all_raw_xPos[i + 1], self.all_xPos[i + 1]  + self.d1 * np.cos(np.pi / 2 - self.phi1)], 
                                        [-1 + self.all_raw_yPos[i + 1], -1+ self.all_yPos[i + 1]  - self.d1 * np.sin(np.pi / 2 - self.phi1)]]
    ...
    def add_labels(self, title = 'Structure Label', 
                labelFontSize = 12, 
                titleFontSize = 16,
                dataMult = {'P_outer':1e-9,
                            'T_outer':1e0,
                            'R_outer':1e-3,
                            'P_inner':1e-9,
                            'T_inner':1e0,
                            'R_inner':1e-3,
                            'indigenous_mass':1./m_earth,
                            'mass_fraction':1e2,
                            'x_MgO':1e2,
                            'x_FeO':1e2,
                            'x_SiO2':1e2,
        ...
        trafo = transforms.blended_transform_factory(self.ax.transData, 
                                                     self.ax.transData)
        ...
        for i in range(self.nLayers):
            ...
            labelRight = r'&&{a:g}-{b:g} \ \rm K \\ &&{c:g}-{d:g} \ \rm GPa'.format(
                a = ftool.fancyround(data[i]['T_outer'][0] * \
                                     dataMult['T_outer'], digits = 3),
                b = ftool.fancyround(data[i]['T_outer'][1] * \
                                     dataMult['T_outer'], digits = 3),
                c = ftool.fancyround(data[i]['P_outer'][0] * \
                                     dataMult['P_outer'], digits = 3),
                d = ftool.fancyround(data[i]['P_outer'][1] * \
                                     dataMult['P_outer'], digits = 3))
            
            
            labelLeft = r'${a:g}-{b:g} \ \rm km$'.format(
                a = ftool.fancyround(data[i]['R_outer'][0] * \
                                     dataMult['R_outer'], digits = 3),
                b = ftool.fancyround(data[i]['R_outer'][1] * \
                                     dataMult['R_outer'], digits = 3))
            ...
            if not middleKeys == None:
                ...
                for j in range(len(middleKeys[i])):
                    ...
                    a = ftool.fancyround(data[i][key][0] * \
                                         dataMult[key], 
                                         digits = 2),
                    b = ftool.fancyround(data[i][key][1] * dataMult[key], 
                                         digits = 2))
                    
                    if middleKeys[i][j] == 'logfO2_mantle':
                        partial_label_middle = r'$-\log(f_{O_2})= \ $' + partial_label_middle
            ...
            if not outer and i == self.nLayers - 1:
                ...
                self.ax.annotate(r"$\}$",
                             fontsize=24,
                            xy=(xPos + self.d1 * np.cos(np.pi / 2 - self.phi1), 
                ...
                self.ax.text(xPos + self.d2 * np.cos(np.pi / 2 - self.phi1),
                ...
                self.ax.text(-(xPos + self.d1 * np.cos(np.pi / 2 - self.phi1)), 
                                yPos - self.d1 * np.sin(np.pi / 2 - self.phi1), 
                             labelLeft, 
                             #rotation = rotAngle,
                             zorder = 100,
            ...
            self.ax.text(0,
                         -1 + (self.all_yPos_center[i+1] + self.all_yPos_center[i]) /\
                             2,
                         labelMiddle,
                         zorder = 100,
                        va = 'center',
            ...
            if i == 0:
                ...
                a = ftool.fancyround(data[i]['T_inner'][0] * \
                                     dataMult['T_inner'], digits = 3),
                b = ftool.fancyround(data[i]['T_inner'][1] * \
                                     dataMult['T_inner'], digits = 3),
                c = ftool.fancyround(data[i]['P_inner'][0] * \
                                     dataMult['P_inner'], digits = 3),
                d = ftool.fancyround(data[i]['P_inner'][1] * \
                                     dataMult['P_inner'], digits = 3))
                ...
                self.ax.annotate(r"$\}$",
                         fontsize=24,
                        xy=(x_pos_center + self.d1 * np.cos(np.pi / 2 - self.phi1), 
                ...
                self.ax.text(x_pos_center + self.d2 * np.cos(np.pi / 2 - self.phi1),
        ...
        for i in range(self.nLayers + 1):
            ...
            if not self.connection_lines[i] == None:
                # Right side
                self.ax.plot(self.connection_lines[i][0],
                             self.connection_lines[i][1],
                             color = 'k')
                # Left side
                if i < self.nLayers:
                    self.ax.plot([-cl for cl in self.connection_lines[i][0]],
                                     self.connection_lines[i][1],
                                     color = 'k')
    ...
    def addData(self, data, 
        ...
        data_labels = {"layerTemperatures":'K',
                       "layerPressures":'GPa',
                       "layerSizes":'km',
                       "layerMasses":'wt\%',
                       "S_core": r'$\rm \ wt\% \ S$',
        ...
        for i in range(self.nLayers):
            ...
            right = r'\end{eqnarray*} '
            labelRight = r'&&{a:g}-{b:g} \ \rm K \\ &&{c:g}-{d:g} \ \rm GPa'.format(
                a = ftool.fancyround(data['layerTemperatures'][i][0] * \
                                     dataMult['layerTemperatures'], digits = 3),
                b = ftool.fancyround(data['layerTemperatures'][i][1] * \
                                     dataMult['layerTemperatures'], digits = 3),
                c = ftool.fancyround(data['layerPressures'][i][0] * \
                                     dataMult['layerPressures'], digits = 3),
                d = ftool.fancyround(data['layerPressures'][i][1] * \
                                     dataMult['layerPressures'], digits = 3))
            
            
            labelLeft = r'${a:g}-{b:g} \ \rm km$'.format(
                a = ftool.fancyround(data['layerSizes'][i][0] * \
                                     dataMult['layerSizes'], digits = 3),
                b = ftool.fancyround(data['layerSizes'][i][1] * \
                                     dataMult['layerSizes'], digits = 3))
            '''
            ...
            if not middleKeys == None:
                ...
                for j in range(len(middleKeys[i])):
                    ...
                    partial_label_middle = r'${a:g}-{b:g}$'.format(    
                    a = ftool.fancyround(dat[0] * dataMult[middleKeys[i][j]], 
                                         digits = 2),
                    b = ftool.fancyround(dat[1] * dataMult[middleKeys[i][j]], 
                                         digits = 2))
            ...
            #Add temperature and pressure ranges at the right
            self.ax.text(xPos + d1 * np.cos(np.pi / 2 - self.phi1),
            ...
            if i >= 0 and not central_data == None:
                #Add layer mass fraction in the center of each layer
                self.ax.text(0,
                             -1 + (all_yPos_center[i+1] + all_yPos_center[i]) /\
                                 2,
                             labelMiddle,
                             zorder = 100,
                            va = 'center',
            ...
            if i == 0:
                #Add temperature and pressure ranges at the right
                label = r'&&{a:g}-{b:g} \ \rm K \\ &&{c:g}-{d:g} \ \rm GPa'.format(
                a = ftool.fancyround(central_data['Temperature'][0] * \
                                     dataMult['layerTemperatures'], digits = 3),
                b = ftool.fancyround(central_data['Temperature'][1] * \
                                     dataMult['layerTemperatures'], digits = 3),
                c = ftool.fancyround(central_data['Pressure'][0] * \
                                     dataMult['layerPressures'], digits = 3),
                d = ftool.fancyround(central_data['Pressure'][1] * \
                                     dataMult['layerPressures'], digits = 3))
                ...
                self.ax.text(d1 * np.cos(np.pi / 2 - self.phi1),
...
class PieGhost():
    def __init__(self, r = 1., res = 5, zorder = 0, color = 'b',
                 offset = [0, 0], **kwargs):
        try:
            self.phi1 = kwargs['phi1']
        except KeyError:
    ...
    def draw(self, axis, loc = [0, 0], orientation = 'normal',
             zorder = 0):
       
        #Draw cirle segment
        axis.plot(self.x, self.y - 1.,
                  color = self.color,
                  linestyle = '--',
                  zorder = self.zorder)
...
class PieSlice():
    def __init__(self, r = 1., res = 5, zorder = 0, color = 'b', offset = [0, 0], **kwargs):
        try:
            self.phi1 = kwargs['phi1']
        except KeyError:
            self.phi1 = np.pi * 1 / 3
        ...
        #Create triangle
        self.polygon = Polygon([(self.x[-1] + self.offset[0], -1 +self.dy), 
                                (0 + self.offset[0], -1), 
