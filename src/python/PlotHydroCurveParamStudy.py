...
suff = 'Fe_0_double_check'

file_dir = '/home/os18o068/Documents/PHD/Abbildungen/Figures_paper_1/'+suff+'/'
data_dir = '/home/os18o068/Documents/PHD/Projects/Planets/Code/'
filename = 'test'
format = 'pdf'

delta_R_labels =  ['Type-1/2', 'Type-2/3']
delta_R_label_pos = [[2., .06], [2., 800.]]
pures = [1, 2, 4]
...
data_hyd = np.load(data_dir+'planet_output_hyd_'+suff+'.npy')
data_dry = np.load(data_dir+'planet_output_dry_'+suff+'.npy')
data_oce = np.load(data_dir+'planet_output_oce_'+suff+'.npy')

#also load nominal data for boundary cases
data_hyd_nom = np.load(data_dir+'planet_output_hyd_nom_double_check.npy')
data_dry_nom = np.load(data_dir+'planet_output_dry_nom_double_check.npy')
data_oce_nom = np.load(data_dir+'planet_output_oce_nom_double_check.npy')

type_names = ['Type-1', 'Type-2', 'Type-3']

acc_require = np.array([1.0e-3, 5.0e-3, 1.0e-2, 6.0e-2, 5.0e-2, 1.0e-2])

data = [data_dry, data_hyd, data_oce]
data_nom = [data_dry_nom, data_hyd_nom, data_oce_nom]
        ...
        for i in range(len(data[l][0][0])):
            ...
            for j in range(len(data[l][0][0][0])):
                ...
                for p in range(len(data[l][0][s][i][j])):
                    ...
                    for d in range(len(pl)):
                        pass
                        #data_nom[l][d][s][1][0][14] = data[l][d][s][1][0][14]

                        #data[l][d][s][1][j][0:14] = data_nom[l][d][s][1][j][0:14]
...
'''
data_nom[l][1][s][1][0][14] = data[l][1][s][1][0][14]*.996
data_nom[l][0][s][1][0][14] = data[l][0][s][1][0][14]
data_nom[l][8][s][1][0][14] = data[l][8][s][1][0][14]
data_nom[l][13][s][1][0][14] = 70000/r_earth
'''
...
for s in range(len(data_hyd[0])):
    ...
    for k in range(len(data)):
        ...
        if k == 2:
            ...
            aa[0][0].text(1.5, 100., 'Ocean Properties', ha='center',
                          bbox=bbox_props)
    ...
    ax[0][0].text(1., 1040, 'Hydrogen in the Core', ha='center',
                  bbox=bbox_props)
        ...
        for j in range(len(data[1][0][0][0])):
            ...
            for k in range(len(data)):
                ...
                for p in range(len(pures)):
                    axes[k][i][2].semilogx(pure_curves[p]['col0']/m_earth, 
                                       pure_curves[p]['col1']/r_earth,
                                       color=pure_curves_color_list[pures[p]-1], 
                                       linewidth = 1.5)
                    
                    axes[k][i][2].text(pure_positions[p][0], pure_positions[p][1], 
    ...
    for k in range(len(data)):
        ...
        figs[k].align_labels()
        figs[k].savefig(file_dir+'internals_type_'+str(k+1)+'_'+suff+'.'+format,
                        bbox_inches='tight')
        
        if k == 2:
            ffigs[0].align_labels()
            ffigs[0].savefig(file_dir+'ocean_props'+'_'+suff+'.'+format,
                             bbox_inches='tight')
...
for l in range(2):
    ...
    for s in range(len(data_hyd[0])):
        ...
        ax[0][0].text(delta_R_label_pos[l][0], delta_R_label_pos[l][1], 
                      delta_R_labels[l], ha='center',
                      bbox=bbox_props)
        ...
        for i in range(len(data[l+1][0][0])):
            ...
            for j in range(len(data[l+1][0][0][0])):
                ...
                if l == 0:
                    ...
                    ax[i][0].semilogx(data_nom[l+1][0][s][-i][-j], 
                                      data_nom[l+1][8][s][-i][-j],
                                      color=cc, linewidth = lwdthnom)
                    ...
                    ax[i][1].semilogx(data_nom[l+1][0][s][-i][-j], 
                                             data_nom[l+1][10][s][-i][-j],
                                             color=cc, linestyle='--', 
                                             linewidth = lwdthnom)
                    ...
                    ax[i][0].semilogx(data_nom[l+1][0][s][-i][-j], 
                                      data_nom[l+1][13][s][-i][-j]*r_earth/1000.,
                                  color=cc, linewidth = lwdthnom)
                ...
                ax[i][2].semilogx(data_nom[l+1][0][s][-i][-j], 
                               (data_nom[l+1][1][s][-i][-j]-\
                                data_nom[l][1][s][-i][-j])/data_nom[l][1][s][-i][-j],
                                   color=cc, linewidth = lwdthnom)
        ...
        fig.align_labels()
        fig.savefig(file_dir+'delta_R'+str(l+1)+'_'+suff+'.'+format,
                    bbox_inches='tight')
