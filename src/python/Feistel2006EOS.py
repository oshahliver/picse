...
def r2(P):
    pi0 = np.complex(P0/P_triple, 0.)
    pi = np.complex(P/P_triple, 0.)
...
def r2PP(P):
    return r2_list[2]*2./P_triple**2
    

def complex_number(p, t):
    tau = t/T_triple
    r_list = [r1, r2(p)]
    return sum([r_list[k]*((t_list[k] - tau)*np.log(t_list[k] - tau) \
                + (t_list[k] + tau)*np.log(t_list[k] + tau) - 2.* t_list[k]\
                *np.log(t_list[k]) - tau**2/t_list[k])
...
def gT(P=None, T=None):
    ...
    cc = np.complex(0., 0.)
    for k in range(2):
        cc += r_list[k]*(-np.log(t_list[k]-tau)+np.log(t_list[k]+tau)-\
                    2.*tau/t_list[k])
...
def gTT(T=None, P=None):
    ...
    cc = np.complex(0., 0.)
    for k in range(2):
        cc += r_list[k]*(1./(t_list[k]-tau)+1./(t_list[k]+tau)-2./t_list[k])
...
def gP(T=None, P=None):
    ...
    t2 = t_list[1]
    #compute complex number
    cc = r2P(P)*((t2-tau)*np.log(t2-tau)+(t2+tau)*\
            np.log(t2+tau)-2*t2*np.log(t2)-tau**2/t2)
...
def gPP(T=None, P=None):
    ...
    t2 = t_list[1]
    #compute complex number
    cc = r2PP(P)*((t2-tau)*np.log(t2-tau)+(t2+tau)*\
            np.log(t2+tau)-2*t2*np.log(t2)-tau**2/t2)
...
def pressure(d = None, T = None, **kwargs):
    temp = T
    return ftool.bisec(a = P_min_bisec, b = P_max_bisec, y = d, whicharg = 'P',
                       f = density, T = temp, identity = 'Feistel pres', 
                       **kwargs)
...
def temperature(d = None, p = None, **kwargs):
    pres = p
    return ftool.bisec(a = T_min_bisec, b = T_max_bisec, y = d, whicharg = 'T', 
                       f = density, P = pres, identity = 'Feistel temp', 
                       **kwargs)
