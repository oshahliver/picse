...
class LinearSystem():
    def __init__(self, params, data, target = None, param_weights = None,
                 data_weights = None):
        print ('params =', params)
    ...
    def row(self, iter_list = []):
        ...
        for_recursive(f=self.partial_row, 
                           ranges=iter_ranges,
                           ind=count)
    ...
    def construct_target_vector(self):
        self.target_vector = np.zeros([len(self.params[0]),
                                       2**(len(self.params[0]))])
        
        self.count = 0
        iter_ranges = [range(0,2) for i in range(len(self.params[0]))]
        for_recursive(f=self.target_row,
                      ranges = iter_ranges)
        self.count += 1
