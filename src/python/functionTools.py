...
def my_cmap(res=3, start=0, reverse=False):
    ...
    for i in range(len(border_colors) - 1 - start):
        ...
        for r in range(n_additional_bins):
            colors.append(np.asarray(border_colors[i + start]) +\
                        (r + 1) / (n_additional_bins + 1) * (np.asarray(border_colors[i+1+start]) - \
...
def my_dev_cmap(N_col=81, 
                border_colors = np.array([color_list[2], 
                                          [1,1,1], 
                                          color_list[0]])):
    ...
    for i in range(N_col-1):
        for j in range(3):
            ...
            grad = np.array(2*(border_colors[ind[1]][j] - \
                             border_colors[ind[0]][j])/(N_col-1))
...
def my_colorbar(mappable, size="5%", pad =.1, label = 'a colorbar', fnts = 12):
    ax = mappable.axes
    fig = ax.figure
...
def extract_value_range(data = None, scalings = None, hard_mins = None,
                        hard_maxs = None):
    """
    ...
    if scalings == None:
        scalings = [1. for i in range(N_params)]
...
def param_to_label(params = [], types = [], labels = [],
                   rounds = None):
    """
    ...
    for i in range(2):
        for j in range(2):
            for k in range(3):
                ...
                data_out[count][6] = (data_in[8][0][i][j][k] - data_in[10][0][i][j][k]) #MH2O, Mantle
...
def sectomin(sec=None, digits=1, ms=True):
    ...
    hours = int(sec/3600 + .5)
    
    #Compute remainding seconds
    seconds = sec - hours*3600.
...
def printTime(sec=None, digits=1, ms = True, where=''):
    hours, minutes, seconds, miliseconds = sectomin(sec=sec, digits=digits,
                                                    ms=ms)
    ...
    if ms:
        print ('Elapsed time in', where, ':', hours, 'h', minutes,'min', 
           seconds,'sec', miliseconds,'ms')
...
def fancyround(num, digits=2):
    ...
    if not abs(num) < 1e-10 and not np.isnan(num):
        try:
            log = np.log10(num)
            exponent = int(log + .5 * (log / abs(log) - 1.))
    ...
    try:
        if missingarg == whicharg:
            ...
            if noisy:
                ...
                print ('      missingarg =', missingarg,', given key =', \
...
def bisec(*args, a = 0., b = 0., y = 0., f = None, limit = 100, 
          whicharg = '', noisy = False, identity = '', plot = False,
          axis = None, acc=1.0e-8, brash=False, log=False, **kwargs):
   ...
   #     acc=1.0e-5 
    ...
    if fc == y:
        ...
        while abs(fc - y)/y > acc:
            ...
            if counter > limit:
                if noisy or brash:
                    print ('WARNING: bisection step limit exceeded in <',identity,
                       '> after', limit,'steps !')
                break
    return c
    
def deriv(*args, x0 = 0., f = None, whicharg = 'x', noisy = False, 
          identity = '', plot = False, axis = None, brash = False, 
          rr = 10, type = 'tangent', order = 4, log='', pnts=30, \
          acc=1.0e-6, **kwargs):
    ...
    if type == 'poly':
        ...
        if plot:
            ...
            axis.set_xlabel('x'); axis.set_ylabel('f(x)')
            scatter_x = []
            scatter_y = []
        ...
        if noisy or brash:
            time_delta = time.time() - time_init
            print ('iteration terminated after',count,'steps with exit code', \
                   exit_code)
            print ('elapsed time:', round(time_delta*1.0e3, 3),'ms')
...
def integrate(start = 0., end = None, y = [], h = None, N = 1, dydx = None,
              order = 4, plot = False, whicharg = None, noisy = False,
              brash = False, identity = 'non passed', axis = None, eps=.5, 
              oldgrad=None, **kwargs):
    ...
    if whicharg == None:
        print ('WARNING: keyword argument "whicharg" has not been passed to',
               'functionTools.integrate()!')
        print('This will most likely lead to a KeyError or TypeError in the',
    ...
    for hh in range(N):
        ...
        for i in range(order):
            ...
            try:
                ...
                newgrads = None
                print ('ERROR: missing key in function <integrate>; id =', \
                       identity)
                print ('kwargs =', kwargs)
    ...
    if plot:
        ...
        for i in range(len(y_list)):
            axis[i].plot(x_list, y_list[i], marker = 'o', linestyle = '-', \
...
def interpolate_deriv(x=None, y=None, x_list=None, y_list=None, 
                      z_list=None, derivatives_x=None, derivatives_y=None,
                      derivatives_xy=None, order=3, **kwargs):
    """
    """
...
def interpolate_deriv1d(grid=[], data=[], order=2, plot=False, which='left',
                        **kwargs):
    x_list=grid
