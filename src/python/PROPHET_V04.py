...
def intersection_angle(X=None, Y=None):
    ...
    """
    return np.arccos(scalar_product(X=X, Y=Y)/(magnitude(X)*magnitude(Y)))
...
def PropagatePizza(data=None, rs=None, n_pizza=None, intersect_limit=None,
                   ...
                   signsign = None):
    """Checks if ray crosses radial boundary between two subsequent intersection
    ...
    while node_iteration:
        ...
        print ('\ngrid indices =', grid_indices1, grid_indices2)
        ...
        rs[-1] or grid_indices2[1] >= len(rs) or k >= intersect_limit :
            node_iteration = False
            data[a].append(polar2[1]*np.array([np.cos(polar2[0]), np.sin(polar2[0])]))
            
        else:
            
            #Extract theta value of grid cell of first point
            ...
            if not np.isnan(node[0]):
                ...
                iap1 = intersection_angle_pizza(Q=vec_a, P=vec_b)
                iap2 = np.arcsin(.75 * np.sin(iap1))
                print ('iap1 =', iap1/np.pi*180)
                print ('iap2 =', iap2/np.pi*180)
                print ('pola1_dummy =', polar1_dummy)
                print ('pola1 =', polar1)
                alpha = -(np.pi/2 - abs(iap2))*iap2/abs(iap2)
                radius_index = grid_indices1[1]-int(.5*(1.+iap2/abs(iap2)))
                print ('alpha =', alpha/np.pi*180)
                print ('radius index =', radius_index)
                print ('add thing =', .5*(1.+iap2/abs(iap2)))
        ...
        if abs(alpha) >= alpha_max:
            ...
            for i in range(len(rs)):
                ...
                if np.isnan(Q2[0]) or np.isnan(Q2[1]):
                    ...
                    nodes, data = PropagatePizza(data=data, 
                                           rs=rs, 
                                           n_pizza=n_pizza, 
