...
from PIMPphysicalparams import T0_list, K0_list, K0prime_list, rho0_list, \
                                aP_list, molar_mass_list, Rgas, mH, kB, \
                                EOS_type, material_list, EOSstring_list, NA, \
                                gas_molar_mass_list, gas_EOS_type,\
                                a_VDW_list, b_VDW_list, \
                                K0prime_list_Mg2SiO4, \
                                phase_list_Mg2SiO4, material_plot_list,\
                                K0_list_Brucite, K0prime_list_Brucite,\
                                rho0_list_Brucite, phase_list_Brucite,\
                                q_list, n_list,\
                                thetaD0_list, gamma0_list, dq_pv, dK0_pv, \
                                dK0prime_pv, dgamma0_pv, dthetaD0_pv, dd0_pv,\
                                dq_ppv, dK0_ppv, \
                                dK0prime_ppv, dgamma0_ppv, dthetaD0_ppv, dd0_ppv,\
                                aT_list, bT_list, cT_list, aP_list_Mg2SiO4,\
                                aT_list_Mg2SiO4, bT_list_Mg2SiO4, cT_list_Mg2SiO4,\
                                daTdH2O_list_Mg2SiO4, dbTdH2O_list_Mg2SiO4,\
                                EOSstring_list
...
def mix1(densities, fractions, **kwargs):
    return 1./sum([fractions[i]/densities[i] for i in range(len(fractions))])


#Birch-Murnaghan (BM)
def P_BM(eta = 0, K = 0, K0prime = 0, **kwargs):
    return 3./2. * K * (eta**(7. / 3.) - eta**(5. / 3.)) * (1. + 3. / 4. * \
                       (K0prime - 4.) * (eta**(2. / 3.) - 1.))

def rho_BM(ll=0, d0=0, T=0, P=0, dmin=None, dmax=None, T0=300,
           aT=None, bT=None, cT=None, **kwargs):
    d0 = rho0_list[ll]
    K0 = K0_list[ll]
    ...
    d = d0T*ftool.bisec(f = P_BM, whicharg = 'eta', K = K, \
                K0prime = K0prime, a = dmin/d0T, b = dmax/d0T,  \
                y = P, identity='rho_BM()',  **kwargs)
    
    return d


def dPdrho_BM(ll=0, eta=0, K=0, K0prime=0, d0T = 0, **kwargs):
    return 3./2.*K/d0T*((7./3.*eta**(4./3.)-5./3.*eta**(2./3.))*(1.-3./4.*\
        (4.-K0prime)*(eta**(2./3.)-1.)) - 1./2.*(eta**(7./3.)- \
        eta**(5./3.))*(4.-K0prime)*eta**(-1./3.))


#Mie-Grüneisen-Debye (MGD)
def dP_MGD(eta = 0, ll = 5, T = 300, gamma0=None, thetaD0=None, rho0=None, 
           q=None, molmass=None, nparticles=None, **kwargs):
    d = eta*rho0
    ...
    if tempType == 'isothermal':
        ...
        E2 = PIMPtools.Eth(d, 300., thetaD0, rho0, gamma0, q, molmass, nparticles, ll)
        deltaP = (E1 - E2) * PIMPtools.gamma(q, gamma0, rho0, d)
        
    return deltaP


def P_MGD(eta = 0., K0 = 0., K0prime = 0., ll = 5, T = 300,  
          gamma0=None, thetaD0=None, rho0=None, q=None, molmass=None,
          nparticles=None, **kwargs):
    d = eta*rho0
    tempType = ftool.checkKey(name='tempType', **kwargs)
    ...
    P = 3./2. * K0 * (eta**(7. / 3.) - eta**(5. / 3.)) * (1. + 3. / 4. \
                             * (K0prime - 4.) * (eta**(2. / 3.) - 1.))
    print ('P =', P*1e-5)
    print ('delta P =', deltaP*1e-5)
    return P + deltaP

def rho_MGD(eta = 0., K0=0, K0prime=0., ll=5, T=300, gamma0=None, thetaD0=None,
            rho0=None, q=None, molmass=None, P=0., 
            nparticles=None, dmin=None, dmax=None, **kwargs):
    
    rho = rho0*ftool.bisec(f = P_MGD, whicharg = 'eta', K0 = K0, \
                                K0prime = K0prime, ll = ll, T = T, y = P,\
                                a = dmin/rho0, b = dmax/rho0, q=q, molmass=molmass,
                          nparticles=nparticles, gamma0=gamma0, rho0=rho0,
                          thetaD0=thetaD0, **kwargs)    
    return rho


def dPdrho_MGD(eta = 0, K0 = 0, K0prime = 0, ll=0, T=300, gamma0=None,
               thetaD0=None, rho0=None, q=None, molmass=None, nparticles=None, 
               **kwargs):
    d0 = rho0

    ddeltaPth = ftool.deriv(f=dP_MGD, x0=eta, whicharg='eta', T=T, 
                            gamma0=gamma0, thetaD0=thetaD0, rho0=rho0,
                            q=q, molmass=molmass, nparticles=nparticles, 
                            **kwargs)
    return 3./2.*K0/d0*((7./3.*eta**(4./3.)-5./3.*eta**(2./3.))*(1.-3./4.*\
                (4.-K0prime)*(eta**(2./3.)-1.)) - 1./2.*(eta**(7./3.)-\
                eta**(5./3.))*(4.-K0prime)*eta**(-1./3.))+\
                ddeltaPth
...
def P_Vinet(eta = 0, K0 = 0, K0prime = 0):
        return 3.*K0*eta**(2./3.)*(1.-eta**(-1./3.))*np.exp(3./2.*\
                       (K0prime-1.)*(1.-eta**(-1./3.)))


def dPdrho_Vinet(eta = 0, K0 = 0, K0prime = 0, ll=0):
    d0 = rho0_list[ll]
    return K0/d0*np.exp(3./2.*(K0prime-1.)*(1.-eta**(-1./3.)))*\
            (2.*eta**(-1./3.)*(1.-eta**(-1./3.)) + eta**(-2./3.)* \
             3./2. * eta**(-2./3.)*(1.-eta**(-1./3.))*(K0prime-1.))

            
#Belonoshko (Bel)       
def P_Bel(eta = 0, K0 = 0, K0prime = 0, ll = 0, T = 300, 
          q=None, gamma0=None, rho0=None, molmass=None, **kwargs):
    d = eta*rho0
    tempType = ftool.checkKey(name='tempType', **kwargs)
    ...
    if tempType == 'isothermal':
        ...
        deltaP = 3. * Rgas * PIMPtools.gamma(q, gamma0, rho0, d) * \
            (T - T0_list[ll]) * (d/molmass)
        
    return 3./2. * K0 * (eta**(7./3.) - eta**(5./3.)) * (1. + 3./4. * \
                        (K0prime - 4.) * (eta**(2./3.)-1.)) + deltaP
                     

def rho_Bel(rho0=None, T=None, P=None, K0=None, K0prime=None,
            ll=9, q=None, molmass=None, gamma0=None, dmin=None, dmax=None,
            **kwargs):
    rho = rho0*ftool.bisec(f = P_Bel, whicharg = 'eta', K0 = K0, \
                                K0prime = K0prime, a = dmin/rho0, b = dmax/rho0,
                                y = P, ll = ll, T = T, q=q, molmass=molmass,
                                gamma0=gamma0, rho0=rho0, **kwargs)
    
    return rho

                       
def dPdrho_Bel(eta = 0, K0 = 0, K0prime = 0, ll = 0, T = 300, 
               q=None, gamma0=None, rho0=None, molmass=None, **kwargs):
    d0 = rho0
    d = eta*d0
    tempType = ftool.checkKey(name='tempType', **kwargs)
    if tempType == 'isothermal':
        ...
        delta = 3.*Rgas*PIMPtools.gamma(q, gamma0, rho0, d)*(T-T0_list[ll])/molmass
        
    return 3./2.*K0/d0*((7./3.*eta**(4./3.)-5./3.*eta**(2./3.))*(1.-3./4.*
                        (4.-K0prime)*(eta**(2./3.)-1.)) - 1./2.*(eta**(7./3.)-
                        eta**(5./3.))*.5*(4.-K0prime)*eta**(-1./3.)) + delta

                                                                 
#Vinet-Rydberg (VR)
def P_VR(eta = 0., K0 = 0., K0prime = 0., ll = 5, T = 300,  
          gamma0=None, theta0=None, rho0=None, q=None, molmass=None,
           **kwargs):
    ...
    d = eta*rho0
    #print ('d =', d)
    P0 = 3.*K0*(eta**(2./3.) -  eta**(1./3.)) * np.exp((3./2.*K0prime-3./2.)*\
                                                        (1.-eta**(-1.3)))
        
    theta = theta0 * np.exp(gamma0/q*(1.-eta**(-q)))
    
    print ('theta =', theta)
    print ('P0 =', P0)
    Pth = 3.*Rgas*(theta/(np.exp(theta/T)-1.))
    ...
    Pe = 3./2.*n_list[ll]*Rgas*e*g*T**2*d/molmass
    print ('Pe =', Pe)
    return P0 + Pth + Pe
                                                                 

def rho_VR(rho0=None, T=None, P=None, K0=None, K0prime=None,
            ll=9, q=None, molmass=None, gamma0=None, dmin=None, dmax=None,
            **kwargs):
    rho = rho0*ftool.bisec(f = P_VR, whicharg = 'eta', K0 = K0, \
                                K0prime = K0prime, a = dmin/rho0, b = dmax/rho0,
                                y = P, ll = ll, T = T, q=q, molmass=molmass,
                                gamma0=gamma0, rho0=rho0, **kwargs)
...
def dPdrho_VR(eta = 0., K0 = 0., K0prime = 0., ll = 5, T = 300,  
          gamma0=None, theta0=None, rho0=None, q=None, molmass=None,
           **kwargs):

    e = np.exp((3./2.*K0prime-3./2.)*(1.-eta**(-1./3.)))
    dP0drho = 3.*K0*(2./3.*eta**(-1./3.)-1./3.*eta**(-2./3.))
    dP0drho *= e
    dP0drho += 3.*K0*(eta**(2/3.)-eta**(-1./3.))*(-1./3.*eta**(-4./3.))*\
        (3/2.*K0prime-3./2.)*e
    dP0drho = dP0drho/rho0

    dPthdrho = 3.*n_list[ll]*Rgas*molmass*gamma0*theta0/(np.exp(theta0/T)-1.)
    dPthdrho *= eta**(-q)/rho0*(1.-q)
    dPthdrho = dPthdrho/rho0
    
    return dP0drho + dPthdrho

#Ichikawa
def P_Ichi(eta = 0., K0 = 0., K0prime = 0., ll = 5, T = 300,  
          gamma0=None, thetaD0=None, rho0=None, q=None, molmass=None,
           **kwargs):
    ...
    gamma = gamma0*(1.+a*(eta**(-b)-1.))
    Eth = 3.*n_list[ll]*Rgas*(T+e0*eta**(-g)*T**2)
    Eth0 = 3.*n_list[ll]*Rgas*(T0_list[ll]+e0*eta**(-g)*T0_list[ll]**2)
...
def rho_Ichi(rho0=None, T=None, P=None, K0=None, K0prime=None,
            ll=9, q=None, molmass=None, gamma0=None, dmin=None, dmax=None,
            **kwargs):

    rho = rho0*ftool.bisec(f = P_Ichi, whicharg = 'eta', K0 = K0, \
                                K0prime = K0prime, a = dmin/rho0, b = dmax/rho0,
                                y = P, ll = ll, T = T, q=q, molmass=molmass,
                                gamma0=gamma0, rho0=rho0, **kwargs)
...
def dPdrho_Ichi(eta = 0., K0 = 0., K0prime = 0., ll = 5, T = 300,  
          gamma0=None, theta0=None, rho0=None, q=None, molmass=None,
           **kwargs):
...
def P_VDW(d=0., T=0., ll=None, **kwargs):
...
def rho_VDW(P=0., T=0., ll=None, **kwargs):
    return ftool.bisec(f=P_VDW, whicharg='d', T=T, a=1.0e-8, b=10., 
                       ll=ll, y=P, **kwargs)
    
    
def dPdrho_VDW(d=0., T=0., ll=None, **kwargs):
    mm = gas_molar_mass_list[ll]
    a = a_VDW_list[ll]
    b = b_VDW_list[ll]
    return Rgas*T/(mm-b*d)-2.*d*a/mm**2+b*d*Rgas*T/(mm-b*d)**2


def dTdP_VDW(d=0., T=0., ll=None, **kwargs):
    mm = gas_molar_mass_list[ll]
    b = b_VDW_list[ll]
...
def rho0_silicates(SiMg=None, Fe_number=None, X_H2O=0.0, T=None, P=None, 
                   phase=None, lay=1):
    ...
    return rho0_silicates + 2*delta*(SiMg - .5)
...
def K0_silicates(SiMg=None, Fe_number=None, X_H2O=0.0, T=None, P=None, 
                 phase=None, lay=1):
    """SiMg and Fe_number are in mole fraction (NOT mol%) but X_H2O is in wt%
    """
    if lay == 1:
...
def K0prime_silicates(SiMg=None, Fe_number=None, X_H2O=0.0, T=None, P=None, 
                      phase=None, lay=1):
    """SiMg and Fe_number are in mole fraction (NOT mol%) but X_H2O is in wt%
    """
    if lay == 1:
        ...
        return K0_prime_silicates + 2*delta*(SiMg - .5)


def Compute(ll=0, what = 'all', P=None, T=None, d=None, \
            dmin=0.001, dmax=3e4, Tmin=10., Tmax=1.0e5, 
            whichEOS=None, Fe_number=0., X_H2O=0., SiMg=None, 
            saturation=False, phase=None, table=False, 
            eps_H2O=0.0, eps_Al=0.0, x_H=0., **kwargs):
    ...
    if not table:
        ...
        try:
            ...
            if ll == 1 or ll == 12 or ll == 13 or ll == 14:
                ...
                if saturation:
                    ...
                    X_H2O = hydration.X_H2O_sat(P=P, T=T, Fe_number=Fe_number,
                                                phase=phase)
                    print ('X_H2O =', X_H2O)
                ...
                aT = max(aT_list_Mg2SiO4[phase] + \
                         X_H2O*daTdH2O_list_Mg2SiO4[phase], 1.0e-7)
                bT = max(bT_list_Mg2SiO4[phase] + \
                         X_H2O*dbTdH2O_list_Mg2SiO4[phase], 1.0e-10)
            ...
            #for Mg(OH)2 (Brucite) different phases are taken into account
                ...
                d0 += -1.62e3*(100.-Fe_number)/Fe_number
                K0 += -40.2e9*(100.-Fe_number)/Fe_number
            ...
            #scaled to the specified iron content (Sun 2018)
                ...
                if saturation:
                    X_H2O = hydration.XH2O_Stv(T=T, P=P, AlSi=0.)*100.
                    d0 -= d0*.0051*X_H2O
        ...
        if whichEOS == 0 and not stopper:
            ...
            if what == 'pres':
                ...
                d = rho_BM(ll=ll, P=P, dmin=dmin, 
                            dmax=dmax, T=T)
            
            elif what == 'dPdrho':
                eta = d/d0T
                dPdrho = dPdrho_BM(eta = eta, K = K, K0prime = K0prime, ll=ll, \
                                   d0T=d0T,
                                   **kwargs)
                ...
                drhodT = ftool.deriv(f=rho_BM, whicharg='T', P=P,
                                           dmax=dmax, dmin=dmin, ll=ll,
                                           x0=T, T0=T0, aT=aT, bT=bT, cT=cT,
                                           acc=1.0e-3,
                                           **kwargs)
        ...
        #Mie-Grüneisen-Debye (MGD)
            ...
            if what == 'pres':
                eta = d/d0
                P = P_MGD(eta = eta, K0 = K0, K0prime = K0prime, ll = ll, \
                          T = T, q=q, molmass=molmass,
                          nparticles=nparticles, gamma0=gamma0, rho0=d0,
                          thetaD0=thetaD0, **kwargs)
            
            elif what == 'dens':
                d = d0*ftool.bisec(f = P_MGD, whicharg = 'eta', K0 = K0, \
                                K0prime = K0prime, ll = ll, T = T, y = P,\
                                a = dmin/d0, b = dmax/d0, q=q, molmass=molmass,
                          nparticles=nparticles, gamma0=gamma0, rho0=d0,
                          thetaD0=thetaD0, **kwargs)
                ...
                eta = d/d0
                T = ftool.bisec(f=P_MGD, whicharg='T', K0 = K0, \
                                K0prime=K0prime, a=Tmin, b=Tmax, y=P,\
                                ll=ll, eta=eta, q=q, molmass=molmass,
                          nparticles=nparticles, gamma0=gamma0, rho0=d0,
                          thetaD0=thetaD0, **kwargs)

            elif what == 'all':
                #Compute density                
                d = rho_MGD(T=T, P=P, dmin=dmin, dmax=dmax, q=q, molmass=molmass,
                            nparticles=nparticles, gamma0=gamma0, rho0=d0,
                            thetaD0=thetaD0, ll=ll, K0=K0, K0prime=K0prime)
                d = d*hydration.delta_rho(P=P, T=T, ll=ll, X_H2O=X_H2O)
                
                #Compute dPdrho at given density and temperature
                eta = d/d0
                dPdrho = dPdrho_MGD(eta = eta, K0 = K0, K0prime = K0prime, ll=ll,
                                    q=q, molmass=molmass,
                          nparticles=nparticles, gamma0=gamma0, rho0=d0,
                          thetaD0=thetaD0, **kwargs)
                
                drhodT = ftool.deriv(f=rho_MGD, whicharg='T', P=P,
                                           dmax=dmax, dmin=dmin, ll=ll,
                                           x0=T,
                                           acc=1.0e-3, q=q, molmass=molmass,
                                           nparticles=nparticles, gamma0=gamma0,
                                           rho0=d0, thetaD0=thetaD0, K0=K0,
                                           K0prime=K0prime
                                           )
        ...
        #Vinet (Vinet)
            ...
            if what == 'pres':
                ...
                d = d0*ftool.bisec(f = P_Vinet, whicharg = 'eta', K0 = K0, \
                                K0prime = K0prime, a = dmin/d0, b = dmax/d0, y=P,\
                                **kwargs)
            
            elif what == 'temp':
                eta = d/d0
                T = ftool.bisec(f=P_Vinet, whicharg='T', K0 = K0, \
                                K0prime=K0prime, a=Tmin, b=Tmax, y=P,\
                                ll=ll, eta=eta, **kwargs)
        ...
        #Belonoshko (Bel)
            ...
            if what == 'pres':
                ...
                print (' Bel:', eta, d0)
                print (' T, rho =', T, d)
                P = P_Bel(eta = eta, K0 = K0, K0prime = K0prime, ll = ll, T = T, \
                          q=q, molmass=molmass, gamma0=gamma0, rho=d0, **kwargs)
            
            elif what == 'dens':
                d = d0*ftool.bisec(f = P_Bel, whicharg = 'eta', K0 = K0, \
                                K0prime = K0prime, a = dmin/d0, b = dmax/d0, y = P,\
                                ll = ll, T = T, q=q, molmass=molmass,
                                gamma0=gamma0, rho0=d0, **kwargs)
                
            elif what == 'temp':
                eta = d/d0
                T = ftool.bisec(f=P_Bel, whicharg='T', K0 = K0, \
                                K0prime=K0prime, a=Tmin, b=Tmax, y=P,\
                                ll=ll, eta=eta, q=q, molmass=molmass,
                                gamma0=gamma0, rho0=d0, **kwargs)
                ...
                #Compute density                
                d = rho_Bel(K0=K0, K0prime=K0prime, ll=ll, q=q, dmin=dmin, dmax=dmax,
                            gamma0=gamma0, rho0=d0, molmass=molmass, T=T, P=P)

                #Compute dPdrho at given density and temperature
                eta = d/d0
                dPdrho = dPdrho_Bel(eta = eta, K0 = K0, K0prime = K0prime, ll = ll, 
                                  T =T, q=q, molmass=molmass,
                                gamma0=gamma0, rho0=d0)
                
                
                alpha = -1./d * ftool.deriv(f=rho_Bel, whicharg='T', P=P,
                                           dmax=dmax, dmin=dmin, ll=ll,
                                           x0=T,
                                           acc=1.0e-3, q=q, molmass=molmass,
                                           nparticles=nparticles, gamma0=gamma0,
        ...
        #Water EOS
            ...
            if what == 'pres':
                ...
                d, dTdP_S, dPdrho, alpha, cp_spec, s_spec, u_spec, phase = \
        ...
        #incompressible substances
            ...
            if what == 'dens':
                print ('WARNING: Incompressible substance in eos.Compute() not available')
        ...
        #Vinet_Rydberg
            ...
            if what == 'pres':
                eta = d/d0
                P = P_VR(eta = eta, K0 = K0, K0prime = K0prime, ll = ll, \
                          T = T, q=q, molmass=molmass,
                          nparticles=nparticles, gamma0=gamma0, rho0=d0,
                          theta0=thetaD0, **kwargs)
            
            elif what == 'dens':
                d = d0*ftool.bisec(f = P_VR, whicharg = 'eta', K0 = K0, \
                                K0prime = K0prime, ll = ll, T = T, y = P,\
                                a = dmin/d0, b = dmax/d0, q=q, molmass=molmass,
                          nparticles=nparticles, gamma0=gamma0, rho0=d0,
                          theta0=thetaD0, **kwargs)
                ...
                eta = d/d0
                T = ftool.bisec(f=P_VR, whicharg='T', K0 = K0, \
                                K0prime=K0prime, a=Tmin, b=Tmax, y=P,\
                                ll=ll, eta=eta, q=q, molmass=molmass,
                          nparticles=nparticles, gamma0=gamma0, rho0=d0,
                          theta0=thetaD0, **kwargs)
                ...
                #Compute density
                d = rho_VR(T=T, P=P, dmin=dmin, dmax=dmax, q=q, molmass=molmass,
                            nparticles=nparticles, gamma0=gamma0, rho0=d0,
                            theta0=thetaD0, ll=ll, K0=K0, K0prime=K0prime)
                
                d = d*hydration.delta_rho(P=P, T=T, ll=ll, X_H2O=X_H2O)
                
                #Compute dPdrho at given density and temperature
                eta = d/d0
                dPdrho = dPdrho_VR(eta = eta, K0 = K0, K0prime = K0prime, ll=ll,
                                    q=q, molmass=molmass,
                          nparticles=nparticles, gamma0=gamma0, rho0=d0,
                          theta0=thetaD0, **kwargs)
                
                drhodT = ftool.deriv(f=rho_VR, whicharg='T', P=P,
                                           dmax=dmax, dmin=dmin, ll=ll,
                                           x0=T,
                                           acc=1.0e-3, q=q, molmass=molmass,
                                           nparticles=nparticles, gamma0=gamma0,
                                           rho0=d0, theta0=thetaD0, K0=K0,
                                           K0prime=K0prime, eta=eta
                                           )
        ...
        #Ichi dude
            ...
            if what == 'pres':
                eta = d/d0
                print ('d, d0 =', d, d0)
                P = P_Ichi(eta = eta, K0 = K0, K0prime = K0prime, ll = ll, \
                          T = T, q=q, molmass=molmass,
                          nparticles=nparticles, gamma0=gamma0, rho0=d0,
                          thetaD0=thetaD0, **kwargs)
            
            elif what == 'dens':
                d = d0*ftool.bisec(f = P_Ichi, whicharg = 'eta', K0 = K0, \
                                K0prime = K0prime, ll = ll, T = T, y = P,\
                                a = dmin/d0, b = dmax/d0, q=q, molmass=molmass,
                          nparticles=nparticles, gamma0=gamma0, rho0=d0,
                          thetaD0=thetaD0, **kwargs)
                ...
                eta = d/d0
                T = ftool.bisec(f=P_Ichi, whicharg='T', K0 = K0, \
                                K0prime=K0prime, a=Tmin, b=Tmax, y=P,\
                                ll=ll, eta=eta, q=q, molmass=molmass,
                          nparticles=nparticles, gamma0=gamma0, rho0=d0,
                          thetaD0=thetaD0, **kwargs)
                ...
                #Compute density
                d = rho_Ichi(T=T, P=P, dmin=dmin, dmax=dmax, q=q, molmass=molmass,
                            nparticles=nparticles, gamma0=gamma0, rho0=d0,
                            thetaD0=thetaD0, ll=ll, K0=K0, K0prime=K0prime)
                ...
                #Compute dPdrho at given density and temperature
                eta = d/d0
                dPdrho = dPdrho_Ichi(eta = eta, K0 = K0, K0prime = K0prime, ll=ll,
                                    q=q, molmass=molmass,
                          nparticles=nparticles, gamma0=gamma0, rho0=d0,
                          thetaD0=thetaD0, **kwargs)
                
                drhodT = ftool.deriv(f=rho_Ichi, whicharg='T', P=P,
                                           dmax=dmax, dmin=dmin, ll=ll,
                                           x0=T,
                                           acc=1.0e-3, q=q, molmass=molmass,
                                           nparticles=nparticles, gamma0=gamma0,
    ...
    #print ('KS =', KS*1e-9)
    #print ('dTdP =', gamma/KS)
    return d, T, P, dTdP_S, dPdrho, phase, X_H2O/100., alpha, xi_Al


def dTdP_ad(ll=None, T=None, phase=None, P=None, Fe_number = 0., 
            saturation =False, gamma=1,):
    stuff = Compute(ll=ll, T=T, P=P, Fe_number=Fe_number, saturation=saturation,
                    phase=phase)
    
    KT = -stuff[0] * stuff[4]
    
    return gamma*T/(1.+gamma*stuff[7]*T)/KT


def PlotCmap(ll=1, Tmin=300, Tmax=2400, Pmin=0, Pmax=30, res=4,
             params = [0], Fe_numbers=[0.0, 25.]):
    ...
    params_scalings = [1., 1., 1., 1.0e-9, 1., 1., 1., 1.0e5, 1.]
    data_lims = [[3000., 4200.], 
                 [-2, 1], 
                 [0., 10.], 
                 [0, 10]]
    ...
    for f in range(len(Fe_numbers)):
        for i in range(N):
            for j in range(N):
                ...
                eos = Compute(ll=ll, T=x[j], P=y[i]*1.0e9, saturation=True,
    ...
    for r in range(nrows):
        ...
        axcols = AxesGrid(fig, (nrows, 1, r+1),
                        nrows_ncols=(1, ncols), 
                        axes_pad = 0.25,
                        share_all = True,
                        label_mode = 'L',
        ...
        for c in range(ncols):
            ...
            im = ax.imshow(zz[c][r], origin='lower', 
                           cmap=cmaps[r],
                           vmin=data_lims[r][0], vmax=data_lims[r][1])
            ...
            if r == 0:
                ax.set_title(r'$\rm {a} \ mol \% \ Fe$'.format(a=str(Fe_numbers[c])),
                             fontsize=label_fontsize)
            
            
        for rax in axcols.cbar_axes:
            rax.colorbar(im)
            rax.tick_params(labelright=True, labelleft=False,
                            left=False, right=True, labelsize=ticklabel_fontsize
                            )
...
def plotFancy(ll = 1,
    ...
    dens = np.array([[Compute(what='all', 
                              ll=ll, 
                              P=p, 
                              T=iso, 
                              Fe_number=Fe_number, 
...
def plotOverview(mats = [1, 9], fnts = 14, N = 20):
    ...
    for i in range(len(mats)):
        ...
        trafo = transforms.blended_transform_factory(ax[i].transAxes, 
                                         ax[i].transAxes) 
        
        a = material_plot_list[mats[i]]
        b = EOSstring_list[EOS_type[mats[i]]]
        label = f'${a} \ ({b})$'
        ax[i].text(.1, .9, label, transform = trafo, fontsize = fnts)
        ax[i].set_xlabel(r'$\rm Pressure \ [Mbar]$', fontsize = fnts)
        ax[i].tick_params(labelsize = fnts)
        ax[i].set_xticks(np.logspace(pres[0] - presScale, 
                                     pres[1] - presScale, 
                                     pres[1] - pres[0] + 1))
        ax[i].set_xlim(10**(pres[0] - presScale), 
                       10**(pres[1] - presScale))
    ...
    u = r'\rm K'
    ax[0].legend(plots, 
                 [f'${T:.0f} \ {u}$' for T in temps], 
                 fontsize = fnts,
                 loc = 3)
    fig.savefig('/home/os18o068/Documents/PHD/Abbildungen/eos.pdf', 
                format = 'pdf', 
                bbox_inches = 'tight')
    plt.close(fig)


def plotTable(N = 2, off = 0, params = [2], ll = 1, cmap = 'jet',
              scalings = [1e-3, 1e-8, 1e5], diff = False,
              starts = [500, 1e5], ends = [3000, 1e11], fnts = 14):
    ...
    tables = []
    for i in range(N):
        tab = eosTablesTest.Table(starts = starts, 
                                  ends = ends, 
                                  scalings = ['lin', 'lin'],
                                  alphas = [i + off, i + off])
        tab.construct_single_axes()
        tab.construct_grid()
        tab.generate(ll = ll, Fe_number = 0.)
        tables.append(tab)
    ...
    if diff:
        ...
        for n in range(N):
            tab = tables[n]
            x = np.linspace(starts[0] * 1.001, ends[0] * .999, NN)
            y = np.linspace(starts[1] * 1.001, ends[1] * .999, NN)
            ...
            for i in range(len(x)):
                for j in range(len(y)):
                    ...
                    di = tab.interpolate(vals = [x[i], y[j]],
                                                   params = params)
                    print ('di =', di)
    ...
    for r in range(len(params)):
        for i in range(N):
            ...
            if diff:
                ...
                im = ax[r][i].imshow(datDiff[i].T[r], 
                                     vmin = -5, 
                                     vmax = -1,
                                     cmap = cmap,
                                     origin = 'lower',
                ...
                ax[r][i].set_xticklabels([f'${t:.0f}$' for t in xTicks],
                                         fontsize = fnts)
                ax[r][i].set_yticklabels([f'${t:.0f}$' for t in yTicks * 1e-9],
                                         fontsize = fnts)
                ...
                if i == N - 1:
                    axin = ax[r][i].inset_axes([1.1, .0, 0.05, 1.])
                    cbar = plt.colorbar(im, cax = axin)
                    cbar.set_label(f'$\log(|\Delta {paramLabels[r]}|/{paramLabels[r]})$',
                                   fontsize = fnts)
                ...
                if i == N - 1:
                    ...
                    addCbar = False
                tab.plot_slice(params = [params[r]], 
                               scalings = ['lin', 'lin'],
                               ax = ax[r][i],
                               fig = fig,
                               label = labels[r],
            ...
            trafo = transforms.blended_transform_factory(ax[r][i].transAxes, 
                                             ax[r][i].transAxes) 
            res = 2**(i + off + 1)
            ax[r][i].text( 
                          0.1, 
    ...
    fig.savefig(r'/home/os18o068/Documents/PHD/Abbildungen/{fn}.pdf'.format(fn = fileName), 
                format = 'pdf', 
                bbox_inches = 'tight')
    plt.close(fig)
...
def Plot(ll=1, N=5, P_min=1.0e5, P_max=1.0e12, mark='', phase=0, 
         Fe_number = 0., **kwargs):
    ...
    for i in range(len(dens)):
       plot, = ax[0].semilogy(dens[i], pres, color=color_list[i],
    ...
    legend1 = ax[0].legend(plot_list1, [str(iso)+' K' for iso in isotherms],
                loc=4, framealpha=.5)
    
    legend2 = ax[1].legend(plot_list1, [str(int(iso))+r'$ \ kg \ m^{-3}$' 
                for iso in isochores], loc=3, framealpha=.5)
    ...
    for axx in ax:
        axx.tick_params(which='both', direction='in', right=True, top=True, 
                        pad=10, labelsize=labelsize)
    ...
    fig.savefig('/home/os18o068/Documents/PHD/Abbildungen/eos.pdf', format = 'pdf', bbox_inches = 'tight')
    plt.close(fig)
