...
import pics.interiors.PlanetFactory as plfac
import numpy as np
...
# M, Mg#, T_surf
accs = [[1.0e-4, 1.0e-3, 1.0e-4], [1.0e-4, 1.0e-3, 1.0e-2], [1.0e-5, 1.0e-3, 1.0e-2]]
...
def generate_data():
    ...
    masses = np.logspace(-1, np.log10(3.0), 6)

    N_params = 24

    data = np.empty([N_params, len(temps), len(Mg_numbers), len(masses)])

    ocean_masses = np.empty([len(temps), len(Mg_numbers), len(masses)])
        ...
        for j in range(len(Mg_numbers)):
            for k in range(len(masses)):
                pl = toolkit.model_hydro(
                    ...
                    temp_jumps=[0.0, delta_T_CMB(masses[k]), 0.0, 0.0, 0.0],
                    Si_number_should=0.3999,
                    P_surface_should=1.0e5,
                    T_surface_should=temps[i],
                    T_center=4000.0,
                    Fe_number_mantle=0.25,
    ...
    for i in range(len(dry1[0])):
        for j in range(len(dry1[0][i])):
            for d in range(len(data1)):
                ...
                if d == 0:
                    (pl1,) = ax2[0][0].plot(
                        dat1[0][i][j], dR, color=color_list[j], linestyle=styles[i]
                    )

                if d == 1:
                    ax2[0][1].plot(
                        dat1[0][i][j], dH2O, color=color_list[j], linestyle=styles[i]
                    )
            ...
            ax2[1][0].plot(
                dat1[0][i][j], ddR1, color=color_list[j], linestyle=styles[i]
            )
            ax2[1][1].plot(
                dat1[0][i][j], ddR2, color=color_list[j], linestyle=styles[i]
            )

    legend1 = ax2[0][0].legend(
        plot_list1, [r"$\rm Mg\# \ = \ 0.2$", r"$\rm Mg\# \ = \ 0.7$"], loc=1
    )
    ax2[0][0].add_artist(legend1)

    legend2 = ax2[0][0].legend(
        plot_list2,
        [r"$\Delta T_{\rm TBL} = 200 \ \rm K$", r"$\Delta T_{\rm TBL} = 1700 \ \rm K$"],
        loc=2,
    )
