...
def print_time(t0, t, **kwargs):
    print(
        "\nelapsed time: "
        + str(s2min(t - t0, **kwargs)[0])
        + "min "
        + str(s2min(t - t0, **kwargs)[1])
        + "s"
    )
...
def fct(x=None, y=None, param=2, table=False, **kwargs):
    ...
    # compute density
    t0 = time.time()
    result = eos.Compute(what="all", T=x, P=y, table=table, **kwargs)[
        param_scaling[param]
    ]
...
class Table:
    ...
    def test_plot(self, param=2, def_zero=1.0e-10):
        ...
        major_tick_labels = [
            np.linspace(int(np.log10(xmin)), int(np.log10(xmax)), nx),
            np.linspace(int(np.log10(ymin)), int(np.log10(ymax)), ny),
        ]

        a, b, c, d = logTrans.transform(data=self.data, param=param, def_zero=def_zero)
    ...
    def Plot(self, param=2, dim=2, log=False, **kwargs):
        ...
        major_tick_labels_x = np.linspace(int(np.log10(xmin)), int(np.log10(xmax)), nx)

        major_tick_labels_y = np.linspace(int(np.log10(ymin)), int(np.log10(ymax)), ny)
        ...
        if param == 33:
            ...
            cbar.ax.set_yticklabels(
                ["solid", "liquid", "vapor", "gas", "supercritical", "high pressure"]
            )
    ...
    def add_data(self, param=2, x_start=0, y_start=0, x_range=1, y_range=1, **kwargs):
                ...
                try:
                    with warnings.catch_warnings():
                        ...
                        if param == 2 and y >= 1.0e8:
                            ...
                            new = phaseCheck.getPhase(
                                T=x, P=y, double_check=True, d=val, ll=ll
                            )
    ...
    def write(self, file_name, loc="cwd"):
        ...
        self.data_table = astropy.table.Table(
            data,
            meta={
        ...
        ascii.write(self.data_table, dir_out + file_name, overwrite=True, format="ecsv")
    ...
    def construct_grid(self):
        ...
        self.x_axis.append(
            10 ** (self.x_range + self.x_start)
            + 10 ** (self.x_range + self.x_start) * 10 ** (-self.alpha_x)
        )

        self.x_axis.append(
            10 ** (self.x_range + self.x_start)
            + 10 ** (self.x_range + self.x_start) * 2 * 10 ** (-self.alpha_x)
        )
        ...
        self.y_axis.append(
            10 ** (self.y_range + self.y_start)
            + 10 ** (self.y_range + self.y_start) * 10 ** (-self.alpha_y)
        )

        self.y_axis.append(
            10 ** (self.y_range + self.y_start)
            + 10 ** (self.y_range + self.y_start) * 2 * 10 ** (-self.alpha_y)
        )
    ...
    def Generate(
        ...
        # interpolation up to desired upper limit of the temperature axis
        self.x_axis.append(
            10 ** (x_range + x_start) + 10 ** (x_range + x_start) * 10 ** (-alpha_x)
        )

        self.x_axis.append(
            10 ** (x_range + x_start) + 10 ** (x_range + x_start) * 2 * 10 ** (-alpha_x)
        )
        ...
        self.y_axis.append(
            10 ** (y_range + y_start)
            + 10 ** (y_range + y_start) * 10 ** (-self.alpha_y)
        )

        self.y_axis.append(
            10 ** (y_range + y_start)
            + 10 ** (y_range + y_start) * 2 * 10 ** (-self.alpha_y)
        )
                ...
                if True:
                    ...
                    if ll == 0:
                        ...
                        try:
                            ...
                            dens, xx, yy, dPdrhoT, ph, XH2O, alpha = eos.Compute(
                                what="all",
                                ll=ll,
                                T=x,
                                P=P_eval,
                            ...
                            if ll == 11:
                                ...
                                m1 = (1 - 0.01 * Fe_number) * mOl + 0.01 * Fe_number * (
                                    mOl - 2 * mMg + 2 * mFe
                                )
                                m2 = mH2O
                    ...
                    if not len(dt_list) == 0:
                        ...
                        space = (
                            total_length - int(percentage / 100 * total_length)
                        ) * " "
                        bar = (int(percentage / 100 * total_length) - 1) * "="
    ...
    def GenerateOlivine(
        self,
        Fe_numbers=[0.0, 5.0, 10.0, 15.0, 20.0, 25.0],
        alpha_x=1,
        alpha_y=1,
        x_range=3,
        y_range=11,
            ...
            for j in range(2):
                ...
                for k in range(3):
                    ...
                    if sat == False:
                        ...
                        self.write_fortran(
                            file_name="eos_" + str(count) + "_fortran.tab"
                        )
    ...
    def reprocess(self, param=2, x_range=[], y_range=[], **kwargs):
        x1, x2 = x_range
        y1, y2 = y_range
        i00, i10 = self.get_index(x1, self.alpha_x), self.get_index(x2, self.alpha_x)
        i01, i11 = self.get_index(y1, self.alpha_y), self.get_index(y2, self.alpha_y)
    ...
    def interpolate_deriv(self, x=None, y=None, order=3, **kwargs):
        ...
        ind = [
            self.get_indices(x, self.alpha_x, order=1),
        ...
        for a in range(2):
            for b in range(2):
                ...
                self.matrix.append(self.row(pnt=[x_list[a], y_list[b]], order=order))
        ...
        for a in range(2):
            for b in range(order - 1):
                ...
                self.matrix.append(
                    self.deriv_x_row(pnt=[x_list[a], y_list[b]], order=order)
                )
        ...
        for a in range(2):
            for b in range(order - 1):
                ...
                self.matrix.append(
                    self.deriv_y_row(pnt=[x_list[a], y_list[b]], order=order)
                )
        ...
        for a in range(order - 1):
            for b in range(order - 1):
                ...
                self.matrix.append(
                    self.deriv_xy_row(pnt=[x_list[a], y_list[b]], order=order)
                )
                # print ('deriv(a,b)=', deriv(x=x_list[a], y=y_list[b], whicharg='y'))
        ...
        result = sum(
            [self.row([x, y], order=order)[i] * self.a[i] for i in range(len(self.a))]
        )
        ...
        for o in range(order - 2):
            ax[0].scatter(
                x_list, [fct(x=t, y=y_list[o], **kwargs)[0] for t in x_list], color="r"
            )
            ax[1].scatter(
                y_list, [fct(x=x_list[o], y=p, **kwargs)[0] for p in y_list], color="b"
            )

            ax[0].plot(
                x_plot_list,
                [fct(x=t, y=y_list[o], **kwargs)[0] for t in x_plot_list],
                color="r",
            )
            ax[1].plot(
                y_plot_list,
                [fct(x=x_list[o], y=p, **kwargs)[0] for p in y_plot_list],
                color="b",
            )
            ...
            for t in x_plot_list:
                temp_results.append(
                    sum(
                        [
                            self.row([t, y], order=order)[i] * self.a[i]
            ...
            for p in y_plot_list:
                pres_results.append(
                    sum(
                        [
                            self.row([x, p], order=order)[i] * self.a[i]
                            for i in range(len(self.a))
                        ]
                    )
                )
            ...
            ax[0].scatter(
                x, fct(x=x, y=y_list[o], **kwargs), color="r", zorder=10, marker="x"
            )
            ax[1].scatter(
                y, fct(x=x_list[o], y=y, **kwargs), color="b", zorder=10, marker="x"
            )

            ax[0].scatter(x, fct(x=x, y=y, **kwargs), color="k", zorder=10, marker="x")
            ax[1].scatter(y, fct(x=x, y=y, **kwargs), color="k", zorder=10, marker="x")

            ax[0].scatter(x, result, color="k", zorder=10, marker="o")
            ax[1].scatter(y, result, color="k", zorder=10, marker="o")

            print("elapsed time for interpolation:", round(dt_int * 1000, 3), "ms")
            print("elapsed time for direct evaluation:", round(dt_fct * 1000, 3), "ms")
            print("time gain factor:", round(dt_fct / dt_int, 3))
        ...
        if not terminate:
            ...
            if plot:
                ...
                for o in range(order + 1):
                    # gather grid points
                    valx_list = [
                        self.data[ind[0][o]][j][param] for j in range(order + 1)
                    ]
                    valy_list = [
                        self.data[i][ind[1][o]][param] for i in range(order + 1)
                    ]
                    ...
                    ax[0].plot(
                        x_plot_list,
                        [
                            fct(x=t, y=y_list[o], param=param, **kwargs)
                            for t in x_plot_list
                        ],
                        color="r",
                    )
                    ax[1].plot(
                        y_plot_list,
                        [
                            fct(x=x_list[o], y=p, param=param, **kwargs)
                            for p in y_plot_list
                        ],
                        color="b",
                    )
                ...
                ax[0].scatter(
                    x,
                    fct(x=x, y=y, param=param, **kwargs),
                    color="k",
                    zorder=10,
                    marker="x",
                )
            ...
            if compare:
                ...
                print("elapsed time for interpolation:", round(dt_int * 1000, 3), "ms")
                print(
                    "elapsed time for direct evaluation:",
                    round(actual_time * 1000, 3),
                    "ms",
                )
                print("time gain factor:", round(dt_fct / dt_int, 2))
