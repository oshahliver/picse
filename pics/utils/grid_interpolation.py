...
class Grid:
    def __init__(
        ...
        self.param_scales = [1.0, 1.0, 1.0, 1.0 / r_earth, 1.0, 1.0, 1.0]
        self.param_strings = [
            r"$T \ [\rm K]$",
    ...
    def compute_output_grid(self, iter_list=None, ph=0, **kwargs):
        ...
        planet = run_planet(
            T_surface=params[0], P_surface=10 ** params[1], ocean_mass_frac=params[2]
        )
    ...
    def generate(self, **kwargs):
        ...
        for ph in range(self.n_phase):
            ...
            self.for_recursive(f=self.compute_output_grid, ranges=self.ranges, **kwargs)
            self.it_counter = 0
    ...
    def interpolate(self, vals=None, order=1, params=[2], warnings="on", phase=0):
    ...
    def plot_slice(
        self, axes=[0, 1], slice=0, param=3, ph=0, extent=None, cmap="viridis"
    ):
        ...
        ax.set_title(
            self.param_strings[axx[0]]
            + " = "
            + str(self.single_axes[ph][axx[0]][slice])
        )
